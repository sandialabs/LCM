...
template <typename T>
inline MDFieldIterator<T>::MDFieldIterator(PHX::MDField<T>& a) : a_(a)
{
  rank_ = a.rank();
  done_ = a.size() == 0;
...
template <typename T>
inline typename MDFieldIterator<T>::return_type
MDFieldIterator<T>::ref()
{
  switch (rank_) {
    ...
    default: ALBANY_ABORT("not impl'ed"); return a_(0);
  }
}
...
void
loop(Functor& f, PHX::MDField<ScalarT>& a)
{
  switch (a.rank()) {
    case 1: {
      int ctr = 0;
...
template <class Functor, typename ScalarT, typename T1, typename T2>
void
loop(Functor& f, const PHX::MDField<ScalarT, T1, T2>& a)
{
...
struct SetLooper
{
  ...
  void
  operator()(typename Ref<ScalarT>::type a, int)
  {
    a = val;
  }
};
...
create_copy(std::string const& name, const Kokkos::DynRankView<T, P...>& src)
{
  ...
  if constexpr (Sacado::IsADType<value_type>::value) {
    ...
    if      (r == 0) dst = dst_type(name);
    else if (r == 1) dst = dst_type(name, src.extent(0));
    else if (r == 2) dst = dst_type(name, src.extent(0), src.extent(1));
    else if (r == 3) dst = dst_type(name, src.extent(0), src.extent(1), src.extent(2));
    else if (r == 4) dst = dst_type(name, src.extent(0), src.extent(1), src.extent(2), src.extent(3));
    else             dst = dst_type(name, src.extent(0), src.extent(1), src.extent(2), src.extent(3), src.extent(4));
  }
