...
Albany::PUMIQPData<DataType, Dim, traits>::PUMIQPData(
      ...
      beginning_index(0)
{
  for (std::size_t i = 1; i < dims.size();
       i++)  // multiply it by the number of dofs per node
...
template <typename DataType, unsigned Dim, class traits>
void
Albany::PUMIQPData<DataType, Dim, traits>::reAllocateBuffer(
...
Albany::PUMIQPData<DataType, Dim, traits>::getMDA(const std::size_t nelems)
{
  ...
  field_type the_array =
      traits_type::buildArray(&buffer[beginning_index], nelems, dims);
