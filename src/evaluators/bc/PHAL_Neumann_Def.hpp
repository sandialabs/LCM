...
NeumannBase<EvalT, Traits>::NeumannBase(const Teuchos::ParameterList& p)
    :
      ...
      meshSpecs(
          p.get<Teuchos::RCP<Albany::MeshSpecsStruct>>("Mesh Specs Struct")),
          ...
          dl->vertices_vector)
{
  ...
  if (inputConditions == "scaled jump") {
    ...
    robin_vals[1] =
        inputValues[1];  // coeff multiplying difference 'dof^4 - dof_value^4'
  ...
  // is there a "(" in the string?
      ...
      (position = inputConditions.find_first_of("(")) != std::string::npos) {
  ...
  cubatureCell = cubFactory.create<PHX::Device, RealType, RealType>(
      *cellType, meshSpecs->cubatureDegree);

  int cubatureDegree = (p.get<int>("Cubature Degree") > 0) ?
                           p.get<int>("Cubature Degree") :
  ...
  for (int i = 0; i < numSidesOnElem; ++i) {
    sideType[i] =
        Teuchos::rcp(new shards::CellTopology(elem_top->side[i].topology));
    cubatureSide[i] = cubFactory.create<PHX::Device, RealType, RealType>(
        *sideType[i], cubatureDegree);
    ...
    PHX::FieldManager<Traits>& fm)
{
  ...
  if (inputConditions == "robin" || inputConditions == "radiate") {
    ...
    dofSide_buffer = Kokkos::createDynRankView(
        dof.get_view(), "dofSide", numCells * maxNumQpSide * numDOFsSet);
  }
  ...
  physPointsCell_buffer = Kokkos::createDynRankView(
      coordVec.get_view(), "physPointsCell", numCells * numNodes * cellDims);
  temporary_buffer = Kokkos::createDynRankView(
      coordVec.get_view(),
      "temporary_buffer",
      numCells * maxNumQpSide * cellDims * cellDims);

  cubPointsSide_buffer = Kokkos::DynRankView<RealType, PHX::Device>(
      "cubPointsSide", maxNumQpSide * maxSideDim);
  refPointsSide_buffer = Kokkos::DynRankView<RealType, PHX::Device>(
      "refPointsSide", maxNumQpSide * cellDims);
  cubWeightsSide_buffer = Kokkos::DynRankView<RealType, PHX::Device>(
      "cubWeightsSide", maxNumQpSide);
  basis_refPointsSide_buffer = Kokkos::DynRankView<RealType, PHX::Device>(
      "basis_refPointsSide", numNodes * maxNumQpSide);

  physPointsSide_buffer = Kokkos::createDynRankView(
      coordVec.get_view(),
      "physPointsSide",
      numCells * maxNumQpSide * cellDims);
  jacobianSide_buffer = Kokkos::createDynRankView(
      coordVec.get_view(),
      "jacobianSide",
      numCells * maxNumQpSide * cellDims * cellDims);
  jacobianSide_det_buffer = Kokkos::createDynRankView(
      coordVec.get_view(), "jacobianSide", numCells * maxNumQpSide);
  weighted_measure_buffer = Kokkos::createDynRankView(
      coordVec.get_view(), "weighted_measure", numCells * maxNumQpSide);
  trans_basis_refPointsSide_buffer = Kokkos::createDynRankView(
      coordVec.get_view(),
      "trans_basis_refPointsSide",
      numCells * numNodes * maxNumQpSide);
  weighted_trans_basis_refPointsSide_buffer = Kokkos::createDynRankView(
      coordVec.get_view(),
      "weighted_trans_basis_refPointsSide",
      numCells * numNodes * maxNumQpSide);
  side_normals_buffer = Kokkos::createDynRankView(
      coordVec.get_view(), "side_normals", numCells * maxNumQpSide * cellDims);
  normal_lengths_buffer = Kokkos::createDynRankView(
      coordVec.get_view(), "normal_lengths", numCells * maxNumQpSide);

  if (inputConditions == "robin" || inputConditions == "radiate") {
    dofCell_buffer = Kokkos::createDynRankView(
        dof.get_view(), "dofCell", numCells, numNodes, numDOFsSet);
  }
    ...
    typename Traits::EvalData workset)
{
  ...
  data_buffer = Kokkos::createDynRankView(
      neumann, "data", numCells * maxNumQpSide * numDOFsSet);
  ...
  for (int ib = 0; ib < ordinalEbIndex.size(); ib++) {
    ...
    for (int is = 0; is < numSidesOnElem; is++) {
      cellsOnSidesOnBlocks[ib][is] = Kokkos::DynRankView<int, PHX::Device>(
          "cellOnSide_i", numCellsOnSidesOnBlocks[ib][is]);
  ...
  for (int iblock = 0; iblock < ordinalEbIndex.size(); ++iblock)
    for (int side = 0; side < numSidesOnElem; ++side) {
      ...
      Kokkos::DynRankView<int, PHX::Device> cellVec =
          cellsOnSidesOnBlocks[iblock][side];

      // need to resize containers because they depend on side topology
      cubPointsSide =
          DynRankViewRealT(cubPointsSide_buffer.data(), numQPsSide, sideDims);
      refPointsSide =
          DynRankViewRealT(refPointsSide_buffer.data(), numQPsSide, cellDims);
      cubWeightsSide =
          DynRankViewRealT(cubWeightsSide_buffer.data(), numQPsSide);
      basis_refPointsSide = DynRankViewRealT(
          basis_refPointsSide_buffer.data(), numNodes, numQPsSide);

      physPointsSide = Kokkos::createViewWithType<DynRankViewMeshScalarT>(
          physPointsSide_buffer,
          physPointsSide_buffer.data(),
          numCells_,
          numQPsSide,
          cellDims);
      jacobianSide = Kokkos::createViewWithType<DynRankViewMeshScalarT>(
          jacobianSide_buffer,
          jacobianSide_buffer.data(),
          numCells_,
          numQPsSide,
          cellDims,
          cellDims);
      jacobianSide_det = Kokkos::createViewWithType<DynRankViewMeshScalarT>(
          jacobianSide_det_buffer,
          jacobianSide_det_buffer.data(),
          numCells_,
          numQPsSide);
      weighted_measure = Kokkos::createViewWithType<DynRankViewMeshScalarT>(
          weighted_measure_buffer,
          weighted_measure_buffer.data(),
          numCells_,
          numQPsSide);
      trans_basis_refPointsSide =
          Kokkos::createViewWithType<DynRankViewMeshScalarT>(
              trans_basis_refPointsSide_buffer,
              trans_basis_refPointsSide_buffer.data(),
              numCells_,
              numNodes,
              numQPsSide);
      weighted_trans_basis_refPointsSide =
      ...
      for (std::size_t iCell = 0; iCell < numCells_; ++iCell) {
        for (std::size_t node = 0; node < numNodes; ++node) {
          for (std::size_t dim = 0; dim < cellDims; ++dim) {
            physPointsCell(iCell, node, dim) =
                coordVec(cellVec(iCell), node, dim);
          }
        }
      }
        ...
        for (std::size_t iCell = 0; iCell < numCells_; ++iCell) {
          for (std::size_t node = 0; node < numNodes; ++node) {
            for (std::size_t icomp = 0; icomp < numDOFsSet; ++icomp) {
              if (vectorDOF) {
                dofCell(iCell, node, icomp) =
                    dof(cellVec(iCell), node, this->offset[icomp]);
              } else {
      ...
      // (elem_side)
      data =
          Kokkos::createViewWithType<Kokkos::DynRankView<ScalarT, PHX::Device>>(
              data_buffer,
              data_buffer.data(),
              numCells_,
              numQPsSide,
      ...
      switch (bc_type) {
        ...
        case CLOSED_FORM:
          ...
          calc_closed_form(
              data, physPointsSide, jacobianSide, *cellType, side, workset);
          break;
        default:
          ...
          for (std::size_t qp = 0; qp < numQPsSide; ++qp)
            for (std::size_t dim = 0; dim < numDOFsSet; ++dim)
              neumann(cell, node, dim) +=
                  data(iCell, qp, dim) *
                  weighted_trans_basis_refPointsSide(iCell, node, qp);
      }
    }
}
...
template <typename EvalT, typename Traits>
void
NeumannBase<EvalT, Traits>::calc_traction_components(
    ...
    int                                                  local_side_id) const
{
  ...
  Kokkos::DynRankView<ScalarT, PHX::Device> grad_T = Kokkos::createDynRankView(
      qp_data_returned, "grad_T", numCells_, numPoints, cellDims);
  using DynRankViewMeshScalarT = Kokkos::DynRankView<MeshScalarT, PHX::Device>;
  DynRankViewMeshScalarT side_normals =
      Kokkos::createDynRankViewWithType<DynRankViewMeshScalarT>(
          side_normals_buffer,
          side_normals_buffer.data(),
          numCells_,
          numPoints,
          cellDims);
  DynRankViewMeshScalarT normal_lengths =
      Kokkos::createDynRankViewWithType<DynRankViewMeshScalarT>(
          normal_lengths_buffer,
          normal_lengths_buffer.data(),
          numCells_,
          numPoints);
  ...
  */
  ...
  // direction vector
  ICT::getPhysicalSideNormals(
      side_normals, jacobian_side_refcell, local_side_id, celltopo);

  // scale normals (unity)
  IRST::vectorNorm(normal_lengths, side_normals, Intrepid2::NORM_TWO);
  IFST::scalarMultiplyDataData<MeshScalarT>(
      side_normals, normal_lengths, side_normals, true);
...
template <typename EvalT, typename Traits>
void
NeumannBase<EvalT, Traits>::calc_dudn_const(
    Kokkos::DynRankView<ScalarT, PHX::Device>& qp_data_returned,
    ScalarT                                    scale) const
{
  int numCells_ = qp_data_returned.extent(
  ...
  for (int side = 0; side < numCells_; side++) {
    for (int pt = 0; pt < numPoints; pt++) {
      for (int dim = 0; dim < numDOFsSet; dim++) {
        qp_data_returned(side, pt, dim) =
            -const_val * scale;  // User directly specified dTdn, just use it
      }
    }
  }
}
    ...
    const Kokkos::DynRankView<ScalarT, PHX::Device>& dof_side) const
{
  ...
  for (int side = 0; side < numCells_; side++) {
    for (int pt = 0; pt < numPoints; pt++) {
      for (int dim = 0; dim < numDOFsSet; dim++) {
        qp_data_returned(side, pt, dim) =
            coeff * (dof_side(side, pt, dim) - dof_value);
      }
    }
  }
}
    ...
    for (int pt = 0; pt < numPoints; pt++) {
      for (int dim = 0; dim < numDOFsSet; dim++) {
        qp_data_returned(cell, pt, dim) =
            coeff * (dof_side(cell, pt, dim) * dof_side(cell, pt, dim) *
    ...
    int                                                  local_side_id) const
{
  ...
  for (int cell = 0; cell < numCells_; cell++)
    for (int pt = 0; pt < numPoints; pt++)
      for (int dim = 0; dim < numDOFsSet; dim++)
        qp_data_returned(cell, pt, dim) =
            const_val * side_normals(cell, pt, dim);
}
...
Neumann<PHAL::AlbanyTraits::Residual, Traits>::Neumann(
...
template <typename Traits>
Neumann<PHAL::AlbanyTraits::Jacobian, Traits>::Neumann(
    Teuchos::ParameterList& p)
...
//          this->fT->sumIntoLocalValue(rowT, this->neumann(cell, node,
//          dim).val());
//       }
...
//               jacobian.sumIntoValues(colT[0], &rowT,1, &value[0], false,
//               true);
//             }
//             else {
//               // Sum Jacobian
...
template <typename Traits>
Neumann<PHAL::AlbanyTraits::Tangent, Traits>::Neumann(Teuchos::ParameterList& p)
...
template <typename Traits>
void
Neumann<PHAL::AlbanyTraits::Tangent, Traits>::evaluateFields(
    typename Traits::EvalData workset)
{
  auto nodeID = workset.wsElNodeEqID;
      ...
      for (std::size_t dim = 0; dim < this->numDOFsSet; ++dim) {
        ...
        if (JV != Teuchos::null) {
          for (int col = 0; col < workset.num_cols_x; col++) {
            JV_nonconst2dView[col][row] +=
                this->neumann(cell, node, dim).dx(col);
          }
        }
        if (fp != Teuchos::null) {
          for (int col = 0; col < workset.num_cols_p; col++) {
            fp_nonconst2dView[col][row] +=
                this->neumann(cell, node, dim).dx(col + workset.param_offset);
          }
        }
      }
    }
...
template <typename Traits>
Neumann<PHAL::AlbanyTraits::DistParamDeriv, Traits>::Neumann(
    Teuchos::ParameterList& p)
...
template <typename Traits>
void
Neumann<PHAL::AlbanyTraits::DistParamDeriv, Traits>::evaluateFields(
    typename Traits::EvalData workset)
{
  auto                                     nodeID = workset.wsElNodeEqID;
  ...
  if (trans) {
    ...
    const Albany::IDArray& wsElDofs =
        workset.distParamLib->get(workset.dist_param_deriv_name)
        ...
        for (int col = 0; col < num_cols; col++) {
          ...
          for (std::size_t node = 0; node < this->numNodes; ++node) {
            for (std::size_t dim = 0; dim < this->numDOFsSet; ++dim) {
              ...
              val += this->neumann(cell, node, dim).dx(i) *
                     local_Vp[node * neq + eq][col];
            }
          }
...
template <typename EvalT, typename Traits>
void
NeumannAggregator<EvalT, Traits>::postRegistrationSetup(
    typename Traits::SetupData d,
