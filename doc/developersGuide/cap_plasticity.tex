...
\section{Planned Extension: Permafrost and Erosion}
\label{sec:permafrost}

The purpose of this model in LCM is to eventually replace the
\texttt{J2Erosion} model in the ACE permafrost-erosion application
\citep{Frederick.etal:2021}. \texttt{J2Erosion} extends $J_2$
plasticity with an ice-saturation--dependent yield stress
($\sigma_0 = \sigma^{\mathrm{sed}} + f(T)\,
\sigma^{\mathrm{ice-sed}}$, with $f(T) \in [0,1]$ the ice saturation
from the sediment freezing curve) and, over time, has accumulated
five failure criteria and several empirical adjustments (polynomial
fits of stiffness and strength in ice saturation and porosity,
ocean-exposure weakening factors, a peat-dependent strain limit).
This section records the assessment and design decisions for the
replacement, so the work can be resumed.
...
current ice model. At the other extreme, thawed sediment is the
material the cap model was designed for: frictional shear strength
plus compaction of a porous skeleton, neither of which $J_2$
represents (in \texttt{J2Erosion} the thawed state is a constant
residual yield stress). Two further physical arguments: frozen soils
exhibit a closed strength envelope (strength rises with confinement,
peaks, and falls at high pressure through pressure melting of pore
ice), which the cap closure can represent phenomenologically and a
$J_2$ cylinder cannot; and thaw settlement is compaction --- crush-curve
physics, with $W$ supplied by the effective-porosity profiles the
ACE model already carries.

\paragraph{Ice-saturation coupling.}
The cap parameters separate the strength mechanisms that $J_2$
lumps, and the ice saturation enters each by its mechanism:
cohesion (ice cementation) through $A - C$ and $N$ as
$c(f) = c^{\mathrm{sed}} + f\,c^{\mathrm{ice}}$, the direct analogue
of the \texttt{J2Erosion} rule; friction ($\theta$, $D$) as a
sediment property, approximately independent of $f$; pore-collapse
resistance ($\kappa_0$, $D_1$) increasing with $f$ as ice fills the
pores; crushable volume $W$ from the effective porosity; stiffness
$E(f)$ from the existing experimental fits. The implementation
should be structured as two calibrated end-member parameter sets ---
frozen ($f = 1$) and thawed ($f = 0$) --- with one interpolation rule
per mechanism, replacing the scattered polynomial fits.
...
should change. The thaw transition replaces pore ice
...
The replacement should therefore take two end-member pairs
$(K, G)_{\mathrm{frozen}}$ and $(K, G)_{\mathrm{thawed}}$ in place of
the modulus-multiplier fits, with $G(f)$ interpolated geometrically
(log-linearly --- the range spans two orders of magnitude, so linear
interpolation in $f$ overweights the frozen value) and $K(f)$
interpolated linearly down to the Wood-mixture value computed from
the ACE porosity profile. Above the water table or in gassy
material the Wood bound disappears (a percent of pore air destroys
it), for which the ACE air-fraction field is the natural switch.
Because the kernel reads per-point $E$ and $\nu$ fields, this is an
evaluator-level change with no impact on the integrator. One
numerical caution: the thawed limit $\nu \to 0.499$ volumetrically
locks low-order hexahedra, so the effective $\nu$ should be capped
near $0.45$ unless mean-dilatation element technology is adopted.
The $(K, G)$ split also makes elasticity consistent with the
strength assumption above: thaw removes the shear-carrying ice phase
from both.
...
temperature-dependent yield in metals, $\kappa$ must store only the
plastic compaction history while the surfaces are built from
$f$-dependent parameters evaluated fresh each step.
...
at the default 0:
\begin{itemize}
\item \emph{Tension} (bit 0x01): the fraction of the zero-pressure
  shear capacity $F_f(0) - N$ lost to mean tension,
...
\item \emph{Backstress saturation} (bit 0x02):
  $\sqrt{J_2^\alpha}/N$, equal to 1 when the kinematic hardening is
  exhausted ($G^\alpha = 0$: the yield surface has translated to the
  failure envelope and the shear response is perfectly plastic).
  This replaces the criterion that yield onset itself constitutes
  failure --- the cap model can distinguish yielded-but-hardening
  material from exhausted material, which $J_2$ cannot.
\item \emph{Crush exhaustion} (bit 0x04):
  $|\varepsilon_v^p(\kappa)|/W$ on the crush curve, the quantity
  whose approach to 1 engages the cap lock (full pore collapse).
\item \emph{Plastic distortion} (bit 0x20):
  $\varepsilon_{\mathrm{eqps}}$ against a user limit, replacing the
  total-distortion limit.
\end{itemize}
...
(bit 0x08) and displacement norm (bit 0x10), which detect toppled or
departed blocks --- are not constitutive and carry over from
...
its integration points have failed, all points by default. All
criteria branch on Sacado values so the residual and Jacobian
evaluations take identical paths.

\paragraph{Implementation plan.}
(1)~A separate \texttt{CapErosion} kernel wrapping the verified
integrator with the ACE plumbing (ice-saturation field,
porosity/peat depth profiles, sea level, erodible cell sets, death
infrastructure), leaving \texttt{CapModel} pure. (2)~Thermal
stretch on $\Fdef$ (and optionally the $9\,\%$ freezing expansion),
which the cap kernel currently lacks. (3)~The saturation-to-parameter
map above. (4)~The failure indicators above. (5)~Later, in order of
expected payoff: rate sensitivity by the Duvaut--Lions wrap of
\citet{Regueiro.Foster:2011}, which regularizes any inviscid
...
Phases 0 and 1 are implemented: the model exists as \texttt{Permafrost}
(material-database name \texttt{Permafrost}), built on the shared
verified integrator, with frozen/thawed end-member parameter sublists,
the saturation-to-parameter map above (including the $(K, G)$
elasticity split with the capped effective Poisson ratio), ice
saturation from the ACE field, a time table, or a constant, the
within-step parameter ramping described in
Section~\ref{sec:integration}, and thermal stretch in the
finite-deformation path. Verification: equivalence with \texttt{Cap}
at $f = 1$, the full map at constant $f = 0.5$, and thaw under load
($f: 1 \to 0.3$ during confined compression) all match the reference
implementation at machine precision. Phase 2 is implemented as well:
the failure indicators and the element-death bookkeeping above, with
per-mode single-element verification --- for each criterion the
indicator series matches the reference at machine precision and the
...
multiset against the analytic Gauss-point displacements). The death
states are consumed by the existing ACE solver and scatter machinery
without modification.

Phase 3 (ACE workflow) is implemented: depth profiles of porosity and
peat (\texttt{ACE Z Depth File}, \texttt{ACE\_Porosity File},
\texttt{ACE Peat File}, or the constant \texttt{ACE Bulk Porosity}),
sea level against time, and ocean-exposure weakening. The porosity
overrides the crushable volume $W$ at each integration point --- the
pore space is the crushable volume, independent of what fills it ---
verified at machine precision against two reference runs through the
two Gauss-point depth groups of a single-element deck. Peat raises
the eqps limit by the factor $(1 + \mathrm{peat})$, a provisional
parity rule pending calibration. Submerged cells of the erodible
sets are weakened: the cohesion by its factor ($C$ rises toward $A$,
shrinking the zero-pressure strength $A - C$ and the offset $N$
while leaving the friction slope and asymptote intact), the moduli
uniformly (so $\nu$, and the stored-stress re-expression, are
unaffected), and the eqps limit by its factor. Unlike the
saturation, the weakening switches on instantaneously when a cell
submerges (the \texttt{J2Erosion} convention); if waterline flips
ever destabilize the drift correction the way parameter jumps did in
Phase 1, the submersion state should be ramped the same way. The
full erosion loop --- live death, the active-part update, workset
rebuilds on the shared ACE mesh --- runs with \texttt{Permafrost} in
a MiniErosion-style coupled test in which a deliberately tight
displacement limit kills the settled upper cells early and the run
continues on the eroded mesh. One calibration note: because the cap
position $\kappa$ is plastic memory while $\kappa_0(f)$ is a
parameter, a state integrated under frozen parameters and then read
under thawed ones sits far inside the thawed crush curve, i.e.\ it
is reinterpreted as heavily precompacted; end members should be
calibrated with a common $\kappa_0$ unless that reinterpretation is
intended. Remaining for validation (Phase 4): frozen/thawed
end-member calibration and the bluff-scale side-by-side against
\texttt{J2Erosion}.

\paragraph{Calibration.}
Thawed end member from standard soil-mechanics cap parameters for
silty/clay sediments plus the ACE porosity profiles ($W$); frozen
end member by refitting the experimental data behind the existing
\texttt{J2Erosion} stiffness/strength fits to cap parameters, with
friction ($\theta$, $D$) taken from the thawed sediment so the
frozen fit reduces to cohesion scaling --- the original
\texttt{J2Erosion} assumption, now in its natural place. The
