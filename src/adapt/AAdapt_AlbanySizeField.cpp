...
AAdapt::AlbanySizeField::AlbanySizeField(
...
void
AAdapt::AlbanySizeField::adaptMesh(
    const Teuchos::RCP<Teuchos::ParameterList>& adapt_params_)
{
  ...
  /* Debugging text
    for(size_t i = 0; i < mesh_struct->getMesh()->countFields(); i++){
      ...
      std::cout << "AlbanySizeField: found field name: " <<
    apf::getName(field_n) << std::endl;
    }
  */

  apf::Field* field =
      mesh_struct->getMesh()->findField("proj_nodal_IsoMeshAdaptMethod");
  TEUCHOS_TEST_FOR_EXCEPTION(
      field == NULL,
      std::logic_error,
      "Cannot find proj_nodal_IsoMeshAdaptMethod");

  ma::Input* in = ma::configure(mesh_struct->getMesh(), field);

  in->maximumIterations =
...
{
  TEUCHOS_TEST_FOR_EXCEPTION(
      mesh_struct->nodal_data_base.is_null(),
      std::logic_error,
      "Mesh Adapt: Attempting to adapt the mesh to a size field and none are "
      "defined");

  const Teuchos::RCP<Albany::NodeFieldContainer> node_states =
      mesh_struct->nodal_data_base->getNodeContainer();
  apf::Mesh2* const m = mesh_struct->getMesh();

  for (Albany::NodeFieldContainer::iterator nfs = node_states->begin();
       nfs != node_states->end();
       ++nfs) {
    Teuchos::RCP<Albany::PUMINodeDataBase<RealType>> nd =
        Teuchos::rcp_dynamic_cast<Albany::PUMINodeDataBase<RealType>>(
            nfs->second);
    TEUCHOS_TEST_FOR_EXCEPTION(
        nd.is_null(),
        std::logic_error,
    ...
    switch (nd->ndims()) {
      case 0:
        value_type = apf::SCALAR;
        nentries   = 1;
        break;
      case 1:
        value_type = apf::VECTOR;
        nentries   = spdim;
        break;
      case 2:
        value_type = apf::MATRIX;
        nentries   = spdim * spdim;
        break;
      default:
        TEUCHOS_TEST_FOR_EXCEPTION(
...
{
  //  std::cout << "Adapt - destroying Node Field:
  //  proj_nodal_IsoMeshAdaptMethod" << std::endl;
  apf::destroyField(
      mesh_struct->getMesh()->findField("proj_nodal_IsoMeshAdaptMethod"));
}
