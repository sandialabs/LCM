...
ScaledSizeField::ScaledSizeField(
...
void
ScaledSizeField::adaptMesh(
    const Teuchos::RCP<Teuchos::ParameterList>& adapt_params_)
{
  ma::IsotropicFunction* isf =
      dynamic_cast<ma::IsotropicFunction*>(&scaledIsoFunc);
...
void
ScaledSizeField::preProcessOriginalMesh()
{
  scaledIsoFunc.averageEdgeLength_ =
      ma::getAverageEdgeLength(mesh_struct->getMesh());
}

void
ScaledSizeField::preProcessShrunkenMesh()
{
}

void
ScaledSizeField::setParams(const Teuchos::RCP<Teuchos::ParameterList>& p)
{
