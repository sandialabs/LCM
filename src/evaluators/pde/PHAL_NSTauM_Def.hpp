...
template <typename EvalT, typename Traits>
void
NSTauM<EvalT, Traits>::postRegistrationSetup(
...
template <typename EvalT, typename Traits>
void
NSTauM<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
    ...
    for (std::size_t qp = 0; qp < numQPs; ++qp) {
      ...
      for (std::size_t i = 0; i < numDims; ++i) {
        for (std::size_t j = 0; j < numDims; ++j) {
          TauM(cell, qp) += rho(cell, qp) * rho(cell, qp) * V(cell, qp, i) *
                            Gc(cell, qp, i, j) * V(cell, qp, j);
          normGc(cell, qp) += Gc(cell, qp, i, j) * Gc(cell, qp, i, j);
        }
      }
      TauM(cell, qp) +=
          12. * mu(cell, qp) * mu(cell, qp) * std::sqrt(normGc(cell, qp));
