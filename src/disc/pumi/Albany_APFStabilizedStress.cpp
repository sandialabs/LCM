...
static void
stabilizeStress(apf::Field* cauchy, apf::Field* pressure)
{
  ...
  while ((elem = m->iterate(elems))) {
    ...
    for (unsigned qp = 0; qp < nqp; ++qp) {
      ...
      for (unsigned i = 1; i < numDims; ++i) dilatation += sigma[i][i];
      dilatation /= numDims;

      for (unsigned i = 0; i < numDims; ++i) sigma[i][i] += p - dilatation;
...
void
APFDiscretization::saveStabilizedStress()
{
