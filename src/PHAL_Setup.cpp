...
bool
Setup::memoizer_active() const
{
  return _enableMemoization;
}

bool
Setup::memoizer_for_params_active() const
{
...
Setup::update_fields()
{
  if (_enableMemoization) {
    ...
    if (_enableMemoizationForParams) update_fields_with_unsaved_params();
  }
}
...
void
Setup::print(std::ostream& os) const
{
  os << "************* Phalanx Setup **************" << std::endl;
  os << "************ Evaluation Types ************" << std::endl;
  for (const auto& eval : *_setupEvals) os << "  " << eval << std::endl;
  os << std::endl;
...
void
Setup::print_fields(std::ostream& os) const
{
    ...
    Teuchos::RCP<StringSet> unsavedFields)
{
  if (_enableMemoization) {
    ...
    while (!unsavedStack.empty()) {
      ...
      // If unsaved field is used to evaluate fields, add evaluated fields to
      // list of unsaved
...
void
Setup::update_fields_with_unsaved_params()
{
  if (_enableMemoizationForParams && !_unsavedParams->empty()) {
