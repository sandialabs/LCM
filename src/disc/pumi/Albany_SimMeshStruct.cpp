    ...
    const Teuchos::RCP<const Teuchos_Comm>&     commT)
{
  ...
  outputInterval = params->get<int>(
      "Sim Write Interval", 1);  // write every time step default
  ...
  model = gmi_sim_load(
      native_file.empty() ? 0 : native_file.c_str(),
...
Albany::SimMeshStruct::getValidDiscretizationParameters() const
{
  ...
  validPL->set<std::string>(
      "Sim Input File Name", "", "File Name For Sim Mesh Input");
  validPL->set<std::string>(
      "Sim Output File Name", "", "File Name For Sim Mesh Output");
  validPL->set<std::string>(
      "Sim Model Input File Name", "", "File Name For Sim Mesh Output");
  validPL->set<std::string>(
      "Sim Restart File Name",
      "",
      "read initial solution field from this file");
  validPL->set<double>(
      "Sim Restart Time", 0, "simulation time to restart from");
  validPL->set<int>(
      "Number Of Time Derivatives", -1, "Number of time derivatives");
  validPL->set<bool>(
