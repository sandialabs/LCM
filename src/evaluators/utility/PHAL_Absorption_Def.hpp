...
template <typename EvalT, typename Traits>
void
Absorption<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  if (is_constant) {
...
Absorption<EvalT, Traits>::getValue(std::string const& n)
{
  ...
  ALBANY_ABORT(std::endl << "Error! Logic error in getting paramter " << n << " in Absorption::getValue()" << std::endl);
