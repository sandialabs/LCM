...
//*****************************************************************//
//
// This class implements spectralelements for Aeras, by reading in a STK mesh
// from an Exodus file containing a bilinear quad/hex mesh and enriching it with
  ...
  *out << "Explicit scheme in Aeras? " << explicit_scheme << std::endl;
#endif
  // IKT, 9/30/15: error check that the user is not trying to prescribe periodic
  // BCs for a problem other than a 1D one. Periodic BCs are only supported for
  // 1D (xz-hydrostatic) problems.
  int numPeriodicBCs = 0;
  for (int dim = 0; dim < stkMeshStruct->numDim; dim++)
    if (stkMeshStruct->PBCStruct.periodic[dim]) numPeriodicBCs++;
  if ((stkMeshStruct->numDim > 1) && (numPeriodicBCs > 0))
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        std::logic_error,
        "Aeras::SpectralDiscretization constructor: periodic BCs are only "
        "supported for 1D spectral elements!  "
            << "You are attempting to specify periodic BCs for a "
            << stkMeshStruct->numDim << "D problem." << std::endl);
  ...
  if (element_name == "Line") {
    ...
    ElemType          = LINE;
      ...
      element_name == "Quadrilateral" || element_name == "ShellQuadrilateral") {
    spatial_dim       = 2;
  ...
  // to recize latticeOrientation array b/c it is indexed in
...
Aeras::SpectralDiscretization::~SpectralDiscretization()
{
  ...
  if (stkMeshStruct->cdfOutput)
    if (netCDFp) {
      ...
      ALBANY_ASSERT(
          !ierr,
          "close returned error code " << ierr << " - " << nc_strerror(ierr)
                                       << std::endl);
    }
#endif
...
Teuchos::RCP<const Thyra_VectorSpace>
Aeras::SpectralDiscretization::getNodeVectorSpace(
...
Teuchos::RCP<const Thyra_VectorSpace>
Aeras::SpectralDiscretization::getOverlapNodeVectorSpace(
    const std::string& /* field_name */) const
{
  TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error, "not implemented");
  return Teuchos::null;
...
Teuchos::RCP<const Thyra_VectorSpace>
Aeras::SpectralDiscretization::getVectorSpace(
    const std::string& /* field_name */) const
{
  TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error, "not implemented");
  return Teuchos::null;
...
Teuchos::RCP<const Thyra_VectorSpace>
Aeras::SpectralDiscretization::getOverlapVectorSpace(
    const std::string& /* field_name */) const
{
  TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error, "not implemented");
  return Teuchos::null;
...
Aeras::SpectralDiscretization::getContactManager() const
{
...
void
Aeras::SpectralDiscretization::setCoordinates(
...
void
Aeras::SpectralDiscretization::setReferenceConfigurationManager(
  ...
  if (transformType == "None") {
    ...
    *out << "Spherical" << endl;
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        std::logic_error,
        "Error: transformMesh() is not implemented yet "
...
// preconditioners.
void
Aeras::SpectralDiscretization::setupMLCoords()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "Warning: setupMLCoords() not implemented in Aeras::"
       << "SpectralDiscretization!  "
       << "ML and MueLu will not receive "
       << "coordinates for repartitioning if used." << std::endl;
#endif
}

void
Aeras::SpectralDiscretization::writeCoordsToMatrixMarket() const
{
#ifdef OUTPUT_TO_SCREEN
  *out
      << "Warning: writeCoordsToMatrixMarketCoords() not implemented in Aeras::"
    ...
    const bool          overlapped)
{
  ...
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
  ...
  writeSolutionToMeshDatabase(
      solution, solution_dot, solution_dotdot, time, overlapped);
  // IKT, FIXME? extend writeSolutionToFile to take in solution_dot and
  // solution_dotdot?
  writeSolutionToFile(solution, time, overlapped);
}

void
Aeras::SpectralDiscretization::writeSolutionMV(
...
void
Aeras::SpectralDiscretization::writeSolutionToFile(
    const Thyra_Vector& /* solution */,
    const double time,
    const bool /* overlapped */)
{
...
void
Aeras::SpectralDiscretization::writeSolutionMVToFile(
    const Thyra_MultiVector& /* solution */,
    const double time,
    const bool /* overlapped */)
{
...
void
Aeras::SpectralDiscretization::setResidualField(
    const Thyra_Vector& /* residual */)
{
...
Aeras::SpectralDiscretization::getSolutionMV(bool overlapped) const
{
  ...
  Teuchos::RCP<Thyra_MultiVector> solnMV =
...
void
Aeras::SpectralDiscretization::getSolutionField(
    Thyra_Vector& result,
...
void
Aeras::SpectralDiscretization::getField(
    Thyra_Vector& /* result */,
...
void
Aeras::SpectralDiscretization::getSolutionMV(
...
void
Aeras::SpectralDiscretization::setSolutionField(
    const Thyra_Vector& solution,
    const bool /* overlapped */)
{
#ifdef OUTPUT_TO_SCREEN
    ...
    const bool /* overlapped */)
{
  ...
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
  ...
  // Copy soln, soln_dot and soln_dotdot vector into solution field, one node at
  // a time Note that soln, soln_dot and soln_dotdot coming in is the local (non
  // overlapped) soln, soln_dot and soln_dotdot
  ...
  container->saveSolnVector(
      solution, solution_dot, solution_dotdot, locally_owned, m_node_vs);
}

void
Aeras::SpectralDiscretization::setField(
    ...
    bool /* overlapped */)
{
  TEUCHOS_TEST_FOR_EXCEPTION(
      true,
      std::logic_error,
      "Aeras::SpectralDiscretization::setField() not implemented!");
}

void
Aeras::SpectralDiscretization::setSolutionFieldMV(
    const Thyra_MultiVector& solution,
    const bool /* overlapped */)
{
#ifdef OUTPUT_TO_SCREEN
...
inline GO
Aeras::SpectralDiscretization::gid(const stk::mesh::Entity node) const
{
  return bulkData.identifier(node) - 1;
}

int
Aeras::SpectralDiscretization::getOwnedDOF(const int inode, const int eq) const
{
...
int
Aeras::SpectralDiscretization::getOverlapDOF(const int inode, const int eq)
    const
{
  if (interleavedOrdering)
    return inode * neq + eq;
...
GO
Aeras::SpectralDiscretization::getGlobalDOF(const GO inode, const int eq) const
{
  if (interleavedOrdering)
    return inode * neq + eq;
  else
...
int
Aeras::SpectralDiscretization::nonzeroesPerRow(const int /* neq */) const
{
...
stk::mesh::EntityId
Aeras::SpectralDiscretization::getMaximumID(
    const stk::mesh::EntityRank rank) const
{
  // Get the local maximum ID
  bulkData.begin_entities(rank);
  stk::mesh::EntityId last_entity = (--bulkData.end_entities(rank))->first.id();

  // Use a parallel MAX reduction to obtain the global maximum ID
  stk::mesh::EntityId result;
  Teuchos::reduceAll(
      *commT, Teuchos::REDUCE_MAX, 1, (GO*)(&last_entity), (GO*)(&result));
  return result;
}

void
Aeras::SpectralDiscretization::enrichMeshLines()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  for (size_t ibuck = 0; ibuck < elementBuckets.size(); ++ibuck) {
    ...
    for (size_t ielem = 0; ielem < elementBucket.size(); ++ielem) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(
          ...
          "  Element "
              << gid(element) << " has " << numNodes << " nodes.");
...
void
Aeras::SpectralDiscretization::enrichMeshQuads()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
...
void
Aeras::SpectralDiscretization::computeOwnedNodesAndUnknownsLines()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  stk::mesh::get_selected_entities(
      select_owned, bulkData.buckets(stk::topology::ELEMENT_RANK), cells);
  for (int rank = 0; rank < commT->getSize(); ++rank) {
    if (rank == commT->getRank()) {
  ...
  // with the end nodes.
  stk::mesh::get_selected_entities(
      select_owned, bulkData.buckets(stk::topology::NODE_RANK), ownednodes);
  ...
  numGlobalNodes =
      Albany::createGlobalLocalIndexer(m_node_vs)->getNumGlobalElements() + 1;
...
void
Aeras::SpectralDiscretization::computeOwnedNodesAndUnknownsQuads()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
...
void
Aeras::SpectralDiscretization::computeOverlapNodesAndUnknownsLines()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  stk::mesh::get_selected_entities(
      select_unowned, bulkData.buckets(stk::topology::NODE_RANK), unownedNodes);
  ...
  // Teuchos::ArrayView<const GO> ownedIndicesT =
  // Albany::getNodeElementList(m_node_vs);
  ...
  for (int rank = 0; rank < commT->getSize(); ++rank) {
    ...
    if (rank == commT->getRank())
      std::cout << "P" << rank
                ...
                << ", numOwnedNodes = " << numOwnedNodes
                << ", indicesT = " << overlapIndicesT << std::endl;
  }
#endif
  ...
  m_overlap_node_vs =
      Teuchos::null;  // delete existing map happens here on remesh
...
void
Aeras::SpectralDiscretization::computeOverlapNodesAndUnknownsQuads()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  for (int rank = 0; rank < commT->getSize(); ++rank) {
    ...
    if (rank == commT->getRank()) {
      std::cout << std::endl
                << "Rank " << rank << ": unowned shared edges = { ";
  ...
  m_overlap_node_vs =
      Teuchos::null;  // delete existing vector space happens here on remesh
  m_overlap_node_vs = Albany::createVectorSpace(commT, overlapIndicesT());
...
void
Aeras::SpectralDiscretization::computeCoordsLines()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  Teuchos::RCP<Intrepid2::Cubature<PHX::Device>> gl1D = Teuchos::rcp(
  ...
  for (size_t iws = 0; iws < numWorksets; ++iws) {
    ...
    for (size_t ielem = 0; ielem < numElements; ++ielem) {
      ...
      for (size_t ii = 0; ii < 2; ++ii) {
        c[ii] = stk::mesh::field_data(*coordinates_field, stkNodes[ii])[0];
      }
      // The following is for periodic BCs.  This will only be relevant for the
      // x-z hydrostatic equations.
      if (stkMeshStruct->PBCStruct.periodic[0]) {
        bool anyXeqZero = false;
        for (int j = 0; j < 2; j++) {
          if (c[j] == 0.0) anyXeqZero = true;
        }
        if (anyXeqZero) {
          ...
          if (flipZeroToScale) {
            for (int j = 0; j < 2; j++) {
              if (c[j] == 0.0) { c[j] = stkMeshStruct->PBCStruct.scale[0]; }
            }
          }
        }
      }
      for (int inode = 0; inode < np; ++inode) {
        ...
        coords[iws][ielem][inode][0] =
            (-c[0] * (x - 1.0) + c[1] * (x + 1.0)) * 0.5;
...
void
Aeras::SpectralDiscretization::computeCoordsQuads()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  // Compute the 1D Gauss-Lobatto quadrature
  Intrepid2::CubaturePolylib<PHX::Device, RealType, RealType> gl1D(
      2 * deg - 1, Intrepid2::POLYTYPE_GAUSS_LOBATTO);
  ...
  for (size_t iws = 0; iws < numWorksets; ++iws) {
    ...
    for (size_t ielem = 0; ielem < numElements; ++ielem) {
      ...
      for (size_t idim = 0; idim < 3; ++idim) {
        ...
        for (int inode = 0; inode < np2; ++inode) {
          ...
          coords[iws][ielem][inode][idim] =
              (c[0] * (x - 1.0) * (y - 1.0) - c[1] * (x + 1.0) * (y - 1.0) +
               c[2] * (x + 1.0) * (y + 1.0) - c[3] * (x - 1.0) * (y + 1.0)) *
              0.25;
        }
      }
      ...
      for (int inode = 0; inode < np2; ++inode) {
        ...
        for (size_t idim = 0; idim < 3; ++idim)
          distance +=
              coords[iws][ielem][inode][idim] * coords[iws][ielem][inode][idim];
...
void
Aeras::SpectralDiscretization::computeGraphs_Explicit()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
...
void
Aeras::SpectralDiscretization::computeGraphsExplicitUpToFillComplete()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  *out << "nodes_per_element: " << nodes_per_element << std::endl;
  ...
  m_overlap_jac_factory =
      Teuchos::null;  // delete existing graph here on remesh
  // Graph for diagonal matrix
  m_overlap_jac_factory = Teuchos::rcp(
      new Albany::ThyraCrsMatrixFactory(m_overlap_vs, m_overlap_vs, 1));
...
void
Aeras::SpectralDiscretization::computeGraphsUpToFillComplete()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  *out << "nodes_per_element: " << nodes_per_element << std::endl;
  ...
  m_implicit_overlap_jac_factory =
  ...
  if (explicit_scheme == false) {
    m_overlap_jac_factory = Teuchos::rcp(new Albany::ThyraCrsMatrixFactory(
        m_overlap_vs, m_overlap_vs, neq * nodes_per_element));
  }
#ifdef OUTPUT_TO_SCREEN
        ...
        for (std::size_t k = 0; k < neq; k++) {
          ...
          for (int l = 0; l < nodes_per_element; l++) {
            ...
            for (std::size_t m = 0; m < neq; m++) {
              ...
              m_implicit_overlap_jac_factory->insertGlobalIndices(
                  row, Teuchos::arrayView(&col, 1));
              m_implicit_overlap_jac_factory->insertGlobalIndices(
                  row, Teuchos::arrayView(&col, 1));
              // IKT, FIXME?  The following line might be needed
              // m_implicit_overlap_jac_factory->insertGlobalIndices(col,
            ...
            for (int l = 0; l < nodes_per_element; l++) {
              ...
              for (std::size_t m = 0; m < neq; m++) {
                ...
                m_overlap_jac_factory->insertGlobalIndices(
                    row, Teuchos::arrayView(&col, 1));
                // IKT, FIXME?  The following line might be needed
                // m_overlap_jac_factory->insertGlobalIndices(col,
                // Teuchos::arrayView(&row,1));
              }
            }
          }
...
void
Aeras::SpectralDiscretization::fillCompleteGraphs()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  // For implicit scheme, m_overlap_jac_factory = m_implicit_overlap_jac_factory
  if (explicit_scheme == false) { m_overlap_jac_factory->fillComplete(); }

  // create m_implicit_jac_factory (owned)
  m_implicit_jac_factory = Teuchos::rcp(new Albany::ThyraCrsMatrixFactory(
      m_vs, m_vs, m_implicit_overlap_jac_factory));
  // For implicit scheme, m_jac_factory = m_implicit_jac_factory
  if (explicit_scheme == false) {
    m_jac_factory = Teuchos::rcp(
        new Albany::ThyraCrsMatrixFactory(m_vs, m_vs, m_overlap_jac_factory));
  }
}

void
Aeras::SpectralDiscretization::fillCompleteGraphsExplicit()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
...
void
Aeras::SpectralDiscretization::computeWorksetInfo()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  typedef Albany::AbstractSTKFieldContainer::SphereVolumeFieldType
  ...
  for (int b = 0; b < numBuckets; ++b) {
    ...
    wsElNodeEqID[b] = Albany::WorksetConn(
        "wsElNodeEqID", buck.size(), nodes_per_element, neq);
        ...
        switch (dim.size()) {
          ...
          case 3:  // vector
          {
            ...
            for (int i = 0; i < dim0; i++) {
              ...
              for (int j = 0; j < dim1; j++) {
                ...
                for (int k = 0; k < dim2; k++) array(i, j, k) = entry[k];
              }
            }
            break;
          }
          case 4:  // tensor
          {
            ...
            array.assign<ElemTag, NodeTag, CompTag, CompTag>(
                stateVec.data(), dim0, dim1, dim2, dim3);
            for (int i = 0; i < dim0; i++) {
              stk::mesh::Entity        element = buck[i];
              stk::mesh::Entity const* rel     = bulkData.begin_nodes(element);
              for (int j = 0; j < dim1; j++) {
  ...
  // The following is for periodic BCs.  This will only be relevant for the x-z
  ...
  ScalarValueState scalarValue_states =
      stkMeshStruct->getFieldContainer()->getScalarValueStates();
  QPScalarState qpscalar_states =
      stkMeshStruct->getFieldContainer()->getQPScalarStates();
  QPVectorState qpvector_states =
      stkMeshStruct->getFieldContainer()->getQPVectorStates();
  QPTensorState qptensor_states =
      stkMeshStruct->getFieldContainer()->getQPTensorStates();
  std::map<std::string, double>& time =
      stkMeshStruct->getFieldContainer()->getTime();

  for (std::size_t b = 0; b < buckets.size(); ++b) {
    stk::mesh::Bucket& buck = *buckets[b];
    for (QPScalarState::iterator qpss = qpscalar_states.begin();
         qpss != qpscalar_states.end();
         ++qpss) {
      Albany::BucketArray<Albany::AbstractSTKFieldContainer::QPScalarFieldType>
          array(**qpss, buck);
      // Debug
    ...
    for (QPTensorState::iterator qpts = qptensor_states.begin();
         qpts != qptensor_states.end();
         ++qpts) {
      Albany::BucketArray<Albany::AbstractSTKFieldContainer::QPTensorFieldType>
          array(**qpts, buck);
      // Debug
...
void
Aeras::SpectralDiscretization::computeSideSetsLines()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  while (ss != stkMeshStruct->ssPartVec.end()) {
    ...
    *out << "STKDisc: sideset " << ss->first << " has size " << sides.size()
    ...
    for (std::size_t localSideID = 0; localSideID < sides.size();
      ...
      // twice in the element list, once for each element that contains it.
...
void
Aeras::SpectralDiscretization::computeNodeSetsLines()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
...
void
Aeras::SpectralDiscretization::createOutputMesh()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
  ...
  // IKT, 9/22/15: this needs to be called all the time even when no exodus
  // output is requested b/c outputStkMeshStruct is called in
  // Aeras::SpectralDiscretization::setOvlpSolutionFieldT, which is always
  // called.
  outputStkMeshStruct = Teuchos::rcp(new Aeras::SpectralOutputSTKMeshStruct(
      discParams,
      commT,
...
void
Aeras::SpectralDiscretization::setupExodusOutput()
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
#endif
...
Basis(const int C)
{
  ...
  // Spectral bases
  std::cout << "AGS -- changing POINTTYPE_SPECTRAL to POINTTYPE_WARPBLEND -- "
               "check with Kyungjoo"
            << std::endl;
...
setup_latlon_interp(
    ...
    Albany::WorksetArray<Teuchos::ArrayRCP<
        std::vector<Aeras::SpectralDiscretization::interp>>>::type& interpdata,
...
int
Aeras::SpectralDiscretization::processNetCDFOutputRequestT(
...
void
Aeras::SpectralDiscretization::setupNetCDFOutput()
{
  const long long unsigned rank = commT->getRank();
#ifdef ALBANY_SEACAS
  if (stkMeshStruct->cdfOutput) {
...
void
Aeras::SpectralDiscretization::reNameExodusOutput(std::string& filename)
{
#ifdef ALBANY_SEACAS
...
Aeras::SpectralDiscretization::updateMesh()
{
  ...
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << std::endl;
  ...
  // and call appropriate computeGraphs routine depending on what kind of
  // scheme. Right now, computeGraphs_Explicit() will not work with shallow
  // water; therefore only call this function for hydrostatic (numLevels > 0)

  // computeGraphs populates m_implicit_graph_factory and
  // m_implicit_overlap_graph_factory for an explicit scheme; for an implicit
  // scheme, it also populates m_graph_factory and m_overlap_graph_factory. For
  // an explicit scheme, these the implicit graph factories are needed to
  // populate correctly the Laplace operator, needed for hyperviscosity
  computeGraphs();
  // computeGraphs_Explicit will populate m_graph_factory and
  // m_overlap_graph_factory for an explicit scheme, which will have graphs of
  // diagonal matrices.
  computeGraphs_Explicit();

#ifdef WRITE_TO_MATRIX_MARKET_TO_MM_FILE
  Albany::writeMatrixMarket(
      m_implicit_jac_factory->createOp(), "ImplicitOp.mm");
  ...
  // IKT, 9/22/15: the following routine needs to be implemented, if we care
  // about netCDFoutput. setupNetCDFOutput();
}
