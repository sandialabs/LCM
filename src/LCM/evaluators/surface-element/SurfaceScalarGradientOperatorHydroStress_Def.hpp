      ...
      grad_val_qp(p.get<std::string>("Surface Scalar Gradient Name"), dl->qp_gradient)
{
  ...
  this->setName("Surface Scalar Gradient Operator HydroStress" + PHX::print<EvalT>());
...
template <typename EvalT, typename Traits>
void
SurfaceScalarGradientOperatorHydroStress<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
