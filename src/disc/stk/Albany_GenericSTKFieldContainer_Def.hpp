...
GenericSTKFieldContainer<Interleaved>::addStateStructs(const Teuchos::RCP<Albany::StateInfoStruct>& sis)
{
  ...
  for (std::size_t i = 0; i < sis->size(); i++) {
    ...
    if (st.entity == StateStruct::ElemData) {
      elem_sis.push_back((*sis)[i]);
      if (dim.size() == 1) {
        // Scalar on cell
        cell_scalar_states.push_back(&metaData->declare_field<double, AbstractSTKFieldContainer::ElemStateLayout>(stk::topology::ELEMENT_RANK, st.name));
        ...
        // Vector on cell
        cell_vector_states.push_back(&metaData->declare_field<double, AbstractSTKFieldContainer::ElemStateLayout>(stk::topology::ELEMENT_RANK, st.name));
        ...
        cell_tensor_states.push_back(&metaData->declare_field<double, AbstractSTKFieldContainer::ElemStateLayout>(stk::topology::ELEMENT_RANK, st.name));
      ...
      if (dim.size() == 2) {  // Scalar at QPs
        qpscalar_states.push_back(&metaData->declare_field<double, AbstractSTKFieldContainer::ElemStateLayout>(stk::topology::ELEMENT_RANK, st.name));
        ...
        qpvector_states.push_back(&metaData->declare_field<double, AbstractSTKFieldContainer::ElemStateLayout>(stk::topology::ELEMENT_RANK, st.name));
        ...
        qptensor_states.push_back(&metaData->declare_field<double, AbstractSTKFieldContainer::ElemStateLayout>(stk::topology::ELEMENT_RANK, st.name));
        // Multi-dim order is Fortran Ordering, so reversed here
        if (dim[1] == 4) {
