    ...
    const Teuchos::RCP<Teuchos::ParameterList>& responseList)
{
  ...
  if (haveNeutEq) {  // Neutron diffusion
    ...
    p->set<string>("Absorption Cross Section Name", "Absorption Cross Section");
    p->set<string>("Scattering Cross Section Name", "Scattering Cross Section");
  ...
  if (haveNeutEq) {  // Neutron scattering cross section
    ...
    p->set<string>("Material Property Name", "Scattering Cross Section");
    p->set<RCP<DataLayout>>("Data Layout", dl->qp_scalar);
    p->set<string>("Coordinate Vector Name", "Coord Vec");
    p->set<RCP<DataLayout>>("Coordinate Vector Data Layout", dl->qp_vector);
    p->set<string>("Temperature Variable Name", "Temperature");
  ...
  if (haveNeutEq) {  // Neutron released per fission
    ...
    p->set<RCP<ParamLib>>("Parameter Library", paramLib);
    Teuchos::ParameterList& paramList = params->sublist("Neutrons per Fission");
    p->set<Teuchos::ParameterList*>("Parameter List", &paramList);
  ...
  if (porousMedia) {  // Porosity, \phi
    ...
    p->set<string>("Material Property Name", "Porosity");
    p->set<RCP<DataLayout>>("Data Layout", dl->qp_scalar);
    p->set<string>("Coordinate Vector Name", "Coord Vec");
    p->set<RCP<DataLayout>>("Coordinate Vector Data Layout", dl->qp_vector);
  ...
  if (porousMedia) {  // Permeability Tensor, K
    ...
    p->set<string>("Material Property Name", "Permeability");
    p->set<RCP<DataLayout>>("Data Layout", dl->qp_scalar);
    p->set<string>("Coordinate Vector Name", "Coord Vec");
    p->set<RCP<DataLayout>>("Coordinate Vector Data Layout", dl->qp_vector);

    p->set<RCP<ParamLib>>("Parameter Library", paramLib);
    Teuchos::ParameterList& paramList = params->sublist("Permeability");
    p->set<Teuchos::ParameterList*>("Parameter List", &paramList);
  ...
  if (porousMedia) {  // Forchheimer Tensor, F
    ...
    p->set<string>("Material Property Name", "Forchheimer");
    p->set<RCP<DataLayout>>("Data Layout", dl->qp_scalar);
    p->set<string>("Coordinate Vector Name", "Coord Vec");
    p->set<RCP<DataLayout>>("Coordinate Vector Data Layout", dl->qp_vector);

    p->set<RCP<ParamLib>>("Parameter Library", paramLib);
    Teuchos::ParameterList& paramList = params->sublist("Forchheimer");
    p->set<Teuchos::ParameterList*>("Parameter List", &paramList);
  ...
  /*  if (haveFlowEq && haveNeumannx) { // Neumann BC's listed in input file and
  sidesets are present
      ...
      RCP<shards::CellTopology> sideType = rcp(new
  shards::CellTopology(side_top)); RCP <Intrepid2::Cubature<PHX::Device> >
  sideCubature = cubFactory.create<PHX::Device, RealType, RealType>(*sideType,
  meshSpecs.cubatureDegree);
      ...
      p->set< RCP<Intrepid2::Cubature<PHX::Device> > >("Side Cubature",
  sideCubature);
  ...
  //    p->set< RCP<DataLayout> >("QP Scalar Data Layout", dl->qp_scalar);
   ...
   if (haveFlowEq && haveNeumanny) { // Neumann BC's listed in input file and
  sidesets are present
  ...
  //    p->set< RCP<DataLayout> >("QP Scalar Data Layout", dl->qp_scalar);
    ...
    if (haveFlowEq && haveNeumannz) { // Neumann BC's listed in input file and
  sidesets are present
  ...
  */
  ...
  if (haveFlowEq) {  // Rm
    ...
    p->set<string>("Permeability Term", "Permeability Term");
    p->set<string>("Forchheimer Term", "Forchheimer Term");
    if (number_of_time_deriv == 0) p->set<bool>("Disable Transient", true);
