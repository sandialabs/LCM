    ...
    FieldMap                  eval_fields)
{
  ...
  for (int cell = 0; cell < workset.numCells; ++cell) {
    for (int pt = 0; pt < num_pts_; ++pt) {
      ...
      sigma_m = (1.0 / J(cell, pt)) * minitensor::dot(F, minitensor::dot(S0_m, minitensor::transpose(F)));
      ...
      damage_deriv_m = max_damage_m_ / saturation_m_ * std::exp(-alpha_m / saturation_m_);

      // optional material tangent computation
      if (compute_tangent_) {
        tangent_m = (1.0 - damage_m(cell, pt)) * tangent_m - damage_deriv_m * minitensor::tensor(S0_m, S0_m);
              ...
              for (int q(0); q < num_dims_; ++q) {
                for (int k(0); k < num_dims_; ++k) {
                  for (int n(0); n < num_dims_; ++n) {
                    tangentA_m(i, j, p, q) = tangentA_m(i, j, p, q) + F(i, k) * tangent_m(k, j, n, q) * F(p, n);
                  }
                }

                tangentA_m(i, j, p, q) = tangentA_m(i, j, p, q) + (1.0 - damage_m(cell, pt)) * S0_m(q, j) * I(i, p);
              }
            }
          }
        }
      ...
      // derivative of damage w.r.t alpha
      damage_deriv_f1 = max_damage_f1_ / saturation_f1_ * std::exp(-alpha_f1 / saturation_f1_);

      damage_deriv_f2 = max_damage_f2_ / saturation_f2_ * std::exp(-alpha_f2 / saturation_f2_);

      // tangent for fibers including damage
      // optional material tangent computation
      if (compute_tangent_) {
        tangent_f1 = (1.0 - damage_f1(cell, pt)) * tangent_f1 - damage_deriv_f1 * minitensor::tensor(S0_f1, S0_f1);
        tangent_f2 = (1.0 - damage_f2(cell, pt)) * tangent_f2 - damage_deriv_f2 * minitensor::tensor(S0_f2, S0_f2);
          ...
          for (int j(0); j < num_dims_; ++j) {
            for (int p(0); p < num_dims_; ++p) {
              for (int q(0); q < num_dims_; ++q) {
                for (int k(0); k < num_dims_; ++k) {
                  for (int n(0); n < num_dims_; ++n) {
                    tangentA_f1(i, j, p, q) = tangentA_f1(i, j, p, q) + F(i, k) * tangent_f1(k, j, n, q) * F(p, n);

                    tangentA_f2(i, j, p, q) = tangentA_f2(i, j, p, q) + F(i, k) * tangent_f2(k, j, n, q) * F(p, n);
                  }
                }

                tangentA_f1(i, j, p, q) = tangentA_f1(i, j, p, q) + (1.0 - damage_f1(cell, pt)) * S0_f1(q, j) * I(i, p);

                tangentA_f2(i, j, p, q) = tangentA_f2(i, j, p, q) + (1.0 - damage_f2(cell, pt)) * S0_f2(q, j) * I(i, p);
              }
            }
          }
        }
