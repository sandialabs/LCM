...
StrongSchwarzBC_Base<EvalT, Traits>::StrongSchwarzBC_Base(Teuchos::ParameterList& p)
...
template <typename T>
void
StrongSchwarzBC_Base<EvalT, Traits>::computeBCs(size_t const ns_node, T& x_val, T& y_val, T& z_val)
{
...
StrongSchwarzBC_Base<EvalT, Traits>::computeBCsDTK()
{
  ...
  ALBANY_ASSERT(
      num_sol_vecs == this_field_array.length(), "coupled_field_array and this_field_array must have the same length!");
  ...
  Teuchos::RCP<Tpetra::MultiVector<double, int, DataTransferKit::SupportId>> this_vector =
      doDTKInterpolation(coupled_manager, this_manager, coupled_field, this_field, neq, dtk_params);

  Teuchos::Array<Teuchos::RCP<Tpetra::MultiVector<double, int, DataTransferKit::SupportId>>> this_vector_arrays(
      num_sol_vecs);
  ...
  if (num_sol_vecs > 1) {
    ...
    this_vector_dot =
        doDTKInterpolation(coupled_manager, this_manager, coupled_field_dot, this_field_dot, neq, dtk_params);
  ...
  if (num_sol_vecs > 2) {
    ...
    this_vector_dotdot =
        doDTKInterpolation(coupled_manager, this_manager, coupled_field_dotdot, this_field_dotdot, neq, dtk_params);
...
{
  Teuchos::RCP<Thyra_Vector const> const_disp = workset.x;
  Teuchos::RCP<Thyra_Vector const> const_velo = workset.xdot;
  Teuchos::RCP<Thyra_Vector const> const_acce = workset.xdotdot;
  ...
  Teuchos::RCP<Thyra_Vector> disp =
      has_disp == true ? Teuchos::rcpFromRef(const_cast<Thyra_Vector&>(*const_disp)) : Teuchos::null;

  auto disp_view = has_disp == true ? Albany::getNonconstLocalData(disp) : Teuchos::null;

  // Velocity
  Teuchos::RCP<Thyra_Vector> velo =
      has_velo == true ? Teuchos::rcpFromRef(const_cast<Thyra_Vector&>(*const_velo)) : Teuchos::null;

  auto velo_view = has_velo == true ? Albany::getNonconstLocalData(velo) : Teuchos::null;

  // Acceleration
  Teuchos::RCP<Thyra_Vector> acce =
      has_acce == true ? Teuchos::rcpFromRef(const_cast<Thyra_Vector&>(*const_acce)) : Teuchos::null;

  auto acce_view = has_acce == true ? Albany::getNonconstLocalData(acce) : Teuchos::null;
  ...
  Teuchos::Array<Teuchos::RCP<Tpetra::MultiVector<double, int, DataTransferKit::SupportId>>> bcs_array =
      sbc.computeBCsDTK();
  ...
  // Displacement
  Teuchos::RCP<Tpetra::MultiVector<double, int, DataTransferKit::SupportId>> bcs_disp = bcs_array[0];
  ...
  // Velocity
  Teuchos::RCP<Tpetra::MultiVector<double, int, DataTransferKit::SupportId>> bcs_velo{Teuchos::null};
  ...
  // Acceleration
  Teuchos::RCP<Tpetra::MultiVector<double, int, DataTransferKit::SupportId>> bcs_acce{Teuchos::null};
...
StrongSchwarzBC<PHAL::AlbanyTraits::Residual, Traits>::StrongSchwarzBC(Teuchos::ParameterList& p)
...
template <typename Traits>
void
StrongSchwarzBC<PHAL::AlbanyTraits::Residual, Traits>::preEvaluate(typename Traits::EvalData workset)
{
  fillSolution<StrongSchwarzBC<PHAL::AlbanyTraits::Residual, Traits>, Traits>(*this, workset);
  return;
}

template <typename Traits>
void
StrongSchwarzBC<PHAL::AlbanyTraits::Residual, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  fillResidual<StrongSchwarzBC<PHAL::AlbanyTraits::Residual, Traits>, Traits>(*this, workset);
  return;
}

// Specialization: Jacobian
template <typename Traits>
StrongSchwarzBC<PHAL::AlbanyTraits::Jacobian, Traits>::StrongSchwarzBC(Teuchos::ParameterList& p)
...
template <typename Traits>
void
StrongSchwarzBC<PHAL::AlbanyTraits::Jacobian, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  workset.is_schwarz_bc_     = true;
  workset.spatial_dimension_ = this->app_->getSpatialDimension();
  PHAL::SDirichlet<PHAL::AlbanyTraits::Jacobian, Traits>::evaluateFields(workset);

  if (workset.f != Teuchos::null) {
    fillResidual<StrongSchwarzBC<PHAL::AlbanyTraits::Jacobian, Traits>, Traits>(*this, workset);
  }
  workset.is_schwarz_bc_ = false;
  return;
}
