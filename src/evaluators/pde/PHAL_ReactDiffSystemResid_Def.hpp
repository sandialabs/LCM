          ...
          p.get<Teuchos::RCP<PHX::DataLayout>>("Node Vector Data Layout"))
{
  ...
  forces = arcpFromArray(
      bf_list->get<Teuchos::Array<double>>("Forces", Teuchos::Array<double>()));

  reactCoeff0 = arcpFromArray(bf_list->get<Teuchos::Array<double>>(
      "Reaction Coefficients0", Teuchos::Array<double>()));
  reactCoeff1 = arcpFromArray(bf_list->get<Teuchos::Array<double>>(
      "Reaction Coefficients1", Teuchos::Array<double>()));
  reactCoeff2 = arcpFromArray(bf_list->get<Teuchos::Array<double>>(
    ...
    for (std::size_t node = 0; node < numNodes; ++node) {
      ...
      for (std::size_t qp = 0; qp < numQPs; ++qp) {
        ...
        Residual(cell, node, 0) +=
            mu0 * UGrad(cell, qp, 0, 0) * wGradBF(cell, node, qp, 0) +
        ...
        Residual(cell, node, 1) +=
            mu1 * UGrad(cell, qp, 1, 0) * wGradBF(cell, node, qp, 0) +
        ...
        Residual(cell, node, 2) +=
            mu2 * UGrad(cell, qp, 2, 0) * wGradBF(cell, node, qp, 0) +
