...
AdvDiffResid<EvalT, Traits>::AdvDiffResid(Teuchos::ParameterList const& p)
    : wBF(p.get<std::string>("Weighted BF Name"), p.get<Teuchos::RCP<PHX::DataLayout>>("Node QP Scalar Data Layout")),
      ...
      UDot(p.get<std::string>("QP Time Derivative Variable Name"), p.get<Teuchos::RCP<PHX::DataLayout>>("QP Vector Data Layout")),
      Residual(p.get<std::string>("Residual Name"), p.get<Teuchos::RCP<PHX::DataLayout>>("Node Vector Data Layout"))
{
  ...
  ALBANY_PANIC(error, "Invalid Augmented Form Type: " << formType << "; valid options are 1 and 2");
  ...
  if (useAugForm == true) std::cout << "  Augmented form type: " << formType << std::endl;
}

//*****
template <typename EvalT, typename Traits>
void
AdvDiffResid<EvalT, Traits>::postRegistrationSetup(typename Traits::SetupData d, PHX::FieldManager<Traits>& fm)
{
    ...
    for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
      for (std::size_t node = 0; node < numNodes; ++node) {
        ...
        for (std::size_t qp = 0; qp < numQPs; ++qp) {
          ...
          Residual(cell, node, 0) += UDot(cell, qp, 0) * wBF(cell, node, qp) + a * UGrad(cell, qp, 0, 0) * wBF(cell, node, qp) +
                                     b * UGrad(cell, qp, 0, 1) * wBF(cell, node, qp) + mu * UGrad(cell, qp, 0, 0) * wGradBF(cell, node, qp, 0) +
    ...
    if (formType == 1) {
      for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
        for (std::size_t node = 0; node < numNodes; ++node) {
          ...
          for (std::size_t qp = 0; qp < numQPs; ++qp) {
            ...
            Residual(cell, node, 0) += UDot(cell, qp, 0) * wBF(cell, node, qp) + a * U(cell, qp, 1) * wBF(cell, node, qp) +
                                       b * U(cell, qp, 2) * wBF(cell, node, qp) + mu * U(cell, qp, 1) * wGradBF(cell, node, qp, 0) +
      ...
      for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
        for (std::size_t node = 0; node < numNodes; ++node) {
          ...
          for (std::size_t qp = 0; qp < numQPs; ++qp) {
            // du/dt + q = 0
            Residual(cell, node, 0) += UDot(cell, qp, 0) * wBF(cell, node, qp) + U(cell, qp, 1) * wBF(cell, node, qp) + U(cell, qp, 2) * wBF(cell, node, qp);
