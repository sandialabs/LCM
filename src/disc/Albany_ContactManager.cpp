...

{
  ...
  masterSideNames   = paramList.get<Teuchos::Array<std::string>>("Master Side Sets");
  slaveSideNames    = paramList.get<Teuchos::Array<std::string>>("Slave Side Sets");
  sideSetIDs        = paramList.get<Teuchos::Array<std::string>>("Contact Side Set Pair");
  constrainedFields = paramList.get<Teuchos::Array<std::string>>("Constrained Field Names");

  // Print names of field variables to be constrainted
  std::cout << "Number of constrained fields: " << constrainedFields.size() << std::endl;
  for (std::size_t i = 0; i < constrainedFields.size(); i++) std::cout << constrainedFields[i] << std::endl;

  // Print the master side set names
  std::cout << "Master side sets found, number = " << masterSideNames.size() << std::endl;
  for (int i = 0; i < masterSideNames.size(); i++) std::cout << masterSideNames[i] << std::endl;

  // Print the slave side set names
  std::cout << "Slave side sets found, number = " << slaveSideNames.size() << std::endl;
  for (int i = 0; i < slaveSideNames.size(); i++) std::cout << slaveSideNames[i] << std::endl;
  ...
  for (size_t i = 0; i < num_contact_pairs; i += 2) std::cout << sideSetIDs[i] << "/" << sideSetIDs[i + 1] << std::endl;
  ...
  for (int pair = 0; pair < number_of_mortar_pairs; pair++) {
    processSS(interface_ctr, slaveSideNames[pair], 0 /* Slave side */, mortarside, slaveNodeGIDs, sfile);
  ...
  for (int pair = 0; pair < number_of_mortar_pairs; pair++) {
    processSS(interface_ctr, masterSideNames[pair], 1 /* mortar side */, nonmortarside, masterNodeGIDs, mfile);
  ...
  for (int workset = 0; workset < numWorksets; workset++) {
    ...
    if (it_side_set != ssList.end()) {
      ...
      for (std::size_t side = 0; side < theSideSet.size(); ++side) {
        ...
        bool const on_boundary = false;                    // will eventually want to allow boundaries to be
                                                           // intersected by contact surfaces
        int const                    contact_pair_id = 0;  // will eventually want to allow multiple pairs
...
void
Albany::ContactManager::fillInMortarResidual(int const ws, Teuchos::ArrayRCP<ST>& resid)
{
