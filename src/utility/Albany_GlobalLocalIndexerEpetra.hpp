...
class GlobalLocalIndexerEpetra : public GlobalLocalIndexer
{
 public:
  ...
  {
    ALBANY_EXPECT(
        gid <= static_cast<GO>(Teuchos::OrdinalTraits<Epetra_GO>::max()),
  ...
  {
    return m_emap->NumMyElements();
  }
  GO
  getNumGlobalElements() const
  {
    return m_emap->NumGlobalElements();
  }
