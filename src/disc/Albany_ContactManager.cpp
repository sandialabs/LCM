...
Albany::ContactManager::ContactManager(
    ...
    :
      ...
      params(params_),
      disc(disc_),
      coordArray(disc_.getCoordinates()),
...

{
  ...
  moertelManager =
      Teuchos::rcp(new MoertelT::ManagerT<ST, LO, Tpetra_GO, KokkosNode>(
          disc.getMapT()->getComm(), printLevel));

  if (probDim == 2) {
    oneD = true;
    moertelManager->SetDimension(
        MoertelT::ManagerT<ST, LO, Tpetra_GO, KokkosNode>::manager_2D);
  } else {
    oneD = false;
    moertelManager->SetDimension(
        MoertelT::ManagerT<ST, LO, Tpetra_GO, KokkosNode>::manager_3D);
  }
  ...
  std::cout << "Number of contact pairs as master/slave:" << num_contact_pairs
            << std::endl;
  for (size_t i = 0; i < num_contact_pairs; i += 2)
    std::cout << sideSetIDs[i] << "/" << sideSetIDs[i + 1] << std::endl;

  if (disc.getMapT()->getComm()->getSize() !=
      2) {  // Need exactly two ranks to write parallel rank output

    ALBANY_ASSERT(
        disc.getMapT()->getComm()->getSize() == 1,
        "ERROR - running in parallel but not using two ranks");
    // We are only serial here

    sfile.open("slave_interface.txt");
    sfile << "This file contains the interface from sideset: "
          << slaveSideNames[0] << std::endl;
    mfile.open("master_interface.txt");
    mfile << "This file contains the interface from sideset: "
          << masterSideNames[0] << std::endl;
  ...
  // 1D interface possible values are 1,2,3,4,5,6,7,8,10 (2 recommended with
  ...
  moertelparams.set(
      "number gaussian points 2D", 12);  // 12 recommended with linear functions
    ...
    std::ofstream&       stream)
{
  ...
  Teuchos::RCP<MoertelT::InterfaceT<ST, LO, Tpetra_GO, KokkosNode>>
      moertelInterface =
          Teuchos::rcp(new MoertelT::InterfaceT<ST, LO, Tpetra_GO, KokkosNode>(
              ctr, oneD, disc.getMapT()->getComm(), printLevel));

  const MOERTEL::Function::FunctionType primal =
      MOERTEL::Function::func_Linear1D;
  const MOERTEL::Function::FunctionType dual =
      MOERTEL::Function::func_Linear1D /*func_Constant1D*/;

  moertelInterface->SetMortarSide(mortarside);
  moertelInterface->SetFunctionTypes(primal, dual);

  //  std::size_t numFields = disc.getWsElNodeEqID()[0][0][0].size(); // num
  //  equations at each node
  std::size_t numFields =
      disc.getWsElNodeEqID()->extent(3);  // num equations at each node
  ...
  for (int workset = 0; workset < numWorksets; workset++) {
    ...
    // If ss exists in this workset, loop over the sides in it and construct
    // moertel nodes/faces and interface
    if (it_side_set != ssList.end()) {
      ...
      for (std::size_t side = 0; side < theSideSet.size(); ++side) {
        ...
        const CellTopologyData_Subcell& subcell_side =
            meshSpecs[elem_block]->ctd.side[elem_side];
        ...
        stream << "    side, local ID inside element = " << elem_side
               << std::endl;
        stream << "    side, global ID               = " << side_GID
               << std::endl;
        ...
        for (int i = 0; i < numSideNodes; ++i) {
          ...
          stream << "         node_LID = " << lnodeId
                 << "   node_GID = " << gnodeId << "    node = " << node
                 << std::endl;
          stream << "         coords = " << coords[0] << ", " << coords[1]
                 << ", " << coords[2] << " - " << std::endl
                 << std::endl;
          ...
          if (ret.second ==
              true) {  // this is a as yet unregistered node. add it
            ...
            std::cout << "Adding node: " << gnodeId << "  to interface: " << ctr
                      << std::endl;
        ...
        //  	  MOERTEL::Segment_BiLinearQuad segment( side_GID, nnodes,
        //  nodeid, printLevel ); // 3D
        moertelInterface->AddSegment(segment, s_or_mortar);
      }
    }
  }

  // once we have looped over all the worksets for this side set name, we close
  // the interface
  ALBANY_ASSERT(
      moertelInterface->Complete() == true,
      "Contact interface is not complete");
