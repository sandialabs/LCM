...
ScatterScalarResponseBase<EvalT, Traits>::setup(Teuchos::ParameterList const& p, const Teuchos::RCP<Albany::Layouts>& dl)
{
  ...
  auto global_response_tag = p.get<PHX::Tag<ScalarT>>("Global Response Field Tag");
...
{
  Teuchos::RCP<Teuchos::ParameterList> validPL = rcp(new Teuchos::ParameterList("Valid ScatterScalarResponse Params"));
  return validPL;
}
...
template <typename Traits>
void
ScatterScalarResponse<PHAL::AlbanyTraits::Residual, Traits>::postEvaluate(typename Traits::PostEvalData workset)
{
