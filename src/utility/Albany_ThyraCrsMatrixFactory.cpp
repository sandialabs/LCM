      ...
      m_filled(false)
{
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      bt == BuildType::None,
      std::logic_error,
      "Error! No build type set for albany.\n");
...
#else
    TEUCHOS_TEST_FOR_EXCEPTION(
        true, std::logic_error, "Error! Epetra is not enabled in albany.\n");
#endif
  } else {
    ...
    : m_domain_vs(domain_vs), m_range_vs(range_vs)
{
  ...
  if (bt == BuildType::Epetra) {
    ...
    m_graph->e_graph = Teuchos::rcp(new Epetra_CrsGraph(
        Copy, *e_range, e_overlap_graph->GlobalMaxNumIndices()));
...
void
ThyraCrsMatrixFactory::insertGlobalIndices(
    const GO                            row,
    const Teuchos::ArrayView<const GO>& indices)
{
  ...
  if (bt == BuildType::Epetra) {
    ...
    ALBANY_EXPECT(
        row <= max_safe_gid, "Error! Input gids exceed Epetra_GO ranges.\n");
    (void)max_safe_gid;
...
ThyraCrsMatrixFactory::fillComplete()
{
  ...
  if (bt == BuildType::Epetra) {
    ...
    m_graph->e_graph = Teuchos::rcp(new Epetra_CrsGraph(
        Copy, *e_range, nonzeros_per_row_array.getRawPtr(), true));
    ...
    for (int lrow = 0; lrow < nonzeros_per_row_array.size(); ++lrow) {
      ...
      if (row_indices.size() > 0) {
        ...
        int                       i = 0;
        for (const auto& index : row_indices) e_indices[i++] = index;
        auto row = e_range->GID(lrow);
        m_graph->e_graph->InsertGlobalIndices(
            row, e_indices.size(), e_indices.getRawPtr());
      }
    }
...
ThyraCrsMatrixFactory::createOp() const
{
  ...
  if (bt == BuildType::Epetra) {
    ...
    Teuchos::RCP<Epetra_CrsMatrix> matrix =
        Teuchos::rcp(new Epetra_CrsMatrix(Copy, *m_graph->e_graph));
    matrix->PutScalar(0.0);
    Teuchos::RCP<Epetra_Operator> mat =
        Teuchos::rcp_implicit_cast<Epetra_Operator>(matrix);
