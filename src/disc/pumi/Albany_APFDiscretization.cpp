...
APFDiscretization::APFDiscretization(
    Teuchos::RCP<APFMeshStruct>             meshStruct_,
...
APFDiscretization::~APFDiscretization() { delete meshOutput; }

void
APFDiscretization::init()
{
  ...
  Teuchos::Array<Teuchos::Array<std::string>> layout =
      meshStruct->solVectorLayout;
  ...
  if (meshStruct->hasRestartSolution) { setRestartData(); }
}

void
APFDiscretization::printCoords() const
{
...
APFDiscretization::getCoordinates() const
{
  ...
  for (size_t i = 0; i < overlapNodes.getSize(); ++i) {
    if (spdim == 3)
      apf::getComponents(
          f, overlapNodes[i].entity, overlapNodes[i].node, &coordinates[3 * i]);
    else {
      double buf[3];
...
void
APFDiscretization::setCoordinates(const Teuchos::ArrayRCP<const double>& c)
{
  const int   spdim  = getNumDim();
  double      buf[3] = {0};
  apf::Field* f      = meshStruct->getMesh()->getCoordinateField();
  for (size_t i = 0; i < overlapNodes.getSize(); ++i) {
    if (spdim == 3) {
      apf::setComponents(
          f, overlapNodes[i].entity, overlapNodes[i].node, &c[spdim * i]);
    } else {
...
void
APFDiscretization::setReferenceConfigurationManager(
    const Teuchos::RCP<AAdapt::rc::Manager>& ircm)
{
  rcm = ircm;
}
...
void
APFDiscretization::setupMLCoords()
{
  ...
  for (std::size_t i = 0; i < ownedNodes.getSize(); ++i) {
    ...
    for (int j = 0; j < mesh_dim; ++j) { coords_data[j][i] = lcoords[j]; }
  }
...
inline int
albanyCountComponents(const int problem_dim, const int pumi_value_type)
{
...
void
APFDiscretization::getField(
    const char* name,
    ST*         data,
    bool        overlapped,
    int         offset,
    bool        neq_sized) const
{
  apf::Mesh*  m = meshStruct->getMesh();
  apf::Field* f = m->findField(name);
  ALBANY_ASSERT(f, "\nExpected field " << name << " doesn't exist!\n");
...
void
APFDiscretization::getSplitFields(
    const Teuchos::Array<std::string>& names,
    const Teuchos::Array<int>&         sizes,
    ST*                                data,
...
void
APFDiscretization::reNameExodusOutput(const std::string& str)
{
  if (meshOutput) { meshOutput->setFileName(str); }
}

static void
saveOldTemperature(Teuchos::RCP<APFMeshStruct> meshStruct)
{
  if (!meshStruct->useTemperatureHack) { return; }

  apf::Mesh*  m = meshStruct->getMesh();
  apf::Field* t = m->findField("temp");
  if (!t) { t = m->findField(APFMeshStruct::solution_name[0]); }
  assert(t);
  apf::Field* told = m->findField("temp_old");
  if (!told) { told = meshStruct->createNodalField("temp_old", apf::SCALAR); }
  assert(told);
  std::cout << "copying nodal " << apf::getName(t) << " to nodal "
            << apf::getName(told) << '\n';
  apf::copyData(told, t);
}

void
APFDiscretization::writeAnySolutionToMeshDatabase(
    const ST*  soln,
    const int  index,
...
void
APFDiscretization::writeAnySolutionToFile(const double time_value)
{
  // Skip this write unless the proper interval has been reached.
  if (outputInterval++ % meshStruct->outputInterval) { return; }
...
void
APFDiscretization::writeRestartFile(const double time)
{
  TEUCHOS_FUNC_TIME_MONITOR("AlbanyAdapt: Write Restart");
  *out << "APFDiscretization::writeRestartFile: writing time " << time
       << std::endl;
  int              dim = getNumDim();
  apf::FieldShape* fs  = apf::getIPShape(dim, meshStruct->cubatureDegree);
...
void
APFDiscretization::writeMeshDebug(const std::string& filename)
{
  apf::FieldShape* fs =
      apf::getIPShape(getNumDim(), meshStruct->cubatureDegree);
  copyQPStatesToAPF(fs, true);
...
double
APFDiscretization::monotonicTimeLabel(const double time)
{
...
void
APFDiscretization::setResidualField(const Thyra_Vector& residual)
{
  Teuchos::ArrayRCP<const ST> data = getLocalData(residual);
  if (solLayout.getDerivNames(0).size() == 0) {
    // dont have split fields
    this->setField(
        APFMeshStruct::residual_name, data.getRawPtr(), /*overlapped=*/false);
  } else {
    this->setSplitFields(
        resNames,
...
APFDiscretization::getSolutionField(bool overlapped) const
{
  ...
  if (meshStruct->solutionInitialized) {
    if (solLayout.getDerivNames(0).size() == 0) {
      this->getField(
          APFMeshStruct::solution_name[0], data.getRawPtr(), overlapped);
    } else {
      this->getSplitFields(
          solLayout.getDerivNames(0),
          solLayout.getDerivSizes(0),
          data.getRawPtr(),
          overlapped);
    }
...
APFDiscretization::getSolutionMV(bool overlapped) const
{
  // Copy soln vector into solution field, one node at a time
  const auto&                     vs = overlapped ? m_overlap_vs : m_vs;
  Teuchos::RCP<Thyra_MultiVector> solution =
      Thyra::createMembers(vs, meshStruct->num_time_deriv + 1);
  ...
  for (int i = 0; i <= meshStruct->num_time_deriv; ++i) {
    ...
    if (meshStruct->solutionInitialized) {
      if (solLayout.getDerivNames(i).size() == 0) {
        this->getField(
            APFMeshStruct::solution_name[i], data.getRawPtr(), overlapped);
      } else {
        this->getSplitFields(
      ...
      if (solLayout.getDerivNames(i).size() == 0) {
        *out << __func__ << ": uninit field " << APFMeshStruct::solution_name[i]
             << '\n';
      } else {
        *out << __func__ << ": uninit fields " << solLayout.getDerivNames(i)
             << '\n';
      }
    }
  }
...
void
APFDiscretization::writeSolution(
...
void
APFDiscretization::writeSolutionMV(
...
void
APFDiscretization::writeSolutionToMeshDatabase(
    const Thyra_Vector& solution,
    const double /* time_value */,
    const bool overlapped)
{
...
void
APFDiscretization::writeSolutionMVToMeshDatabase(
    const Thyra_MultiVector& solution,
    const double /* time_value */,
    const bool overlapped)
{
...
void
APFDiscretization::writeSolutionToFile(
    const Thyra_Vector& solution,
    const double        time_value,
    const bool /* overlapped */)
{
...
void
APFDiscretization::writeSolutionMVToFile(
    const Thyra_MultiVector& solution,
    const double             time_value,
    const bool /* overlapped */)
{
...
int
APFDiscretization::nonzeroesPerRow(const int num_eq) const
{
  ...
  switch (numDim) {
    ...
    default:
      TEUCHOS_TEST_FOR_EXCEPTION(
          true, std::logic_error, "APFDiscretization:  Bad numDim" << numDim);
  }
...
void
APFDiscretization::computeOwnedNodesAndUnknowns()
{
  apf::Mesh* m = meshStruct->getMesh();
...
void
APFDiscretization::computeOverlapNodesAndUnknowns()
{
  apf::Mesh*      m       = meshStruct->getMesh();
...
void
APFDiscretization::computeGraphs()
{
...
void
APFDiscretization::computeWorksetInfo()
{
  apf::Mesh*       m      = meshStruct->getMesh();
  ...
  /*
   * Note: Max workset size is given in input file, or set to a default in
   * Albany_APFMeshStruct.cpp The workset size is set in
   * Albany_APFMeshStruct.cpp to be the maximum number in an element block if
   * the element block size < Max workset size.
   * STK bucket size is set to the workset size. We will "chunk" the elements
   * into worksets here.
   */
  ...
  while ((element = m->iterate(it))) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        !block,
        std::logic_error,
        "No element block for model region " << m->getModelTag(mr) << " at "
                                             << __FILE__ << " +" << __LINE__
                                             << '\n');
    // find the latest bucket being filled with elements for this block
    buck_it = bucketMap.find(block);
    if ((buck_it ==
         bucketMap.end()) ||  // this block hasn't been encountered yet
        (buckets[buck_it->second].size() >=
         static_cast<size_t>(
             worksetSize))) {  // the current bucket for this block is "full"
  ...
  if (meshStruct->allElementBlocksHaveSamePhysics) {
    ...
    for (int i = 0; i < numBuckets; i++) {
      wsPhysIndex[i] =
          meshStruct->getMeshSpecs()[0]->ebNameToIndex[wsEBNames[i]];
    }
  }
  ...
  for (int b = 0; b < numBuckets; b++) {
    ...
    // workset
    {
      ...
      const int nodes_per_element =
          apf::countElementNodes(shape, m->getType(element));
      wsElNodeEqID[b] =
  ...
  for (std::size_t i = 0; i < meshStruct->scalarValue_states.size(); i++) {
    // special case : need to store one double value that represents all the
    // elements in the workset (time) numBuckets are the number of worksets
  ...
  for (std::size_t b = 0; b < buckets.size(); b++) {
    ...
    for (std::size_t i = 0; i < meshStruct->elemnodescalar_states.size(); ++i) {
      stateArrays
          .elemStateArrays[b][meshStruct->elemnodescalar_states[i]->name] =
  ...
  if (Teuchos::nonnull(meshStruct->nodal_data_base) &&
    ...
    Teuchos::RCP<NodeFieldContainer> node_states =
        meshStruct->nodal_data_base->getNodeContainer();
...
void
APFDiscretization::forEachNodeSetNode(
...
void
APFDiscretization::computeNodeSets()
{
  ...
  auto fill_fn = [&](size_t owned_i, apf::StkModel* node_set) {
    ...
    dofLids.resize(neq);
    for (std::size_t eq = 0; eq < neq; eq++) dofLids[eq] = getDOF(owned_i, eq);
    ++(nodeSetSizes[NS_name]);
  };
...
void
APFDiscretization::computeSideSets()
{
  apf::Mesh*      m    = meshStruct->getMesh();
  apf::StkModels& sets = meshStruct->getSets();
  ...
  while ((side = m->iterate(it))) {
    ...
    if (!sets.invMaps[d - 1].count(me)) continue;
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        side_elems.n != 1,
        std::logic_error,
        "PUMIDisc: cannot figure out side set topology for side set "
            << ss_name << std::endl);
    ...
    int workset      = elemGIDws[sstruct.elem_GID]
                      .ws;  // workset ID that this element lives in
    sstruct.elem_LID =
        elemGIDws[sstruct.elem_GID].LID;  // local element id in this workset
    sstruct.elem_ebIndex =
        meshStruct->getMeshSpecs()[0]
            ->ebNameToIndex[wsEBNames[workset]];  // element block that workset
                                                  // lives in
    ...
    if (sit != ssList.end())           // The sideset has already been created
      sit->second.push_back(sstruct);  // Save this side to the vector that
...
void
APFDiscretization::copyQPScalarToAPF(
    unsigned           nqp,
...
void
APFDiscretization::copyQPVectorToAPF(
    unsigned           nqp,
    const std::string& stateName,
    apf::Field*        f)
{
  ...
  for (std::size_t b = 0; b < buckets.size(); ++b) {
    ...
    for (std::size_t e = 0; e < buck.size(); ++e) {
      for (std::size_t p = 0; p < nqp; ++p) {
        for (std::size_t i = 0; i < spdim; ++i) { v[i] = ar(e, p, i); }
...
void
APFDiscretization::copyQPTensorToAPF(
    unsigned           nqp,
    const std::string& stateName,
    apf::Field*        f)
{
    ...
    for (std::size_t e = 0; e < buck.size(); ++e) {
      for (std::size_t p = 0; p < nqp; ++p) {
        for (std::size_t i = 0; i < spdim; ++i) {
          for (std::size_t j = 0; j < spdim; ++j) { v[i][j] = ar(e, p, i, j); }
        }
...
void
APFDiscretization::copyQPStatesToAPF(apf::FieldShape* fs, bool copyAll)
{
  apf::Mesh2* m = meshStruct->getMesh();
  for (std::size_t i = 0; i < meshStruct->qpscalar_states.size(); ++i) {
    PUMIQPData<double, 2>& state = *(meshStruct->qpscalar_states[i]);
    if (!copyAll && !state.output) { continue; }
  ...
  if (meshStruct->saveStabilizedStress) { saveStabilizedStress(); }
}

void
APFDiscretization::removeQPStatesFromAPF()
{
  apf::Mesh2* m = meshStruct->getMesh();
  for (std::size_t i = 0; i < meshStruct->qpscalar_states.size(); ++i) {
    PUMIQPData<double, 2>& state = *(meshStruct->qpscalar_states[i]);
...
void
APFDiscretization::copyQPScalarFromAPF(
    unsigned           nqp,
    const std::string& stateName,
    apf::Field*        f)
{
...
void
APFDiscretization::copyQPVectorFromAPF(
    unsigned           nqp,
    const std::string& stateName,
    apf::Field*        f)
{
  ...
  for (std::size_t b = 0; b < buckets.size(); ++b) {
    ...
    for (std::size_t e = 0; e < buck.size(); ++e) {
      for (std::size_t p = 0; p < nqp; ++p) {
        ...
        for (std::size_t i = 0; i < spdim; ++i) { ar(e, p, i) = v[i]; }
      }
    }
  }
}
...
void
APFDiscretization::copyQPTensorFromAPF(
    unsigned           nqp,
    const std::string& stateName,
    apf::Field*        f)
{
  ...
  for (std::size_t b = 0; b < buckets.size(); ++b) {
    ...
    for (std::size_t e = 0; e < buck.size(); ++e) {
      for (std::size_t p = 0; p < nqp; ++p) {
        ...
        for (std::size_t i = 0; i < spdim; ++i) {
          for (std::size_t j = 0; j < spdim; ++j) { ar(e, p, i, j) = v[i][j]; }
        }
      }
    }
  }
...
void
APFDiscretization::copyQPStatesFromAPF()
{
  apf::Mesh2* m = meshStruct->getMesh();
  apf::Field* f;
  for (std::size_t i = 0; i < meshStruct->qpscalar_states.size(); ++i) {
    ...
    if (f) copyQPScalarFromAPF(nqp, state.name, f);
  }
  for (std::size_t i = 0; i < meshStruct->qpvector_states.size(); ++i) {
    PUMIQPData<double, 3>& state = *(meshStruct->qpvector_states[i]);
    int                    nqp   = state.dims[1];
    f                            = m->findField(state.name.c_str());
    if (f) copyQPVectorFromAPF(nqp, state.name, f);
  }
  for (std::size_t i = 0; i < meshStruct->qptensor_states.size(); ++i) {
    PUMIQPData<double, 4>& state = *(meshStruct->qptensor_states[i]);
    int                    nqp   = state.dims[1];
    f                            = m->findField(state.name.c_str());
    if (f) copyQPTensorFromAPF(nqp, state.name, f);
  }
}

void
APFDiscretization::copyNodalDataToAPF(const bool copy_all)
{
  ...
  for (auto& nfs : node_states) {
    Teuchos::RCP<PUMINodeDataBase<RealType>> nd =
        Teuchos::rcp_dynamic_cast<PUMINodeDataBase<RealType>>(nfs.second);
    TEUCHOS_TEST_FOR_EXCEPTION(
        nd.is_null(),
        std::logic_error,
    ...
    switch (nd->ndims()) {
      ...
      default:
        TEUCHOS_TEST_FOR_EXCEPTION(
            true, std::logic_error, "dim is not in {1,2,3}");
    }
...
void
APFDiscretization::removeNodalDataFromAPF()
{
  if (meshStruct->nodal_data_base.is_null()) { return; }
...
void
APFDiscretization::initTimeFromParamLib(Teuchos::RCP<ParamLib> paramLib)
{
  for (std::size_t b = 0; b < buckets.size(); ++b) {
    if (stateArrays.elemStateArrays[b].count("Time")) {
      TEUCHOS_TEST_FOR_EXCEPTION(
          !paramLib->isParameter("Time"),
          std::logic_error,
...
APFDiscretization::initMesh()
{
  ...
  // these fields
  if (meshStruct->shouldLoadLandIceData) { setLandIceData(); }
...
void
APFDiscretization::updateMesh(
    bool                   shouldTransferIPData,
    Teuchos::RCP<ParamLib> paramLib)
{
  ...
  if (shouldTransferIPData) { copyQPStatesFromAPF(); }

  // Use the parameter library to re-initialize Time state arrays
  if (Teuchos::nonnull(paramLib)) { initTimeFromParamLib(paramLib); }
}

void
APFDiscretization::attachQPData()
{
...
void
APFDiscretization::detachQPData()
{
  removeQPStatesFromAPF();
}

static apf::Field*
interpolate(apf::Field* nf, int cubatureDegree, char const* name)
{
...
static void
temperaturesToQP(apf::Mesh* m, int cubatureDegree)
{
...
void
APFDiscretization::initTemperatureHack()
{
  if (!meshStruct->useTemperatureHack) { return; }
  apf::Mesh* m = meshStruct->getMesh();
