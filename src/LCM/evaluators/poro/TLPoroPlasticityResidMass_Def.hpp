...
template <typename EvalT, typename Traits>
void
TLPoroPlasticityResidMass<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  bool print = false;
  ...
  for (int cell = 0; cell < workset.numCells; ++cell) {
    for (int node = 0; node < numNodes; ++node) {
      for (int qp = 0; qp < numQPs; ++qp) {
        temp = 3.0 - 12.0 * kcPermeability(cell, qp) * dt / (elementLength(cell, qp) * elementLength(cell, qp));
        ...
        if ((temp > 0) && (stab_param_ > 0)) {
          ...
          if (temp2 < 10.0 && temp2 > -10.0) {
            TResidual(cell, node) -= (porePressure(cell, qp) - porePressureold(cell, qp))
                                     ...
                                     * stab_param_ * std::abs(temp)  // should be 1 but use 0.5 for safety
            ...
            TResidual(cell, node) += pterm(cell, qp)
                                     //* stabParameter(cell, qp)
                                     * stab_param_ * std::abs(temp)  // should be 1 but use 0.5 for safety
                                     * (0.5 + 0.5 * std::tanh(temp2)) / biotModulus(cell, qp) * (wBF(cell, node, qp));
          } else if (temp2 >= 10.0) {
            TResidual(cell, node) -= (porePressure(cell, qp) - porePressureold(cell, qp))
                                     //* stabParameter(cell, qp)
                                     * stab_param_ * std::abs(temp)  // should be 1 but use 0.5 for safety
