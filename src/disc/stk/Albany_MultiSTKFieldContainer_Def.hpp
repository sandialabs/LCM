    ...
    int const                                    offset)
{
  ...
  for (auto it = all_elements.begin(); it != all_elements.end(); ++it) {
    ...
    STKFieldContainerHelper::fillVector(field_vector, *field, field_node_vs_indexer, bucket, nodalDofManager, offset);
  }
}
    ...
    int const                                    offset)
{
  ...
  for (auto it = all_elements.begin(); it != all_elements.end(); ++it) {
    const stk::mesh::Bucket& bucket = **it;
    STKFieldContainerHelper::saveVector(field_vector, *field, field_node_vs_indexer, bucket, nodalDofManager, offset);
  }
}
