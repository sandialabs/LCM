...
struct AerasMeshSpectStruct
{
      ...
      const Teuchos::RCP<Teuchos::ParameterList>&  discParams)
  {
    ...
    *out << "DEBUG: in AerasMeshSpectStruct!  Element Degree =  "
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        ...
        std::endl
            << "Error!  Attempting to enrich a non-quadrilateral element "
            << "(" << orig_name
            << ")!  Aeras::SpectralDiscretization is currently "
            << "implemented only for "
            << "Quadrilateral, ShellQuadrilateral and "
    ...
    *out << "DEBUG: original ctd key = " << orig_ctd.key << std::endl;
    ...
    // If cubature rule in input file is not GAUSS_LOBATTO, print warning and
    // reset cubature to Gauss-Lobatto.
    ...
    // For 1D elements, create a new key for the ctd -- this is needed for
    // Intrepid2 setJacobian function.
    if (orig_numDim == 1)
    ...
    *out << "DEBUG: new_ctd.key = " << new_ctd.key << std::endl;
    ...
    // new (enriched) ctd.
    return Teuchos::rcp(new Albany::MeshSpecsStruct(
        new_ctd,
        orig_numDim,
        new_cubatureDegree,
        orig_nsNames,
  ...
  // The following function sets the cubature degree based on the element degree
  // for spectral elements, so that the user does not need to worry about
  // specifying this in the input file. Cubature rules are only implemented for
  // elements up to degree 12.  Cubature rules for higher order elements may be
  // added, if desired.
  int
  setCubatureDegree(
      ...
      const Teuchos::RCP<Teuchos::ParameterList>& discParams)
  {
    ...
    switch (elementDegree) {
      ...
      default:
        TEUCHOS_TEST_FOR_EXCEPTION(
            true,
            std::logic_error,
            "Cubature Degree is not implemented for element of degree "
                << elementDegree << "!  "
                << "To use an element of this order, please implement the "
                   "right cubatureDegree to the setCubature "
    ...
    if (orig_cubatureDegree == cubatureDegree)
      *out << "Setting Cubature Degree to default value or value specified in "
              "input file: "
           << orig_cubatureDegree << std::endl;
    *out << "Setting Cubature Degree to " << cubatureDegree
         << " for element of degree " << elementDegree << std::endl;
...
class SpectralDiscretization : public Albany::AbstractDiscretization
{
 public:
  ...
  getNodeVectorSpace() const override;
  Teuchos::RCP<const Thyra_VectorSpace>
  getOverlapNodeVectorSpace() const override;

  //! Get solution DOF vector space (owned and overlapped)
  Teuchos::RCP<const Thyra_VectorSpace>
  getVectorSpace() const override;
  Teuchos::RCP<const Thyra_VectorSpace>
  getOverlapVectorSpace() const override;
  ...
  Teuchos::RCP<Thyra_LinearOp>
  createOverlapJacobianOp() const override
  {
  ...
  {
    return m_implicit_jac_factory->createOp();
  }
  Teuchos::RCP<Thyra_LinearOp>
  createImplicitOverlapJacobianOp() const
  {
    return m_implicit_overlap_jac_factory->createOp();
  }

  //! Get Node set lists (typedef in Albany_AbstractDiscretization.hpp)
  const Albany::NodeSetList&
  getNodeSets() const override
  {
    return nodeSets;
  };
  const Albany::NodeSetCoordList&
  getNodeSetCoords() const override
  {
    return nodeSetCoords;
  };
  const Albany::NodeSetGIDsList&
  ...
  const Albany::SideSetList&
  getSideSets(const int workset) const override
  {
  ...
  const Albany::Conn&
  getWsElNodeEqID() const override;
  ...
  getElNodeEqID(const std::string& /* field_name */) const override
  {
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        std::logic_error,
        "Albany::SpectralDiscretization: getElNodeEqID(const std::string& "
        "field_name) const not implemented");
  }
  ...
  getDOFManager(const std::string& /* field_name */) const override
  {
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        std::logic_error,
        "Albany::SpectralDiscretization: getDOFManager(const std::string& "
        "field_name) const not implemented");
  }
  ...
  getOverlapDOFManager(const std::string& /* field_name */) const override
  {
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        std::logic_error,
        "Albany::SpectralDiscretization: getOverlapDOFManager(const "
        "std::string& field_name) const not implemented");
  }
  ...
  void
  setCoordinates(const Teuchos::ArrayRCP<const double>& c) override;
  ...
  getContactManager() const override;
#endif
  ...
  void
  printConnectivity(bool printEdges = false) const;
  void
  printCoordsAndGIDs() const;

  //! Get sideSet discretizations map
  const SideSetDiscretizationsType&
  getSideSetDiscretizations() const override
  {
    // warning: returning empty structure (feature not supported with this
    // discretization)
  ...
  getSideToSideSetCellMap() const override
  {
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        std::logic_error,
        "Error! Functionality not supported by Aeras discretization.\n");
  ...
  getSideNodeNumerationMap() const override
  {
    TEUCHOS_TEST_FOR_EXCEPTION(
        ...
        std::logic_error,
        "Error! Functionality not supported by Aeras discretization.\n");
    static std::map<std::string, std::map<GO, std::vector<int>>>
        sideNodeNumerationMap;
    return sideNodeNumerationMap;
  }

  //! Set stateArrays
  void
  setStateArrays(Albany::StateArrays& sa) override
  {
    stateArrays = sa;
    return;
  }
  ...
  getWsEBNames() const override;
  //! Retrieve Vector (length num worksets) of physics set index
  const Albany::WorksetArray<int>::type&
  getWsPhysIndex() const override;
  ...
  bool
  isExplicitScheme() const override
  {
  ...
  Teuchos::RCP<Albany::LayeredMeshNumbering<LO>>
  getLayeredMeshNumbering() const override
  {
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        std::logic_error,
        "Albany::SpectralDiscretization: getLayeredMeshNumbering() not "
        "implemented");
    return Teuchos::null;
  }
 ...
 private:
  ...
  //! Private to prohibit copying
  SpectralDiscretization&
  operator=(const SpectralDiscretization&);

  inline GO
  gid(const stk::mesh::Entity node) const;

  // Copy values from STK Mesh field to given Thyra_Vector
  void
  getSolutionField(Thyra_Vector& result, bool overlapped = false) const;

  void
  getSolutionMV(Thyra_MultiVector& result, bool overlapped = false) const;

  //! Copy field from STK Mesh field to given Thyra_Vector
  void
  getField(Thyra_Vector& field_vector, const std::string& field_name)
      const override;
  ...
  void
  setSolutionField(const Thyra_Vector& solution, const bool overlapped);
  void
  setSolutionField(
      const Thyra_Vector& solution,
      const Thyra_Vector& solution_dot,
  ...
  setSolutionField(
      ...
      const bool          overlapped);
  void
  setSolutionFieldMV(const Thyra_MultiVector& solution, const bool overlapped);
  ...
  int
  processNetCDFOutputRequestT(const Thyra_Vector&);
  ...
  // Create enum type for the different kinds of elements (currently lines and
  // quads)
  enum elemType
  {
    LINE,
 ...
 protected:
  ...
  int points_per_edge;  // number of points per edge (i.e., the degree of
                        // enrichment) -- read in from ParameterList.
