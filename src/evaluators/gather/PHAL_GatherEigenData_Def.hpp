...
GatherEigenDataBase<EvalT, Traits>::GatherEigenDataBase(
    ...
    const Teuchos::RCP<Albany::Layouts>& dl)
{
  ...
  std::string eigenvector_name_root =
      p.get<std::string>("Eigenvector name root");
...
template <typename EvalT, typename Traits>
void
GatherEigenDataBase<EvalT, Traits>::postRegistrationSetup(
...
template <typename EvalT, typename Traits>
void
GatherEigenDataBase<EvalT, Traits>::evaluateFields(
    typename Traits::EvalData workset)
{
  if (nEigenvectors == 0) { return; }

  if (workset.eigenDataPtr != Teuchos::null &&
      workset.eigenDataPtr->eigenvectorRe != Teuchos::null) {
    auto             nodeID = workset.wsElNodeEqID;
    std::vector<int> dims;
    eigenvector_Re[0].dimensions(dims);
    if ((workset.eigenDataPtr->eigenvectorIm != Teuchos::null)) {
      // Gather real and imaginary eigenvalues from workset Eigendata info
      // structure
      ...
      // Gather real and imaginary eigenvectors from workset Eigendata info
      // structure
    ...
    } else {  // Only real parts of eigenvectors is given -- "gather" zeros into
              // imaginary fields
      // Gather real and imaginary eigenvalues from workset Eigendata info
      // structure
      Teuchos::RCP<const Thyra_MultiVector> e_r =
      ...
      for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
        for (std::size_t node = 0; node < this->numNodes; ++node) {
          for (int dof = 0; dof < dims[2]; ++dof) {
            ...
            for (int k = 0; k < numVecsToGather; ++k) {
              (this->eigenvector_Re[k])(cell, node, dof) =
                  e_r_data[k][offset_eq];
...
template <typename EvalT, typename Traits>
GatherEigenData<EvalT, Traits>::GatherEigenData(
    const Teuchos::ParameterList&        p,
    const Teuchos::RCP<Albany::Layouts>& dl)
...
GatherEigenData<PHAL::AlbanyTraits::Residual, Traits>::GatherEigenData(
    const Teuchos::ParameterList&        p,
    const Teuchos::RCP<Albany::Layouts>& dl)
...
template <typename Traits>
GatherEigenData<PHAL::AlbanyTraits::Jacobian, Traits>::GatherEigenData(
    const Teuchos::ParameterList&        p,
    const Teuchos::RCP<Albany::Layouts>& dl)
...
template <typename Traits>
void GatherEigenData<PHAL::AlbanyTraits::Jacobian, Traits>::evaluateFields(
    typename Traits::EvalData /* workset */)
{
  /*
    if(nEigenvectors == 0) return;
    ...
    if(workset.eigenDataPtr != Teuchos::null) {
       if(workset.eigenDataPtr->eigenvectorRe != Teuchos::null) {
          ...
          if(workset.eigenDataPtr->eigenvectorIm != Teuchos::null) {
             ...
             const Epetra_MultiVector& e_r =
    *(workset.eigenDataPtr->eigenvectorRe); const Epetra_MultiVector& e_i =
    *(workset.eigenDataPtr->eigenvectorIm); const std::vector<double> &v_r =
    *(workset.eigenDataPtr->eigenvalueRe); const std::vector<double> &v_i =
    *(workset.eigenDataPtr->eigenvalueIm); int numVecsInWorkset =
    std::min(e_r.NumVectors(),e_i.NumVectors()); int numVecsToGather  =
    std::min(numVecsInWorkset, (int)nEigenvectors); for (std::size_t k = 0; k <
    numVecsToGather; ++k) { (this->eigenvalue_Re[k])(0) = v_r[k];
               (this->eigenvalue_Im[k])(0) = v_i[k];
             }

             //Gather real and imaginary eigenvectors from workset Eigendata
    info structure for (std::size_t cell=0; cell < workset.numCells; ++cell ) {
        for(std::size_t node =0; node < this->numNodes; ++node) {
    ...
    *(workset.eigenDataPtr->eigenvectorRe); const Epetra_MultiVector& e_i =
    *(workset.eigenDataPtr->eigenvectorIm); const std::vector<double> &v_r =
    *(workset.eigenDataPtr->eigenvalueRe); int numVecsInWorkset =
    std::min(e_r.NumVectors(),e_i.NumVectors()); int numVecsToGather  =
    std::min(numVecsInWorkset, (int)nEigenvectors); for (std::size_t k = 0; k <
    numVecsToGather; ++k) { (this->eigenvalue_Re[k])(0) = v_r[k];
