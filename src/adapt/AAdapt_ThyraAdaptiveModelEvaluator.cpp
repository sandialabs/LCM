...
Teuchos::RCP<Teuchos::StringToIntegralParameterEntryValidator<
...
get_Epetra_RowMatrix(const EpetraExt::ModelEvaluator::OutArgs& epetraOutArgs)
{
  using Teuchos::RCP;
  const RCP<Epetra_Operator>  eW = epetraOutArgs.get_W();
  const RCP<Epetra_RowMatrix> ermW =
      Teuchos::rcp_dynamic_cast<Epetra_RowMatrix>(eW, false);
  TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      "\"" << epetraOutArgs.modelEvalDescription()
           << "\"\n"
              "must support the Epetra_RowMatrix interface through a dynamic "
              "cast.\n"
              "The concrete type "
           << Teuchos::typeName(*eW)
           << " does not support\n"
...
create_and_assert_W(const EpetraExt::ModelEvaluator& epetraModel)
{
  using Teuchos::RCP;
  RCP<Epetra_Operator> eW = epetraModel.create_W();
  TEUCHOS_TEST_FOR_EXCEPTION(
      is_null(eW),
      std::logic_error,
...
void
ThyraAdaptiveModelEvaluator::initialize(
    const RCP<const EpetraExt::ModelEvaluator>&  epetraModel,
    const RCP<LinearOpWithSolveFactoryBase<ST>>& W_factory)
{
  ...
  p_map_.resize(inArgs.Np());
  p_space_.resize(inArgs.Np());
  p_map_is_local_.resize(inArgs.Np(), false);
  for (int l = 0; l < implicit_cast<int>(p_space_.size()); ++l) {
    RCP<const Epetra_Map> p_map_l = (p_map_[l] = epetraModel_->get_p_map(l));
#ifdef TEUCHOS_DEBUG
    TEUCHOS_TEST_FOR_EXCEPTION(
        is_null(p_map_l),
        std::logic_error,
  ...
  g_map_.resize(outArgs.Ng());
  g_space_.resize(outArgs.Ng());
  g_map_is_local_.resize(outArgs.Ng(), false);
  for (int j = 0; j < implicit_cast<int>(g_space_.size()); ++j) {
    RCP<const Epetra_Map> g_map_j = (g_map_[j] = epetraModel_->get_g_map(j));
...
void
ThyraAdaptiveModelEvaluator::setNominalValues(
...
void
ThyraAdaptiveModelEvaluator::setStateVariableScalingVec(
...
void
ThyraAdaptiveModelEvaluator::setStateFunctionScalingVec(
...
void
ThyraAdaptiveModelEvaluator::uninitialize(
...
bool
ThyraAdaptiveModelEvaluator::finalPointWasSolved() const
{
  return finalPointWasSolved_;
}

const Teuchos::RCP<Thyra::VectorBase<ST>>
ThyraAdaptiveModelEvaluator::resize_g_space(
    int                            index,
    Teuchos::RCP<const Epetra_Map> map)
{
  RCP<const Epetra_Map> g_map_j = (g_map_[index] = map);
  ...
  const Teuchos::RCP<Thyra::VectorBase<ST>> g_j =
      Thyra::createMember(*g_space_[index]);
...
std::string
ThyraAdaptiveModelEvaluator::description() const
{
...
void
ThyraAdaptiveModelEvaluator::setParameterList(
    RCP<Teuchos::ParameterList> const& paramList)
{
  TEUCHOS_TEST_FOR_EXCEPT(is_null(paramList));
  paramList->validateParameters(
      *getValidParameters(), 0);  // Just validate my params
...
ThyraAdaptiveModelEvaluator::getValidParameters() const
{
  ...
  if (is_null(validPL)) {
    ...
    stateFunctionScalingValidator =
        rcp(new StringToIntegralParameterEntryValidator<EStateFunctionScaling>(
            tuple<std::string>("None", "Row Sum"),
            tuple<std::string>(
            ...
            tuple<EStateFunctionScaling>(
                STATE_FUNC_SCALING_NONE, STATE_FUNC_SCALING_ROW_SUM),
            StateFunctionScaling_name));
    pl->set(
        StateFunctionScaling_name,
        StateFunctionScaling_default,
        "Determines if and how the state function f(...) and all of its\n"
        "derivatives are scaled.  The scaling is done explicitly so there "
        "should\n"
...
int
ThyraAdaptiveModelEvaluator::Np() const
{
  return p_space_.size();
}

int
ThyraAdaptiveModelEvaluator::Ng() const
{
...
ModelEvaluatorBase::InArgs<ST>
ThyraAdaptiveModelEvaluator::createInArgs() const
{
  if (!currentInArgsOutArgs_) updateInArgsOutArgs();
  return prototypeInArgs_;
}

void
ThyraAdaptiveModelEvaluator::reportFinalPoint(
...
void
ThyraAdaptiveModelEvaluator::evalModelImpl(
    const ModelEvaluatorBase::InArgs<ST>&  inArgs_in,
    const ModelEvaluatorBase::OutArgs<ST>& outArgs) const
{
  ...
  if (inArgs.supports(Thyra::ModelEvaluatorBase::IN_ARG_x_dot)) {
    if (is_null(inArgs_in.get_x_dot())) inArgs.set_x_dot(Teuchos::null);
  }
  ...
  typedef Teuchos::VerboseObjectTempState<LinearOpWithSolveFactoryBase<ST>>
            VOTSLOWSF;
  ...
  convertOutArgsFromThyraToEpetra(
      outArgs, &evaluated_epetraUnscaledOutArgs, &W_op, &efwdW, &eW);
  ...
  if (firstTimeStateFuncScaling) {
    preEvalScalingSetup(
        &epetraInArgs, &evaluated_epetraUnscaledOutArgs, out, verbLevel);
  }

  timer.stop();
  if (out.get() && includesVerbLevel(verbLevel, Teuchos::VERB_LOW))
    OSTab(out).o() << "\nTime to setup OutArgs = " << timer.totalElapsedTime()
                   << " sec\n";
  ...
  finishConvertingOutArgsFromEpetraToThyra(
      epetraOutArgs, W_op, efwdW, eW, outArgs);
...
void
ThyraAdaptiveModelEvaluator::convertInArgsFromEpetraToThyra(
    const EpetraExt::ModelEvaluator::InArgs& epetraInArgs,
    ModelEvaluatorBase::InArgs<ST>*          inArgs) const
{
  using Teuchos::implicit_cast;
  typedef ModelEvaluatorBase MEB;
  ...
  if (inArgs->supports(MEB::IN_ARG_t)) { inArgs->set_t(epetraInArgs.get_t()); }
}

void
ThyraAdaptiveModelEvaluator::convertInArgsFromThyraToEpetra(
    const ModelEvaluatorBase::InArgs<ST>& inArgs,
    EpetraExt::ModelEvaluator::InArgs*    epetraInArgs) const
{
  using Teuchos::rcp;
  using Teuchos::rcp_const_cast;
  ...
  if (inArgs.supports(IN_ARG_x_poly) && (x_poly = inArgs.get_x_poly()).get()) {
  ...
  if (inArgs.supports(IN_ARG_t)) epetraInArgs->set_t(inArgs.get_t());

  if (inArgs.supports(IN_ARG_alpha))
    epetraInArgs->set_alpha(inArgs.get_alpha());

  if (inArgs.supports(IN_ARG_beta)) epetraInArgs->set_beta(inArgs.get_beta());
}

void
ThyraAdaptiveModelEvaluator::convertOutArgsFromThyraToEpetra(
    ...
    RCP<Epetra_Operator>*                  eW_out) const
{
  using Teuchos::implicit_cast;
  using Teuchos::OSTab;
  using Teuchos::rcp;
  ...
  // DgDx_dot
  {
    ...
    for (int j = 0; j < outArgs.Ng(); ++j) {
      if (!outArgs.supports(OUT_ARG_DgDx_dot, j).none() &&
        ...
        epetraUnscaledOutArgs.set_DgDx_dot(
            j, convert(DgDx_dot_j, g_map_[j], x_map_));
      }
    }
  }
      ...
      for (int l = 0; l < outArgs.Np(); ++l) {
        if (!(DgDp_j_l_support = outArgs.supports(OUT_ARG_DgDp, j, l)).none() &&
          ...
          epetraUnscaledOutArgs.set_DgDp(
              j, l, convert(DgDp_j_l, g_map_[j], p_map_[l]));
        }
      }
    }
  }
  ...
  if (outArgs.supports(OUT_ARG_f_poly) &&
    ...
    for (unsigned int i = 0; i <= f_poly->degree(); i++) {
      RCP<Epetra_Vector> epetra_ptr = Teuchos::rcp_const_cast<Epetra_Vector>(
          get_Epetra_Vector(*f_map_, f_poly->getCoefficient(i)));
...
void
ThyraAdaptiveModelEvaluator::preEvalScalingSetup(
    ...
    const Teuchos::EVerbosityLevel      verbLevel) const
{
  ...
  EpetraExt::ModelEvaluator::InArgs&  epetraInArgs = *epetraInArgs_inout;
  EpetraExt::ModelEvaluator::OutArgs& epetraUnscaledOutArgs =
      *epetraUnscaledOutArgs_inout;

  if ((stateFunctionScaling_ == STATE_FUNC_SCALING_ROW_SUM) &&
    ...
    if (epetraInArgs.supports(EME::IN_ARG_beta)) epetraInArgs.set_beta(1.0);
    if (epetraInArgs.supports(EME::IN_ARG_alpha)) epetraInArgs.set_alpha(0.0);
  }
}

void
ThyraAdaptiveModelEvaluator::postEvalScalingSetup(
    ...
    const Teuchos::EVerbosityLevel            verbLevel) const
{
  ...
  switch (stateFunctionScaling_) {
    case STATE_FUNC_SCALING_ROW_SUM: {
      ...
      const RCP<Epetra_RowMatrix> ermW =
          get_Epetra_RowMatrix(epetraUnscaledOutArgs);
      ...
      RCP<Epetra_Vector> invRowSums =
          rcp(new Epetra_Vector(ermW->OperatorRangeMap()));
...
void
ThyraAdaptiveModelEvaluator::finishConvertingOutArgsFromEpetraToThyra(
...
void
ThyraAdaptiveModelEvaluator::updateNominalValuesAndBounds() const
{
  using Teuchos::implicit_cast;
  using Teuchos::rcp;
  typedef ModelEvaluatorBase        MEB;
  ...
  if (!nominalValuesAndBoundsAreUpdated_) {
    ...
    EpetraExt::scaleModelBounds(
        epetraOrigLowerBounds,
        epetraOrigUpperBounds,
        epetraModel_->getInfBound(),
        epetraInArgsScaling_,
        &epetraScaledLowerBounds,
        &epetraScaledUpperBounds);
...
void
ThyraAdaptiveModelEvaluator::updateInArgsOutArgs() const
{
  typedef EpetraExt::ModelEvaluator EME;
...
#ifdef HAVE_THYRA_ME_POLYNOMIAL
  inArgs.setSupports(
      IN_ARG_x_dot_poly, epetraInArgs.supports(EME::IN_ARG_x_dot_poly));
    ...
    for (int l = 0; l < l_Np; ++l) {
      ...
      if (!outArgs.supports(OUT_ARG_DfDp, l).none())
        outArgs.set_DfDp_properties(
            l, convert(epetraOutArgs.get_DfDp_properties(l)));
    }
  }
  // DgDx_dot and DgDx
  for (int j = 0; j < l_Ng; ++j) {
    ...
    if (!outArgs.supports(OUT_ARG_DgDx_dot, j).none())
      outArgs.set_DgDx_dot_properties(
          j, convert(epetraOutArgs.get_DgDx_dot_properties(j)));
    ...
    if (!outArgs.supports(OUT_ARG_DgDx, j).none())
      outArgs.set_DgDx_properties(
          j, convert(epetraOutArgs.get_DgDx_properties(j)));
  }
  // DgDp
  for (int j = 0; j < l_Ng; ++j)
    for (int l = 0; l < l_Np; ++l) {
      ...
      outArgs.setSupports(OUT_ARG_DgDp, j, l, convert(epetra_DgDp_j_l_support));
      if (!outArgs.supports(OUT_ARG_DgDp, j, l).none())
        outArgs.set_DgDp_properties(
            j, l, convert(epetraOutArgs.get_DgDp_properties(j, l)));
    }
#ifdef HAVE_THYRA_ME_POLYNOMIAL
  outArgs.setSupports(
      OUT_ARG_f_poly, epetraOutArgs.supports(EME::OUT_ARG_f_poly));
#endif  // HAVE_THYRA_ME_POLYNOMIAL
