...
Aeras::SpectralOutputSTKMeshStruct::SpectralOutputSTKMeshStruct(
    ...
    const Albany::WorksetArray<
        Teuchos::ArrayRCP<Teuchos::ArrayRCP<double*>>>::type& coords_,
      ...
      points_per_edge(points_per_edge_)
{
  ...
  *out << "numDim, cub, worksetSize, points_per_edge, ctd name: " << numDim
       << ", " << cub << ", " << worksetSize << ", " << points_per_edge << ", "
       << ctd.name << "\n";
#endif
...

{
  ...
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << "\n";
  ...
  if (ElemType == QUAD) {  // Quads
#ifdef OUTPUT_TO_SCREEN
    std::cout << "Spectral Mesh # ws, # eles: " << wsElNodeID.size() << ", "
              << wsElNodeID[0].size() << std::endl;
    for (int ws = 0; ws < wsElNodeID.size(); ws++) {
      for (int e = 0; e < wsElNodeID[ws].size(); e++) {
        std::cout << "Spectral Mesh Element " << e << ": Nodes = ";
        for (size_t inode = 0; inode < points_per_edge * points_per_edge;
             ++inode)
    ...
    GO count = 0;
    GO numOutputEles =
        wsElNodeID[0].size() * (points_per_edge - 1) * (points_per_edge - 1);
    for (int ws = 0; ws < wsElNodeID.size(); ws++) {     // workset
      for (int e = 0; e < wsElNodeID[ws].size(); e++) {  // cell
        for (int i = 0; i < points_per_edge - 1;
             i++) {  // Each spectral element broken into (points_per_edge-1)^2
                     // bilinear elements
          for (int j = 0; j < points_per_edge - 1; j++) {
            ...
            // Add 1 to elem_id in the following line b/c STK is 1-based whereas
            // wsElNodeID is 0-based
            stk::mesh::Entity elem = bulkData->declare_entity(
                stk::topology::ELEMENT_RANK, 1 + elem_id, singlePartVec);
            stk::mesh::Entity node0 = bulkData->declare_entity(
                stk::topology::NODE_RANK,
                1 + wsElNodeID[ws][e][i + j * points_per_edge],
                nodePartVec);
            stk::mesh::Entity node1 = bulkData->declare_entity(
                stk::topology::NODE_RANK,
                1 + wsElNodeID[ws][e][i + 1 + j * points_per_edge],
                nodePartVec);
            stk::mesh::Entity node2 = bulkData->declare_entity(
                stk::topology::NODE_RANK,
                1 + wsElNodeID[ws][e]
                              [i + points_per_edge + 1 + j * points_per_edge],
                nodePartVec);
            stk::mesh::Entity node3 = bulkData->declare_entity(
                stk::topology::NODE_RANK,
                1 + wsElNodeID[ws][e]
                              [i + points_per_edge + j * points_per_edge],
                nodePartVec);
#ifdef OUTPUT_TO_SCREEN
            std::cout << "ws, e, i , j " << ws << ", " << e << ", " << i << ", "
                      << j << std::endl;
            std::cout
                << "Output Mesh elem_GID, node0, node1, node2, node3: "
                << elem_GID << ", "
...
#ifdef OUTPUT_TO_SCREEN
            std::cout << "Output mesh node0 coords: "
                      << coords[ws][e][i + j * points_per_edge][0] << ", "
                      << coords[ws][e][i + j * points_per_edge][1] << ", "
                      << coords[ws][e][i + j * points_per_edge][2] << std::endl;
#endif
...
#ifdef OUTPUT_TO_SCREEN
            std::cout << "Output mesh node1 coords: "
                      << coords[ws][e][i + 1 + j * points_per_edge][0] << ", "
                      << coords[ws][e][i + 1 + j * points_per_edge][1] << ", "
                      << coords[ws][e][i + 1 + j * points_per_edge][2]
                      << std::endl;
#endif
...
#ifdef OUTPUT_TO_SCREEN
            std::cout
                << "Output mesh node2 coords: "
                << coords[ws][e][i + points_per_edge + 1 + j * points_per_edge]
                         [0]
                << ", "
                << coords[ws][e][i + points_per_edge + 1 + j * points_per_edge]
                         [1]
...
#endif
            coord[0] =
                coords[ws][e][i + points_per_edge + 1 + j * points_per_edge][0];
            coord[1] =
                coords[ws][e][i + points_per_edge + 1 + j * points_per_edge][1];
            coord[2] =
                coords[ws][e][i + points_per_edge + 1 + j * points_per_edge][2];
            // set node 3 in STK bilinear mesh
            coord = stk::mesh::field_data(*coordinates_field, node3);
#ifdef OUTPUT_TO_SCREEN
            std::cout
                << "Output mesh node3 coords: "
                << coords[ws][e][i + points_per_edge + j * points_per_edge][0]
                << ", "
                << coords[ws][e][i + points_per_edge + j * points_per_edge][1]
                << ", "
                << coords[ws][e][i + points_per_edge + j * points_per_edge][2]
                << std::endl;
#endif
            coord[0] =
                coords[ws][e][i + points_per_edge + j * points_per_edge][0];
    ...
    for (int ws = 0; ws < wsElNodeID.size(); ws++) {     // workset
      for (int e = 0; e < wsElNodeID[ws].size(); e++) {  // cell
        for (int i = 0; i < points_per_edge - 1;
             i++) {  // Each spectral element broken into (points_per_edge-1)
                     // linear elements
          // Set connectivity for new mesh
          const GO elem_GID =
              count + numOutputEles * commT->getRank() * commT->getSize();
          count++;
          stk::mesh::EntityId elem_id = (stk::mesh::EntityId)elem_GID;
          singlePartVec[0]            = partVec[ebNo];
          // Add 1 to elem_id in the following line b/c STK is 1-based whereas
          ...
          stk::mesh::Entity elem = bulkData->declare_entity(
              stk::topology::ELEMENT_RANK, 1 + elem_id, singlePartVec);
          stk::mesh::Entity node0 = bulkData->declare_entity(
              stk::topology::NODE_RANK, 1 + wsElNodeID[ws][e][i], nodePartVec);
          stk::mesh::Entity node1 = bulkData->declare_entity(
              stk::topology::NODE_RANK,
              1 + wsElNodeID[ws][e][i + 1],
              nodePartVec);
#ifdef OUTPUT_TO_SCREEN
...
Aeras::SpectralOutputSTKMeshStruct::getValidDiscretizationParametersQuads()
    const
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << "\n";
...
Teuchos::RCP<const Teuchos::ParameterList>
Aeras::SpectralOutputSTKMeshStruct::getValidDiscretizationParametersLines()
    const
{
#ifdef OUTPUT_TO_SCREEN
  *out << "DEBUG: " << __PRETTY_FUNCTION__ << "\n";
  ...
  validPL->set<bool>(
      "Periodic_x BC", false, "Flag to indicate periodic mesh in x-dimension");
  // IKT, 8/31/15: why are Periodic_y BC and Periodic_z BC needed in valid
  // parameterlist when we will always have 1D mesh?
  validPL->set<bool>(
      "Periodic_y BC", false, "Flag to indicate periodic mesh in y-dimension");
  validPL->set<bool>(
      "Periodic_z BC", false, "Flag to indicate periodic mesh in z-dimension");
