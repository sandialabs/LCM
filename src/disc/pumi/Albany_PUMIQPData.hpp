...
template <
    typename DataType,
    unsigned Dim,
    class traits = PUMIQPData_Traits<DataType, Dim>>
class PUMIQPData
{
 public:
  PUMIQPData(
  ...
  void
  reAllocateBuffer(const std::size_t nelems);
  Albany::MDArray
  getMDA(const std::size_t nElemsInBucket);

  const std::string name;  // Name of data field
  const bool output;  // Is field output to disk each time step (or at end of
                      // simulation)?
  std::vector<DataType> buffer;  // array storage for shards::Array
  std::vector<PHX::DataLayout::size_type> dims;
  int         nfield_dofs;      // total number of dofs in this field
  std::size_t beginning_index;  // Buffer starting location for the next array
                                // allocation
};
...
struct PUMIQPData_Traits<T, 2>
{
  ...
  {
    size = 2
  };  // Two array dimension tags: Cell and QuadPoint
...
struct PUMIQPData_Traits<T, 3>
{
  ...
  {
    size = 3
  };  // Three array dimension tags: Cell, QuadPoint, and Dim
  typedef shards::Array<T, shards::NaturalOrder, Cell, QuadPoint, Dim>
      field_type;
  static Albany::MDArray
  buildArray(
      T*                                       buf,
...
struct PUMIQPData_Traits<T, 4>
{
  ...
  {
    size = 4
  };  // Four array dimension tags: Cell, QuadPoint, Dim, and Dim
  typedef shards::Array<T, shards::NaturalOrder, Cell, QuadPoint, Dim, Dim>
      field_type;
  static Albany::MDArray
  buildArray(
      T*                                       buf,
