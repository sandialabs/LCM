...
ThermoPoroPlasticityResidEnergy<EvalT, Traits>::ThermoPoroPlasticityResidEnergy(Teuchos::ParameterList const& p)
    : wBF(p.get<std::string>("Weighted BF Name"), p.get<Teuchos::RCP<PHX::DataLayout>>("Node QP Scalar Data Layout")),
      ...
      gammaMixture(p.get<std::string>("Mixture Specific Heat Name"), p.get<Teuchos::RCP<PHX::DataLayout>>("QP Scalar Data Layout")),
      gammaFluid(p.get<std::string>("Pore-Fluid Specific Heat Name"), p.get<Teuchos::RCP<PHX::DataLayout>>("QP Scalar Data Layout")),
      ...
      PGrad(p.get<std::string>("Pore Pressure Gradient Name"), p.get<Teuchos::RCP<PHX::DataLayout>>("QP Vector Data Layout")),
...
template <typename EvalT, typename Traits>
void
ThermoPoroPlasticityResidEnergy<EvalT, Traits>::postRegistrationSetup(typename Traits::SetupData d, PHX::FieldManager<Traits>& fm)
{
...
ThermoPoroPlasticityResidEnergy<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  ...
  for (int cell = 0; cell < workset.numCells; ++cell) {
    for (int node = 0; node < numNodes; ++node) {
      for (int qp = 0; qp < numQPs; ++qp) {
        for (int dim = 0; dim < numDims; ++dim) {
          TResidual(cell, node) -= dt * gammaFluid(cell, qp) * porosity(cell, qp) * fluxdt(cell, qp, dim) * TGrad(cell, qp, dim) * wBF(cell, node, qp);
        }
      }
    }
  }
  ...
  for (int cell = 0; cell < workset.numCells; ++cell) {
    for (int node = 0; node < numNodes; ++node) {
      ...
      for (int qp = 0; qp < numQPs; ++qp) {
        ...
        TResidual(cell, node) += 3.0 * alphaSkeleton(cell, qp) * bulk(cell, qp) * Temp(cell, qp) * dJ * wBF(cell, node, qp);
  ...
  for (int cell = 0; cell < workset.numCells; ++cell) {
    for (int node = 0; node < numNodes; ++node) {
      for (int qp = 0; qp < numQPs; ++qp) {
        ...
        TResidual(cell, node) -= 3.0 * dporePressure * Temp(cell, qp) * stabParameter(cell, qp) * alphaMixture(cell, qp) * wBF(cell, node, qp);

        TResidual(cell, node) += (dTemperature)*stabParameter(cell, qp) *
                                 (gammaMixture(cell, qp) + Temp(cell, qp) * 9.0 * alphaSkeleton(cell, qp) * bulkModulus * alphaSkeleton(cell, qp) *
                                                               bulkModulus / (bulkModulus + 4.0 / 3.0 * shearModulus)) *
                                 wBF(cell, node, qp);
      }
    }
  }
