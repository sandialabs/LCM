...
inline double
dot(blas_int n, const double* x, blas_int incx, const double* y, blas_int incy)
{
...
// No error checking.
inline double
dot(const vector<double>& x, const vector<double>& y)
{
  return dot(x.size(), &x[0], 1, &y[0], 1);
}
inline vector<double>&
axpy(const double a, const vector<double>& x, vector<double>& y)
{
...
inline int
numel(const mxArray* ma)
{
  return mxGetNumberOfElements(ma);
}

inline bool
get_stringv(const mxArray* ms, vector<char>& s)
{
...
inline bool
get_string(const mxArray* ms, string& s)
{
  vector<char> vs;
...
inline bool
are_same_array(const mxArray* ma, const mxArray* mb)
{
  return mxGetPr(ma) == mxGetPr(mb);
}

string
init_mex(int& nrhs, const mxArray**& prhs)
{
