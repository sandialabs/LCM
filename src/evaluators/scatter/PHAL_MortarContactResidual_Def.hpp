...
MortarContactResidual<PHAL::AlbanyTraits::Tangent, Traits>::
    ...
    : MortarContactResidualBase<PHAL::AlbanyTraits::Tangent, Traits>(p, dl),
      numFields(MortarContactResidualBase<PHAL::AlbanyTraits::Tangent, Traits>::
                    numFieldsBase)
{
}
...
template <typename Traits>
void
MortarContactResidual<PHAL::AlbanyTraits::Tangent, Traits>::evaluateFields(
    typename Traits::EvalData workset)
{
  auto                             nodeID = workset.wsElNodeEqID;
  Teuchos::RCP<Tpetra_Vector>      fT     = workset.fT;
  ...
  for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
    for (std::size_t node = 0; node < this->numNodes; ++node) {
      for (std::size_t eq = 0; eq < numFields; eq++) {
        ...
        if (Teuchos::nonnull(fT)) fT->sumIntoLocalValue(row, valref.val());
        ...
        if (Teuchos::nonnull(fpT))
          for (int col = 0; col < workset.num_cols_p; col++)
            fpT->sumIntoLocalValue(
                row, col, valref.dx(col + workset.param_offset));
      }
    }
  }
}
...
MortarContactResidual<PHAL::AlbanyTraits::DistParamDeriv, Traits>::
    MortarContactResidual(
        const Teuchos::ParameterList&        p,
        const Teuchos::RCP<Albany::Layouts>& dl)
    : MortarContactResidualBase<PHAL::AlbanyTraits::DistParamDeriv, Traits>(
          p,
          dl),
      numFields(MortarContactResidualBase<
                PHAL::AlbanyTraits::DistParamDeriv,
                Traits>::numFieldsBase)
{
}
    ...
    evaluateFields(typename Traits::EvalData workset)
{
  ...
  if (workset.local_Vp[0].size() == 0)
    return;  // In case the parameter has not been gathered, e.g. parameter is
             // used only in Dirichlet conditions.
    ...
    for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
      ...
      for (int i = 0; i < num_deriv; i++) {
        for (int col = 0; col < num_cols; col++) {
          ...
          if (row >= 0) fpVT->sumIntoLocalValue(row, col, val);
        }
      }
    }
  } else {
