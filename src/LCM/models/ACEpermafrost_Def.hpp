    ...
    : BaseKernel(model)
{
  ...
  soil_thermal_cond_    = p->get<RealType>("ACE Sediment Thermal Conductivity", 0.0);
...
ACEpermafrostMiniKernel<EvalT, Traits>::operator()(int cell, int pt) const
{
  ...
  */
  ...
  if (sediment_given == true) {
    ...
    calc_soil_heat_capacity = (0.7e3 * sand_frac) + (0.6e3 * clay_frac) + (0.7e3 * silt_frac) + (1.9e3 * peat_frac);
    // K values in [W/K/m]
    calc_soil_thermal_cond = (8.0 * sand_frac) + (0.4 * clay_frac) + (4.9 * silt_frac) + (0.08 * peat_frac);
    // Rho values in [kg/m3]
    // Peat density from Emily Bristol
    calc_soil_density = ((1.0 - peat_frac) * soil_density_) + (peat_frac * 250.0);
  }
