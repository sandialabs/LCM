...
{
  double sum = std::sqrt(
      input_normal[0] * input_normal[0] + input_normal[1] * input_normal[1] +
...
{
  return std::sqrt(
      (c0[0] - c1[0]) * (c0[0] - c1[0]) + (c0[1] - c1[1]) * (c0[1] - c1[1]) +
      (c0[2] - c1[2]) * (c0[2] - c1[2]));
}
    ...
    const stk::mesh::BulkData& bulkData)
{
  ...
  const stk::mesh::PairIterRelation elem_nodes =
      element.relations(stk::topology::NODE_RANK);
  ...
  for (unsigned inode = 0; inode < num_node - 1; inode++) {
    ...
    double*           coord_data_i =
        stk::percept::PerceptMesh::field_data(coordField, node_i);

    for (unsigned jnode = inode + 1; jnode < num_node; jnode++) {
      stk::mesh::Entity node_j = *elem_nodes[jnode].entity();
      double*           coord_data_j =
          stk::percept::PerceptMesh::field_data(coordField, node_j);
    ...
    const stk::mesh::BulkData& bulkData)
{
  ...
  for (unsigned inode = 0; inode < num_node; inode++) {
    ...
    double*           coord_data =
        stk::percept::PerceptMesh::field_data(coordField, node);

    if (coord_data[0] < 0.0 ||
        coord_data[1] < 0.0) {  // || coord_data[2] > 1.1)
      found = false;
      break;
    }
  }
