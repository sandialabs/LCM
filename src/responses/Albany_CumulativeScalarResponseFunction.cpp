...
void
Albany::CumulativeScalarResponseFunction::evaluateTangent(
    const double                                 alpha,
    const double                                 beta,
    const double                                 omega,
    const double                                 current_time,
...
{
  // zero-out vecotres
  if (!g.is_null()) { g->assign(0); }
  if (!gx.is_null()) { gx->assign(0); }
  if (!gp.is_null()) { gp->assign(0); }
  ...
  for (unsigned int i = 0; i < responses.size(); i++) {
    ...
    if (!gx.is_null()) { gx->update(1.0, *gx_i); }
    if (!gp.is_null()) { gp->update(1.0, *gp_i); }
  }
}
...
{
  if (!g.is_null()) { g->assign(0.0); }
  if (!dg_dx.is_null()) { dg_dx->assign(0.0); }
  ...
  for (unsigned int i = 0; i < responses.size(); i++) {
    ...
    // Copy results into combined result
    if (!g.is_null()) { g->update(1.0, *g_i); }
    if (!dg_dx.is_null()) { dg_dx->update(1.0, *dg_dx_i); }
    if (!dg_dxdot.is_null()) { dg_dxdot->update(1.0, *dg_dxdot_i); }
    if (!dg_dxdotdot.is_null()) { dg_dxdotdot->update(1.0, *dg_dxdotdot_i); }
    if (!dg_dp.is_null()) { dg_dp->update(1.0, *dg_dp_i); }
  }
}

void
Albany::CumulativeScalarResponseFunction::evaluateDistParamDeriv(
    const double                            current_time,
    const Teuchos::RCP<const Thyra_Vector>& x,
    const Teuchos::RCP<const Thyra_Vector>& xdot,
    const Teuchos::RCP<const Thyra_Vector>& xdotdot,
    ...
    const Teuchos::RCP<Thyra_MultiVector>&  dg_dp)
{
  ...
  for (unsigned int i = 0; i < responses.size(); i++) {
    ...
    RCP<Thyra_MultiVector> dg_dp_i =
        Thyra::createMembers(dg_dp->range(), vs_i->dim());

    // Evaluate response function
    responses[i]->evaluateDistParamDeriv(
        current_time, x, xdot, xdotdot, param_array, dist_param_name, dg_dp_i);
