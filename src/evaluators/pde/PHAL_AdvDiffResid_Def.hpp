    ...
    if (formType == 1) {
        ...
        for (std::size_t node = 0; node < numNodes; ++node) {
          ...
          for (std::size_t qp = 0; qp < numQPs; ++qp) {
            ...
            Residual(cell, node, 0) +=
                UDot(cell, qp, 0) * wBF(cell, node, qp) + a * U(cell, qp, 1) * wBF(cell, node, qp) +
            ...
            Residual(cell, node, 1) +=
                U(cell, qp, 1) * wBF(cell, node, qp) - UGrad(cell, qp, 0, 0) * wBF(cell, node, qp);
            Residual(cell, node, 2) +=
                U(cell, qp, 2) * wBF(cell, node, qp) - UGrad(cell, qp, 0, 1) * wBF(cell, node, qp);
          }
        }
      }
    } else if (formType == 2) {
      for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
        for (std::size_t node = 0; node < numNodes; ++node) {
          ...
          for (std::size_t qp = 0; qp < numQPs; ++qp) {
            // du/dt + q = 0
            Residual(cell, node, 0) += UDot(cell, qp, 0) * wBF(cell, node, qp) + U(cell, qp, 1) * wBF(cell, node, qp) +
                                       U(cell, qp, 2) * wBF(cell, node, qp);
            // q - (a,b).grad(u) + mu*delta(u) = 0
            Residual(cell, node, 1) += U(cell, qp, 1) * wBF(cell, node, qp) -
                                       a * UGrad(cell, qp, 0, 0) * wBF(cell, node, qp) -
                                       mu * UGrad(cell, qp, 0, 0) * wGradBF(cell, node, qp, 0);
            Residual(cell, node, 2) += U(cell, qp, 2) * wBF(cell, node, qp) -
                                       b * UGrad(cell, qp, 0, 1) * wBF(cell, node, qp) -
                                       mu * UGrad(cell, qp, 0, 1) * wGradBF(cell, node, qp, 1);
          }
        }
