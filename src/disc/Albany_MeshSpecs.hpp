...
// need to have a mesh. All we need are general topological information about
// the mesh. This includes worksetSize, CellTopologyData, etc.

namespace Albany {

struct MeshSpecsStruct
{
  // Empty initialization. This constructor initialize all entries to empty
  // values, or, if possible, invalid values. This will be used to create mesh
  // specs for sideSets that will have no mesh. Having no mesh allows to specify
  // long parameter lists in the input file. For instance, if all we need is to
  // know topological (and quadrature) information on a sideset, there is no
  // need to create a (potentially heavy) side mesh/discretization. Hence, we
  // instead create mesh specs, with bare minimum information. To facilitate
  // this, we offer an empty constructor, and the routines that build mesh specs
  // for side sets will have to manually modify the specs.
  MeshSpecsStruct();

  // This constructor initializes all the possible informations in a mesh specs
  // object.
  MeshSpecsStruct(
