...
class MyPairIterRelation
{
 ...
 public:
  // AGS: Had to comment this out to get stk_rebalance to compile without all of
  // Percept
  // MyPairIterRelation(PerceptMesh& eMesh, stk::mesh::Entity entity,
  ...
  struct MyRelation
  {
    ...
    inline stk::mesh::Entity
    entity() const
    {
      return m_entity;
    }
    inline stk::mesh::ConnectivityOrdinal
    relation_ordinal() const
    {
      return m_ordinal;
    }
  };
  ...
  MyPairIterRelation&
  operator=(const MyPairIterRelation& mp)
  {
    m_size     = mp.m_size;
