...
struct EBSpecsStruct
{
  ...
  void
  Initialize(GO nnelems[], double blLength[]);

  //! Multiple element block initializer
  void
  Initialize(int i, const Teuchos::RCP<Teuchos::ParameterList>& params);

  //! Query function to determine if a given i, j, k value is in this element
  //! block
...
//    void calcElemSizes(std::vector<std::vector<double> > &h){
  ...
  // function.
  void
  calcElemSizes(std::vector<double> h[]) __attribute__((optnone))
#else
  void
  calcElemSizes(std::vector<double> h[])
#endif
  {
...
class TmplSTKMeshStruct : public GenericSTKMeshStruct
{
 public:
  ...
  //! Default and optional element types created by the class (only meaningful
  //! in 2D - quads and tris)
