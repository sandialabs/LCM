...
static void
switchToOriginals(int npartitions)
{
...
static apf::Migration*
getPlan(apf::Mesh* m, int npartitions)
{
  apf::Splitter* splitter =
      apf::makeZoltanSplitter(m, apf::GRAPH, apf::PARTITION, false);
    ...
    const Teuchos::RCP<const Teuchos_Comm>&     commT)
{
  ...
  outputInterval = params->get<int>(
      "PUMI Write Interval", 1);  // write every time step default
  ...
  if (params->isParameter("PUMI Input File Name")) {
    ...
    // If we are running in parallel but have a single mesh file, split it and
    // rebalance
    bool useSerialMesh = params->get<bool>("Use Serial Mesh", false);
    if (useSerialMesh &&
        commT->getSize() >
            1) {  // do the equivalent of the SCOREC "split" utility
  ...
  if (isQuadMesh) apf::changeMeshShape(mesh, apf::getSerendipity(), false);
  ...
  if (params->isParameter("Resize Input Mesh Element Size")) {
    SizeFunction sizeFunction(
        params->get<double>("Resize Input Mesh Element Size", 0.1));
  ...
  if (params->isParameter("Load LandIce Data")) shouldLoadLandIceData = true;
}
...
void
Albany::PUMIMeshStruct::setMesh(apf::Mesh2* new_mesh)
{
  if (mesh) {
    mesh->destroyNative();
...
Albany::PUMIMeshStruct::getValidDiscretizationParameters() const
{
  ...
  validPL->set<int>(
      "PUMI Write Interval",
      3,
      "Step interval to write solution data to output file");
  validPL->set<bool>(
      "2nd Order Mesh",
      false,
      "Flag to indicate 2nd order Lagrange shape functions");
  validPL->set<int>(
  ...
  validPL->set<std::string>(
      "PUMI Input File Name", "", "File Name For PUMI Mesh Input");
  validPL->set<std::string>(
      "PUMI Output File Name", "", "File Name For PUMI Mesh Output");
  validPL->set<std::string>(
      "Mesh Model Input File Name", "", "meshmodel geometry file");

  validPL->set<int>(
      "Write Restart File at Step",
      0,
      "Continuation step to write restart files");
  validPL->set<double>(
      "PUMI Restart Time", 0, "Simulation time to restart from");
  validPL->set<bool>(
      "Load LandIce Data",
      false,
      "Load fields required for LandIce FO problem");
  ...
  validPL->set<double>(
      "Resize Input Mesh Element Size",
      1.0,
      "Resize mesh element to this size at input");
  ...
  validPL->set<bool>(
      "Save Stabilized Stress", false, "Save stabilized stress to file");
