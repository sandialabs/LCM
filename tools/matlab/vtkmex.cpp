...
class ArrayList
{
 public:
  ...
  void
  push_back(const Int* a)
  {
    v_.insert(v_.end(), a, a + n_);
  }
  T*
  operator()(Int i)
  {
...
class Signature
{
 ...
 public:
  ...
  bool
  operator<(const Signature& ts) const
  {
    for (Int i = 0; i < _n; ++i) {
      if (ps_[i] < ts.ps_[i])
        return true;
      else if (ps_[i] > ts.ps_[i])
        return false;
    }
    return false;
  }
...
inline std::vector<EdgeSignature>
get_all_edgesigs(const Tris& tris)
{
  ...
  for (size_t itri = 0, k = 0; itri < tris.size(); ++itri) {
    ...
    for (Int i = 0; i < 3; ++i, ++k) {
      Int edge[2];
      for (Int j = 0; j < 2; ++j) edge[j] = tri[idxs[i][j]];
...
get_all_trisigs(const Tets& tets)
{
  ...
  for (size_t itet = 0, k = 0; itet < tets.size(); ++itet) {
    ...
    for (Int i = 0; i < 4; ++i, ++k) {
      Int tri[3];
      for (Int j = 0; j < 3; ++j) tri[j] = tet[idxs[i][j]];
...
Edges
get_unique_edges(const IntArrayList& polys, const IntArrayList& pis)
{
...
template <Int D>
struct Point : public Dimensional<D>
{
  Real d[D];
  Point(const Real* data = 0)
  {
    if (data)
      for (Int i = 0; i < D; ++i) d[i] = data[i];
  }
};
template <Int D>
struct Vector : public Point<D>
{
  Vector(const Real* data = 0) : Point<D>(data) {}
};
template <Int D>
struct LineSegment : public Dimensional<D>
{
  Point<D> p[2];
  LineSegment(const Point<D>& p1, const Point<D>& p2);
};
template <Int D>
struct Tri : public Dimensional<D>
{
  Point<D> p[3];
...
template <Int D>
struct Plane : public Dimensional<D>
{
  Vector<D> n;
  Real      c;
};
...
template <Int D>
inline void
scale(Real x[D], const Real a)
{
  for (Int i = 0; i < D; ++i) x[i] *= a;
}
template <Int D>
inline void
scale(Point<D>& x, const Real a)
{
  scale<D>(x.d, a);
}
...
template <Int D>
inline Real
dot(const Real x[D], const Real y[D])
{
  Real a = 0;
...
template <Int D>
inline Real
dot(const Point<D>& x, const Point<D>& y)
{
...
template <Int D>
inline Real
norm2(const Real x[D])
{
  return dot<D>(x, x);
}
template <Int D>
inline Real
norm2(const Point<D>& v)
{
...
// c = a x b
inline void
cross(const Real a[3], const Real b[3], Real c[3])
{
  c[0] = a[1] * b[2] - a[2] * b[1];
  c[1] = a[2] * b[0] - a[0] * b[2];
  c[2] = a[0] * b[1] - a[1] * b[0];
...
inline void
cross(const Point<3>& a, const Point<3>& b, Point<3>& c)
{
  cross(a.d, b.d, c.d);
}
...
inline void
init_Point(Point<3>& p, const Real a, const Real b, const Real c)
{
  p.d[0] = a;
  p.d[1] = b;
  p.d[2] = c;
...
template <Int D>
inline void
init_Point(Point<D>& p, const Real* d)
{
  for (Int i = 0; i < D; ++i) p.d[i] = d[i];
}

template <Int D>
inline LineSegment<D>::LineSegment(const Point<D>& p1, const Point<D>& p2)
{
  p[0] = p1;
  p[1] = p2;
}
...
template <Int D>
inline Tri<D>::Tri(const Point<D>& p1, const Point<D>& p2, const Point<D>& p3)
{
  p[0] = p1;
  p[1] = p2;
  p[2] = p3;
...
template <Int D>
inline Real
dist2(const Point<D>& x, const Point<D>& y)
{
  Real a = 0;
  for (Int i = 0; i < D; ++i) {
...
Real
dist2(const Point<3>& p, const LineSegment<3>& ls)
{
  // Solution to
  ...
  if (alpha_star <= 0)
    return dist2(p, ls.p[0]);
  else if (alpha_star >= 1)
    return dist2(p, ls.p[1]);
  else {
...
inline Real
dist2(const LineSegment<3>& ls, const Point<3>& p)
{
  return dist2(p, ls);
}

Real
signed_dist(const Point<3>& p, const Plane<3>& plane)
{
  return dot(plane.n, p) - plane.c;
}
inline Real
signed_dist(const Plane<3>& plane, const Point<3>& p)
{
...
class TriWithData<3>
{
 ...
 public:
  ...
  bool
  is_projected_in(const Point<3>& p) const
  {
    Vector<3> v;
...
inline Real
signed_dist(const Point<3>& p, const TriWithData<3>& t)
{
...
inline Real
signed_dist(const TriWithData<3>& t, const Point<3>& p)
{
...
mexFunction(int nlhs, mxArray** plhs, int nrhs, const mxArray** prhs)
{
  ...
  if (cmd == "get_skin") {
    if (nlhs != 1 || nrhs != 1) mexErrMsgTxt("skin_tris = get_skin(tets)");
    ...
    if (nlhs != 1 || nrhs != 2) mexErrMsgTxt("lc = get_unique_edges(c, is)");
    const mexutil::ConstDenseMexMat mc(prhs[0]), mis(prhs[1]);
    reqorexit(mis.m == 2);
    IntArrayList polys(mc.n);
    convert(mc, polys);
    IntArrayList pis(mis.n);
    convert(mis, pis);
    ...
    convert(mps, ps);
    Tris tris(mtris.m);
    convert(mtris, tris, -1);
    Vertices verts(3);
    convert(mverts, verts);
