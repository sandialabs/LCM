...
void
only_keep_connectivity_to_specified_ranks(stk::mesh::BulkData& mesh, stk::mesh::Entity entity, stk::mesh::EntityRank keeper1, stk::mesh::EntityRank keeper2)
{
    ...
    int const                                                 worksetSize)
{
  ...
  if (num_time_deriv >= 1) {
    solution_vector[1] = params->get<Teuchos::Array<std::string>>("SolutionDot Vector Components", default_solution_vector);
    if (solution_vector[1].length() > 0) user_specified_solution_components = true;
  }

  if (num_time_deriv >= 2) {
    solution_vector[2] = params->get<Teuchos::Array<std::string>>("SolutionDotDot Vector Components", default_solution_vector);
  ...
  Teuchos::Array<std::string> residual_vector = params->get<Teuchos::Array<std::string>>("Residual Vector Components", default_residual_vector);
  ...
  if (user_specified_solution_components && (residual_vector.length() > 0)) {
    if (interleavedOrdering)
      this->fieldContainer =
          Teuchos::rcp(new MultiSTKFieldContainer<true>(params, metaData, bulkData, neq, req, numDim, sis, solution_vector, residual_vector));
    else
      this->fieldContainer =
          Teuchos::rcp(new MultiSTKFieldContainer<false>(params, metaData, bulkData, neq, req, numDim, sis, solution_vector, residual_vector));

  }

  else {
    if (interleavedOrdering)
      this->fieldContainer = Teuchos::rcp(new OrdinarySTKFieldContainer<true>(params, metaData, bulkData, neq, req, numDim, sis));
    else
      this->fieldContainer = Teuchos::rcp(new OrdinarySTKFieldContainer<false>(params, metaData, bulkData, neq, req, numDim, sis));
  }
  ...
  if (!eMesh.is_null()) {
    if (buildUniformRefiner())  // cant currently build both types of refiners
                                // (FIXME)
...
bool
GenericSTKMeshStruct::buildPerceptEMesh()
{
  ...
  // Or, if a percept mesh is needed to support general adaptation indicated in
  // the "Adaptation" sublist
  if (!adaptParams.is_null()) {
    std::string& method = adaptParams->get("Method", "");

    if (method == "Unif Size") return true;
  }
...
GenericSTKMeshStruct::buildUniformRefiner()
{
  ...
  if (convert.length()) checkInput("convert", convert, convert_options);
...
GenericSTKMeshStruct::buildLocalRefiner()
{
  ...
  if (pattern == "Local_Tet4_Tet4_N") {
    //      refinerPattern = Teuchos::rcp(new
    //      stk::adapt::Local_Tet4_Tet4_N(*eMesh, block_names));
...
void
GenericSTKMeshStruct::cullSubsetParts(std::vector<std::string>& ssNames, std::map<std::string, stk::mesh::Part*>& partVec)
{
  /*
  When dealing with sideset lists, it is common to have parts that are subsets
  of other parts, like: Part[ surface_12 , 18 ] { Supersets { {UNIVERSAL} }
    Intersection_Of { } }
...
void
GenericSTKMeshStruct::computeAddlConnectivity()
{
...
void
GenericSTKMeshStruct::setDefaultCoordinates3d()
{
  // If the mesh is already a 3d mesh, coordinates_field==coordinates_field3d
  if (this->numDim == 3) return;

  // We make coordinates_field3d store the same coordinates as
  // coordinates_field, padding the vector of coordinates with zeros
...
void
GenericSTKMeshStruct::rebalanceAdaptedMeshT(const Teuchos::RCP<Teuchos::ParameterList>& params_, Teuchos::RCP<Teuchos::Comm<int> const> const& comm)
{
// Zoltan is required here
  ...
  if (comm->getRank() == 0) {
    ...
    std::cout << "Before rebal " << comm->getRank() << "  " << stk::mesh::count_selected_entities(owned_selector, bulkData->buckets(stk::topology::NODE_RANK))
              << endl;
  }
...
GenericSTKMeshStruct::addNodeSetsFromSideSets()
{
  ...
  for (const auto& ssn_part_pair : ssPartVec) {
    // If a nodeset with the same name already exists, we ASSUME it contains
    // this sideset's nodes.
    auto itns = nsPartVec.find(ssn_part_pair.first);
    if (itns != nsPartVec.end()) return;
...
void
GenericSTKMeshStruct::checkNodeSetsFromSideSetsIntegrity()
{
  ...
  for (auto ssn : m_nodesets_from_sidesets) {
    ...
    ALBANY_PANIC(it == ssPartVec.end(), "Error! Side set " << ssn << " not found. This error should NEVER occurr though. Bug?\n");
...
GenericSTKMeshStruct::initializeSideSetMeshSpecs(const Teuchos::RCP<Teuchos_Comm const>& comm)
{
  ...
  for (auto ms : this->getMeshSpecs()) {
    ...
    for (auto ssName : ms->ssNames) {
      ...
      ALBANY_PANIC(part == nullptr, "Error! One of the stored meshSpecs claims to have sideset " + ssName + " which, however, is not a part of the mesh.\n");
      ...
      // At this point, we cannot assume there will be a discretization on this
      // side set, so we use cubature degree=-1, and the workset size of this
      // mesh. If the user *does* add a discretization (in the Side Set
      // Discretizations sublist), he/she can specify cubature and workset size
      // there. The method initializeSideSetMeshStructs will overwrite this mesh
      // specs anyways. Note: it *may* be that the user need no cubature on this
      // side (only node/cell fields).
      //       But if the user *does* nned cubature, we want to set a *very
      //       wrong* number, so that the code will crash somewhere, and he/sh
      //       can realize he/she needs to set cubature info somewhere

      // We allow a null ctd here, and we simply do not store the side mesh
      // specs. The reason is that we _may_ be loading a mesh that stores some
      // empty side sets. If we are using the sideset, we will probably run into
...
GenericSTKMeshStruct::initializeSideSetMeshStructs(const Teuchos::RCP<Teuchos_Comm const>& comm)
{
  if (params->isSublist("Side Set Discretizations")) {
    ...
    for (int i(0); i < sideSets.size(); ++i) {
      ...
      if (!params_ss->isParameter("Number Of Time Derivatives")) params_ss->set<int>("Number Of Time Derivatives", num_time_deriv);
      ...
      if (method == "SideSetSTK") {
        ...
        if (this->sideSetMeshStructs.find(ss_name) == this->sideSetMeshStructs.end()) {
          ...
          ALBANY_PANIC(this->sideSetMeshStructs[ss_name] == Teuchos::null, "Error! Could not cast side mesh to AbstractSTKMeshStruct.\n");
        }
      }
      ...
      // name)
      ALBANY_PANIC(sideDim != this->sideSetMeshStructs[ss_name]->numDim, "Error! Mesh on side " << ss_name << " has the wrong dimension.\n");
      ...
      // mesh, in the case it was constructed as side mesh of an extruded mesh
      // with a different ordering and/or different number of layers. Notice
    ...
    int                                                                              worksetSize)
{
  if (this->sideSetMeshStructs.size() > 0) {
    ...
    for (auto it : sideSetMeshStructs) {
      ...
      // We check since the basal mesh for extruded stk mesh should already have
      // it set
      if (!it.second->fieldAndBulkDataSet) {
        ...
        it.second->setFieldAndBulkData(
            ...
            req,
            sis,
            worksetSize);  // Cell equations are also defined on the side, but
                           // not viceversa
      }
    }
  }
...
void
GenericSTKMeshStruct::buildCellSideNodeNumerationMap(std::string const& sideSetName, std::map<GO, GO>& sideMap, std::map<GO, std::vector<int>>& sideNodeMap)
{
  ALBANY_PANIC(
  ...
  //         maps map 3D values to 2D ones. For instance, if node 0 of side3D 41
  //         is mapped to node 2 of cell2D 12, the stk field values will be
  //         field(12) = [*, *, 0], while the input map will be sideNodeMap[41]
  //         = [2, *, *] This is because in Load/SaveSideSetStateField we can
  //         only use 3D data to access the map. On the other hand, the stk
  //         fields are defined in 2D, so it makes sense to have them indicized
  //         on the 2D values.
  // NOTE 2: The stk side_map maps a 2D cell id to a pair <cell3D_GID,
  // side_lid>, where side_lid is the lid of the side
  //         within the element. The input map, instead, maps the directly
  ...
  if (side_mesh->side_maps_present && !side_mesh->ignore_side_maps) {
    ...
    // WARNING: the maps may be not be valid. This can happen if they were built
    // with an
    //          extruded mesh with N layers, and we are now using this side mesh
    //          to build an extruded mesh with M!=N layers, or with different
    //          ordering (COLUMN/LAYER). If this is the case, you must first
    //          edit the exodus file and delete the maps or set 'Ignore Side
...
void
GenericSTKMeshStruct::loadRequiredInputFields(const AbstractFieldContainer::FieldContainerRequirements& req, const Teuchos::RCP<Teuchos_Comm const>& comm)
{
  Teuchos::RCP<Teuchos::FancyOStream> out = Teuchos::fancyOStream(Teuchos::rcpFromRef(std::cout));
  out->setProcRankAndSize(comm->getRank(), comm->getSize());
  out->setOutputToRootOnly(0);
  ...
  *out << "[GenericSTKMeshStruct] Processing field requirements...\n";
  ...
  // Check whether we need the serial map or not. The only scenario where we DO
  // need it is if we are loading a field from an ASCII file. So let's check the
  // fields info to see if that's the case.
  ...
  // NOTE: the serial vs cannot be created linearly, with GIDs from 0 to
  // numGlobalNodes/Elems, since
  //       this may be a boundary mesh, and the GIDs may not start from 0, nor
  //       be contiguous. Therefore, we must create a root vs. Moreover, we need
  //       the GIDs sorted (so that, regardless of the GID, we read the serial
  //       input files in the correct order), and we can't sort them once the vs
  //       is created.
  ...
  for (int ifield = 0; ifield < num_fields; ++ifield) {
    ...
    // Check if it's an output file (nothing to be done then). If not, check
    // that the usage is a valid string
    if (fusage == "Output") {
      *out << "  - Skipping field '" << fname
           << "' since it's listed as output. Make sure there's an evaluator "
              "set to save it!\n";
      continue;
    } else {
      ALBANY_PANIC(
    ...
    if (forigin == "Mesh") {
      *out << "  - Skipping field '" << fname << "' since it's listed as present in the mesh.\n";
      ...
      ALBANY_PANIC(forigin != "File", "Error! 'Field Origin' for field '" << fname << "' must be one of 'File' or 'Mesh'.\n");
    }

    // The field is not already present (with updated values) in the mesh, and
    // must be loaded/computed filled here.
    ...
    ALBANY_PANIC(
        ...
        "(or 'Random Value') for loading a field.\n");

    // Depending on the input field type, we need to use different
    // pointers/importers/vectors
      ...
      ALBANY_ABORT(
          "Error! Field '" << fname << "' has type '" << ftype << "'.\n"
                           << "Unfortunately, the only supported field types "
                              ...
                              "'Node/Elem Layered Scalar/Vector'.\n");
    }
    ...
    const Teuchos::RCP<Teuchos::FancyOStream>    out)
{
  ...
  if (field_params.isParameter("Random Value")) {
    *out << "  - Filling " << field_type << " field '" << field_name << "' with random values.\n";
    ...
    // If there is no such parameter, we fill the non random entries with
    // zeroes.
    ...
    const Teuchos::RCP<Teuchos::FancyOStream>    out)
{
  ALBANY_ABORT(
      ...
      "an expression evaluation package is not used.\n");
}
    ...
    const Teuchos::RCP<Teuchos_Comm const>&      comm) const
{
  ...
  if (comm->getRank() == 0) {
    ...
    for (int icomp = 0; icomp < numComponents; ++icomp) {
      ...
      for (GO i = 0; i < numNodes; ++i) ifile >> comp_view[i];
    }
    ifile.close();
  }
}
    ...
    const Teuchos::RCP<Teuchos_Comm const>&      comm) const
{
  ...
  if (comm->getRank() == 0) {
    ...
    // ordering) we want to keep the layer dimension last. Ex: a 2D field f(i,j)
    // would be stored at the raw array position i*num_cols+j. In our case,
    //     num_cols is the number of layers, and num_rows the number of field
    //     components
    for (int il = 0; il < numLayers; ++il) {
      for (int icomp(0); icomp < numComponents; ++icomp) {
...
GenericSTKMeshStruct::getValidGenericSTKParameters(std::string listname) const
{
  ...
  validPL->set<std::string>("Exodus Output File Name", "", "Request exodus output to given file name. Requires SEACAS build");
  validPL->set<std::string>(
      "Exodus Solution Name",
      "",
      "Name of solution output vector written to Exodus file. Requires SEACAS "
      "build");
  validPL->set<std::string>(
  ...
  validPL->set<std::string>("Exodus Solution DTK Name", "", "Name of solution dtk written to Exodus file. Requires SEACAS build");
  validPL->set<std::string>("Exodus SolutionDot DTK Name", "", "Name of solution_dot dtk written to Exodus file. Requires SEACAS build");
  validPL->set<std::string>(
      "Exodus SolutionDotDot DTK Name",
      "",
      "Name of solution_dotdot dtk written to Exodus file. Requires SEACAS "
      "build");
#endif
  ...
  validPL->set<std::string>("NetCDF Output File Name", "", "Request NetCDF output to given file name. Requires SEACAS build");
  ...
  validPL->set<int>(
      ...
      1,
      "Number of samples in Latitude direction for NetCDF output. Default is "
      "100.");
  validPL->set<int>(
      "NetCDF Output Number of Longitudes",
      1,
  ...
  validPL->set<bool>("Separate Evaluators by Element Block", false, "Flag for different evaluation trees for each Element Block");
  validPL->set<std::string>(
      "Transform Type",
      "None",
  ...
  validPL->set<bool>(
      ...
      false,
      "Writing Coordinates to MatrixMarket File");  // for writing coordinates
                                                    // to matrix market file
  validPL->set<double>(
      "LandIce alpha",
      0.0,
      "Surface boundary inclination for LandIce problems (in degrees)");  // for
                                                                          // LandIce
                                                                          // problem
                                                                          // that
                                                                          // require
  ...
  validPL->set<double>(
      "LandIce L",
      1,
      "Domain length for LandIce problems");  // for LandIce problem that
  ...
  validPL->set<bool>(
      "Contiguous IDs",
      "true",
      "Tells Ascii mesh reader is mesh has contiguous global IDs on 1 "
      "processor.");  // for LandIce problem that require tranformation of STK
                      // mesh

  Teuchos::Array<std::string> defaultFields;
  validPL->set<Teuchos::Array<std::string>>("Restart Fields", defaultFields, "Fields to pick up from the restart file when restarting");
  validPL->set<Teuchos::Array<std::string>>(
      "Solution Vector Components",
      defaultFields,
      "Names and layout of solution output vector written to Exodus file. "
      "Requires SEACAS build");
  validPL->set<Teuchos::Array<std::string>>(
      "SolutionDot Vector Components",
      defaultFields,
      "Names and layout of solution_dot output vector written to Exodus file. "
      "Requires SEACAS build");
  validPL->set<Teuchos::Array<std::string>>(
      "SolutionDotDot Vector Components",
      defaultFields,
      "Names and layout of solution_dotdot output vector written to Exodus "
      "file. Requires SEACAS build");
  validPL->set<Teuchos::Array<std::string>>(
