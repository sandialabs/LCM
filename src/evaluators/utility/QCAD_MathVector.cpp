...
QCAD::mathVector::mathVector() { dim_ = -1; }

QCAD::mathVector::mathVector(int n) : data_(n) { dim_ = n; }
...
{
  double d = 0;
  for (int i = 0; i < dim_; i++) d += data_[i] * v2[i];
  return d;
}
...
double& QCAD::mathVector::operator[](int i) { return data_[i]; }

const double& QCAD::mathVector::operator[](int i) const { return data_[i]; }
...
std::ostream&
QCAD::operator<<(std::ostream& os, const QCAD::mathVector& mv)
{
