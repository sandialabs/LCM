      ...
      matPropType(SCALAR_CONSTANT)
{
  ...
  Teuchos::ParameterList* mp_list = p.get<Teuchos::ParameterList*>("Parameter List");
  ...
  if (type == "Constant") {
    if (rank == 2) {
      ...
      Teuchos::Array<double> tmp     = mp_list->get<Teuchos::Array<double>>("Value");
...
template <typename EvalT, typename Traits>
void
NSMaterialProperty<EvalT, Traits>::postRegistrationSetup(typename Traits::SetupData d, PHX::FieldManager<Traits>& fm)
{
  this->utils.setFieldData(matprop, fm);
  if (matPropType == SQRT_TEMP || matPropType == INV_SQRT_TEMP) this->utils.setFieldData(T, fm);
...
template <typename EvalT, typename Traits>
void
NSMaterialProperty<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  if (matPropType == SCALAR_CONSTANT) {
    ...
    for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
      for (std::size_t qp = 0; qp < dims[1]; ++qp) {
        matprop(cell, qp) = 1.0 / (3.0 * (1.0 - mu(cell, qp)) * (sigma_a(cell, qp) + sigma_s(cell, qp)));
      }
    }
    ...
    if (index == 0)
      ...
      slope                 = ((depValues[index] - depValues[index - 1]) / (timeValues[index] - timeValues[index - 1]));
