...
template <typename EvalT, typename Traits>
void
ComprNSResid<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
      ...
      for (std::size_t node = 0; node < numNodes; ++node) {
        ...
        for (std::size_t qp = 0; qp < numQPs; ++qp) {
          ...
          Residual(cell, node, 1) += qFluct(cell, qp, 0) *
                                         (qFluct(cell, qp, 1) * qFluctGrad(cell, qp, 1, 0) + qFluct(cell, qp, 2) * qFluctGrad(cell, qp, 1, 1)) *
                                         wBF(cell, node, qp)  // rho*(u*du/dx + v*du/dy)
                                     + Rgas * (qFluct(cell, qp, 0) * qFluctGrad(cell, qp, 3, 0) + qFluct(cell, qp, 3) * qFluctGrad(cell, qp, 0, 0)) *
          ...
          Residual(cell, node, 2) += qFluct(cell, qp, 0) *
                                         (qFluct(cell, qp, 1) * qFluctGrad(cell, qp, 2, 0) + qFluct(cell, qp, 2) * qFluctGrad(cell, qp, 2, 1)) *
                                         wBF(cell, node, qp)  // rho*(u*dv/dx + v*dv/dy)
                                     + Rgas * (qFluct(cell, qp, 0) * qFluctGrad(cell, qp, 3, 1) + qFluct(cell, qp, 3) * qFluctGrad(cell, qp, 0, 1)) *
          ...
          Residual(cell, node, 3) +=
              qFluct(cell, qp, 0) *
                  (qFluct(cell, qp, 1) * qFluctGrad(cell, qp, 3, 0) + qFluct(cell, qp, 2) * qFluctGrad(cell, qp, 3, 1)  // rho*(u*dT/dx + v*dT/dy)
                   + (gamma_gas - 1.0) * qFluct(cell, qp, 3) * (qFluctGrad(cell, qp, 1, 0) + qFluctGrad(cell, qp, 2, 1))) *
