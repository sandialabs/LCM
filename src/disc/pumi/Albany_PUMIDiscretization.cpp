...
Albany::PUMIDiscretization::setLandIceData()
{
  ...
  for (std::size_t i = 0; i < meshStruct->elemnodescalar_states.size(); ++i) {
    ...
    if (!f) {
      if (!PCU_Comm_Self())
        std::cout << "PUMIDisc: initializing " << state.name << " to zero"
                  << std::endl;
      ...
      for (std::size_t b = 0; b < buckets.size(); ++b) {
        ...
        for (std::size_t e = 0; e < buck.size(); ++e)
          for (std::size_t n = 0; n < state.dims[1]; ++n) ar(e, n) = 0.0;
      }
    ...
    /* otherwise we pull in the data from the mesh */
      ...
      if (!PCU_Comm_Self())
        std::cout << "PUMIDisc: initializing " << state.name << " from mesh"
                  << std::endl;
      ...
      for (std::size_t b = 0; b < buckets.size(); ++b) {
        ...
        for (std::size_t e = 0; e < buck.size(); ++e) {
          ...
          for (std::size_t n = 0; n < num_nodes; ++n) { ar(e, n) = values[n]; }
