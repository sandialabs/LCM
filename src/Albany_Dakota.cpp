...
Albany_Dakota(int argc, char* argv[])
{
  ...
  *out << "\nStarting Albany_Dakota!" << endl;
  ...
  if (analysis_comm == Albany_MPI_COMM_NULL) { return 0; }

  RCP<const Teuchos_Comm> appComm =
      Albany::createTeuchosCommFromMpiComm(analysis_comm);

  RCP<Albany::SolverFactory> slvrfctry =
      rcp(new Albany::SolverFactory(cmd.yaml_filename, appComm));
  ...
  if (cmd.vtune) { Albany::connect_vtune(appComm->getRank()); }
  ...
  RCP<Thyra::ResponseOnlyModelEvaluatorBase<ST>> app =
      slvrfctry->create(appComm, appComm);
  trikota_interface =
      rcp(new TriKota::ThyraDirectApplicInterface(
              dakota.getProblemDescDB(), app, p_index, g_index),
          false);
