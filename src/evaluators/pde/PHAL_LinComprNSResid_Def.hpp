      ...
      Residual(p.get<std::string>("Residual Name"), p.get<Teuchos::RCP<PHX::DataLayout>>("Node Vector Data Layout"))
{
  ...
  if (IBP_convect_terms == true) std::cout << "Integrating convective terms by parts in weak form." << std::endl;
...
template <typename EvalT, typename Traits>
void
LinComprNSResid<EvalT, Traits>::postRegistrationSetup(typename Traits::SetupData d, PHX::FieldManager<Traits>& fm)
{
...
template <typename EvalT, typename Traits>
void
LinComprNSResid<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  if (eqn_type == EULER) {  // Euler equations
    if (numDims == 1) {     // 1D case
      ...
      if (IBP_convect_terms == false) {  // variational formulation in which the convective terms are
          ...
          if (enableTransient)
            for (std::size_t qp = 0; qp < numQPs; ++qp) {
              for (std::size_t i = 0; i < vecDim; i++) {
                ...
                if (i == vecDim - 1) Residual(cell, node, i) = qFluctDot(cell, qp, i) * wBF(cell, node, qp);
              }
            }
          for (std::size_t qp = 0; qp < numQPs; ++qp) {
