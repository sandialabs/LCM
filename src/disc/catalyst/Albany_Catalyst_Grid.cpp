...
vtkStandardNewMacro(Grid) vtkStandardNewMacro(GridImplementation)
...
void
GridImplementation::PrintSelf(ostream& os, vtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

bool
GridImplementation::SetDecorator(Decorator* decorator)
{
  ...
  for (size_t wsId = 0; wsId < ebNames.size(); ++wsId) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        this->CellTypeLookup[wsId] == VTK_EMPTY_CELL,
        std::runtime_error,
...
vtkIdType
GridImplementation::GetNumberOfCells()
{
  return static_cast<vtkIdType>(this->ElementsWsLid->size());
}

int
GridImplementation::GetCellType(vtkIdType cellId)
{
  int ws     = -1;
  int unused = -1;
...
void
GridImplementation::GetCellPoints(vtkIdType cellId, vtkIdList* ptIds)
{
  int ws  = -1;
  int lid = -1;
...
struct WsLidMatcher
{
  ...
  bool
  operator()(const WsLIDList::value_type& val) const
  {
...
void
GridImplementation::GetPointCells(vtkIdType ptId, vtkIdList* cellIds)
{
    ...
    for (lid = 0; lid < wsLookup.dimension_0(); ++lid) {
      for (int node = 0; node < wsLookup.dimension_1(); ++node) {
        if (static_cast<vtkIdType>(
                wsLookup(lid, node, 0) / this->DegreesOfFreedom) == ptId) {
...
void
GridImplementation::GetIdsOfCellsOfType(int type, vtkIdTypeArray* array)
{
...
int
GridImplementation::IsHomogeneous()
{
...
void
GridImplementation::Allocate(vtkIdType, int)
{
  TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, "Read-only container.");
}

vtkIdType
GridImplementation::InsertNextCell(int, vtkIdList*)
{
  TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, "Read-only container.");
  return -1;
}
...
vtkIdType
GridImplementation::InsertNextCell(int, vtkIdType, vtkIdType*)
{
  TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, "Read-only container.");
  return -1;
}
...
vtkIdType
GridImplementation::InsertNextCell(
    int,
    vtkIdType,
    vtkIdType*,
    vtkIdType,
...
void
GridImplementation::ReplaceCell(vtkIdType cellId, int npts, vtkIdType* pts)
{
  TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, "Read-only container.");
}

inline VTKCellType
GridImplementation::TopologyToCellType(const CellTopologyData* ctd)
{
  ...
  switch (ctd->key) {
    ...
    case shards::Particle::key: return VTK_VERTEX;
    case shards::Line<2>::key: return VTK_LINE;
    case shards::Triangle<3>::key: return VTK_TRIANGLE;
    case shards::Quadrilateral<4>::key: return VTK_QUAD;
    case shards::Tetrahedron<4>::key: return VTK_TETRA;
    case shards::Hexahedron<8>::key: return VTK_HEXAHEDRON;
    case shards::Wedge<6>::key: return VTK_WEDGE;
    case shards::Pyramid<5>::key: return VTK_PYRAMID;
  }
}

inline vtkIdType
GridImplementation::GetCellSize(VTKCellType cellType)
{
  switch (cellType) {
...
inline void
GridImplementation::GetWorksetFromCellId(vtkIdType cellId, int& ws, int& lid)
{
...
VTKCellType
GridImplementation::GetCellTypeFromWorkset(int ws)
{
  return (ws >= 0 && ws < this->CellTypeLookup.size()) ?
             this->CellTypeLookup[ws] :
             VTK_EMPTY_CELL;
}
