    ...
    bool const                                                             hasRestartSolution)
{
  ...
  if (name == "EBPerturb" || name == "EBPerturbGaussian" || name == "EBConstant") {
    ...
    /* The element block-based IC specification here is currently a hack. It
     * assumes the initial value is constant within each element across the
     * element block (or optionally perturbed somewhat element by element). The
     * proper way to do this would be to project the element integration point
     * values to the nodes using the basis functions and a consistent mass
     * matrix.
     *
     * The current implementation uses a single integration point per element -
     * this integration point value for this element within the element block is
     * specified in the input file (and optionally perturbed). An approximation
     * of the load vector is obtained by accumulating the resulting (possibly
     * perturbed) value into the nodes. Then, a lumped version of the mass
     * matrix is inverted and used to solve for the approximate nodal point
     * initial conditions.
     */

    // Use a Tpetra_Vector to hold the lumped mass matrix (has entries only on
    // the diagonal). Zero-ed out.
  ...
  if (name == "Coordinates") {
    // Place the coordinate locations of the nodes into the solution vector for
    // an initial guess
