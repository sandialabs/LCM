...
ResponseSaveField<EvalT, Traits>::ResponseSaveField(
    ...
    : weights("Weights", dl->qp_scalar)
{
  ...
  Teuchos::RCP<Adapt::NodalDataBase> ndb = pStateMgr->getNodalDataBase();
  ...
  if (fieldIndices == "Cell,QuadPt") {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        std::logic_error,
        "Invalid value of the 'Field Indices' parameter: "
            << fieldIndices
  ...
  if (fieldIndices ==
      "Cell,QuadPt") {  // register a cell,qp state => cell-valued quantity
    if (outputCellAverage) {
      pStateMgr->registerStateVariable(
          stateName, cell_dl, "ALL", "scalar", 0.0, false, outputToExodus);
    } else {
      pStateMgr->registerStateVariable(
          stateName, scalar_dl, "ALL", "scalar", 0.0, false, outputToExodus);
    }
  } else if (fieldIndices == "Cell,Node") {
    TEUCHOS_TEST_FOR_EXCEPTION(
        isVectorField,
        std::logic_error,
        "Vector-valued Cell,Node fields are not supported yet");
    pStateMgr->registerNodalVectorStateVariable(
        stateName,
        dl->node_node_scalar,
        dl->dummy,
        "all",
  ...
  // Create field tag
  response_field_tag = Teuchos::rcp(
      new PHX::Tag<ScalarT>(fieldName + " Save Field Response", dl->dummy));
...
template <typename EvalT, typename Traits>
void
ResponseSaveField<EvalT, Traits>::postRegistrationSetup(
...
// **********************************************************************
template <typename EvalT, typename Traits>
void ResponseSaveField<EvalT, Traits>::preEvaluate(
...
template <typename EvalT, typename Traits>
void
ResponseSaveField<EvalT, Traits>::evaluateFields(
    typename Traits::EvalData workset)
{
  ...
  if (fieldIndices == "Cell,QuadPt") {
    ...
    if (!isVectorField) {
      switch (size) {  // Note: size should always == 2 now: qp_scalar type or
          ...
          for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
            if (outputCellAverage) {
              ...
              for (std::size_t qp = 0; qp < numQPs; ++qp) {
                integral +=
                    ADValue(field(cell, qp)) * ADValue(weights(cell, qp));
        ...
        default:
          TEUCHOS_TEST_FOR_EXCEPTION(
              true,
              std::logic_error,
              "Unexpected dimensions in SaveField response Evaluator: "
                  << size);
      }
    } else {
      ScalarT t;
          ...
          for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
            ...
            for (std::size_t qp = 0; qp < numQPs; ++qp) {
              ...
              if (vectorOp == "magnitude") {
                ...
                if (numDims > iX)
                  t += field(cell, qp, iX) * field(cell, qp, iX);
                if (numDims > iY)
                  t += field(cell, qp, iY) * field(cell, qp, iY);
                stateValue = sqrt(t);
              } else if (vectorOp == "xzMagnitude") {
                if (numDims > iX)
                  t += field(cell, qp, iX) * field(cell, qp, iX);
                if (numDims > iZ)
                  t += field(cell, qp, iZ) * field(cell, qp, iZ);
                stateValue = sqrt(t);
              ...
              if (outputCellAverage) {
                sta(cell, (std::size_t)0) +=
                    ADValue(stateValue) * ADValue(weights(cell, qp));
                vol += ADValue(weights(cell, qp));
              } else
                sta(cell, qp) = ADValue(stateValue);
            }
    ...
    int node_var_offset,
        node_var_ndofs;  // offset into MultiVector of vector corresponding to
                         // stateName, (ndofs not used)
    node_data->getNDofsAndOffset(stateName, node_var_offset, node_var_ndofs);

    const Teuchos::RCP<Thyra_MultiVector>& node_data_vector =
      ...
      switch (size) {  // Note: size should always == 2 now: node_scalar type or
        ...
        case 2:
          std::cout << "DEBUG: ResponseSaveField is saving nodal " << fieldName
                    << " nCells = " << workset.numCells
                    << ", nNodes=" << numNodes
                    << ", EBName = " << workset.EBName << std::endl;
          ...
          for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
            for (std::size_t node = 0; node < numNodes; ++node) {
              ...
              // data->sumIntoGlobalValue(global_row, node_var_offset, 10.0 );
              // //DEBUG - to get weighting correct

              // std::cout << "DEBUG: Saving nodal " <<  fieldName << "(" <<
              // global_row << ") from local ("
              //      << cell << "," << node << ") = " << field(cell,node) <<
              //      std::endl;
            }
          }
          break;
    ...
    vectorOp);
      ...
      default:
        TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error,
                                   "Unexpected dimensions in SaveField response
    Evaluator: " << size);
      }
    } */
  }  // end of Cell,Node
...
// **********************************************************************
template <typename EvalT, typename Traits>
void ResponseSaveField<EvalT, Traits>::postEvaluate(
    typename Traits::PostEvalData /* workset */)
{
  ...
  if (fieldIndices == "Cell,Node") {
    ...
    // workset.wsElNodeID; Teuchos::RCP<const Tpetra_Map> overlap_node_map =
...
ResponseSaveField<EvalT, Traits>::getValidResponseParameters() const
{
  ...
  validPL->set<std::string>(
      "Vector Operation",
      "magnitude",
      "How to convert vector to scalar value, e.g., magnitude, xyMagnitude, "
      "xCoord");
  validPL->set<std::string>(
      "Field Indices",
      "Cell,QuadPt",
      "How the field is indexed.  Allowed values are Cell,QuadPt and "
      "Cell,Node");
  validPL->set<std::string>(
      "State Name", "<Field Name>", "State name to save field as");
  validPL->set<bool>(
      "Output to Exodus",
      true,
      "Whether state should be output in STK dump to exodus");
  validPL->set<bool>(
      "Output Cell Average",
      true,
      "Whether cell average or all quadpoint data should be output to exodus");
  validPL->set<bool>(
      "Memory Placeholder Only",
      false,
      "True if data should not actually be transferred to this state, i.e., "
      "the state is just used as a memory container and should not be "
      "overwritten when responses are computed");
  validPL->set<std::string>(
      "Description",
      "",
