...
template <typename EvalT, typename Traits>
void
HeatEqResid<EvalT, Traits>::postRegistrationSetup(typename Traits::SetupData d, PHX::FieldManager<Traits>& fm)
{
...
HeatEqResid<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  ...
  if (haveSource) {
    ...
    for (int i = 0; i < Source.extent(0); i++)
      for (int j = 0; j < Source.extent(1); j++) neg_source(i, j) = Source(i, j) * -1.0;
