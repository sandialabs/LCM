...
DamageCoefficients<EvalT, Traits>::DamageCoefficients(
    ...
    : damage_(p.get<std::string>("Damage Name"), dl->qp_scalar),
      ...
      damage_diffusivity_(p.get<std::string>("Damage Diffusivity Name"), dl->qp_tensor),
...
template <typename EvalT, typename Traits>
void
DamageCoefficients<EvalT, Traits>::postRegistrationSetup(typename Traits::SetupData d, PHX::FieldManager<Traits>& fm)
{
...
template <typename EvalT, typename Traits>
void
DamageCoefficients<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
