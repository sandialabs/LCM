    ...
    const Teuchos::RCP<Thyra_Vector>&       g)
{
  ...
  Teuchos::ArrayRCP<ST> g_data =
      getNonconstLocalData(g);  // We already checked g is not null
  ...
  for (unsigned int i = 0; i < responses.size(); i++) {
    ...
    Teuchos::RCP<Thyra_Vector> g_i =
        Thyra::createMember(productVectorSpace->getBlock(i));
    g_i->assign(0.0);
...
void
AggregateScalarResponseFunction::evaluateTangent(
    const double                                 alpha,
    const double                                 beta,
    const double                                 omega,
    const double                                 current_time,
    ...
    const Teuchos::RCP<Thyra_MultiVector>&       gp)
{
   ...
   *
   *     // Evaluate response function
   *     responses[i]->evaluateTangent(alpha, beta, omega, current_time,
   * sum_derivs, x, xdot, xdotdot, p, deriv_p, Vx, Vxdot, Vxdotdot, Vp, g_i,
   * gx_i, gp_i);
   *   }
   */
  ...
  for (unsigned int i = 0; i < responses.size(); i++) {
    ...
    for (unsigned int j = 0; j < responses[i]->numResponses(); ++j) {
      if (!g.is_null()) { g_data[offset + j] = gi_data[j]; }
      if (!gx.is_null()) {
...
void
AggregateScalarResponseFunction::evaluateDistParamDeriv(
    const double                            current_time,
    const Teuchos::RCP<const Thyra_Vector>& x,
    const Teuchos::RCP<const Thyra_Vector>& xdot,
    const Teuchos::RCP<const Thyra_Vector>& xdotdot,
    ...
    const Teuchos::RCP<Thyra_MultiVector>&  dg_dp)
{
  ...
  for (unsigned int i = 0; i < responses.size(); i++) {
    ...
    if (!dg_dp.is_null()) { dg_dp_i = dg_dp->subView(colRange); }
