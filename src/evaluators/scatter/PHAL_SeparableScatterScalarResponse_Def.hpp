    ...
    preEvaluate(typename Traits::PreEvalData workset)
{
  ...
  // once ATO:Constraint_2D_adj test passes with this change.  Remove ifdef
  // guards when this is done.
  Teuchos::RCP<Thyra_MultiVector> dgdp            = workset.dgdp;
  Teuchos::RCP<Thyra_MultiVector> overlapped_dgdp = workset.overlapped_dgdp;
  if (dgdp != Teuchos::null) { dgdp->assign(0.0); }
  if (overlapped_dgdp != Teuchos::null) { overlapped_dgdp->assign(0.0); }
}
    ...
    postEvaluate(typename Traits::PostEvalData workset)
{
  ...
  if (!dgdp.is_null() && !overlapped_dgdp.is_null()) {
    workset.p_cas_manager->combine(
        overlapped_dgdp, dgdp, Albany::CombineMode::ADD);
  }
}
    ...
    Traits>::evaluateFields(typename Traits::EvalData workset)
{
  ...
  if (level_it ==
    ...
    return SeparableScatterScalarResponse<
        PHAL::AlbanyTraits::DistParamDeriv,
        Traits>::evaluateFields(workset);
  ...
  const Teuchos::ArrayRCP<Teuchos::ArrayRCP<GO>>& wsElNodeID =
      workset.disc->getWsElNodeID()[workset.wsIndex];
  auto overlap_p_vs = workset.distParamLib->get(workset.dist_param_deriv_name)
