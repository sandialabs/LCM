...
void
AAdapt::SPRSizeField::setParams(const Teuchos::RCP<Teuchos::ParameterList>& p)
{
  if (p->isType<long int>("Target Element Count")) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        p->isParameter("Error Bound"),
        std::logic_error,
        "\"Target Element Count\" and \"Error Bound\" cannot both be "
        "defined\n");
  } else {
    using_rel_err = true;
...
void
AAdapt::SPRSizeField::postProcessShrunkenMesh()
{
  apf::destroyField(mesh_struct->getMesh()->findField("size"));
}

void
AAdapt::SPRSizeField::postProcessFinalMesh()
{
