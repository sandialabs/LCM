...
/**
 * \brief A utility class that encapsulates a defined region of a mesh.  Other
 * evaluators which operator on a mesh region use a MeshRegion instance to
 * determine whether a mesh point lies inside the specified region.
 */
 ...
 public:
  ...
  getValidParameters()
  {
    ...
    validPL->set<std::string>(
        "Operation Domain", "", "Deprecated - does nothing");  // TODO: remove?

    validPL->set<std::string>(
        "Element Block Name", "", "Element block name to restrict region to");
    validPL->set<std::string>(
        "Element Block Names", "", "Element block names to restrict region to");
    validPL->set<bool>(
        "Quantum Element Blocks Only",
        false,
        "Restricts region to quantum element blocks");
    ...
    polyPL.set<int>(
        "Number of Points", 0, "The number of points in the polygon");
    for (int i = 0; i < MAX_POLYGON_PTS; ++i) {
      polyPL.set<Teuchos::Array<double>>(
          Albany::strint("Point", i),
          Teuchos::Array<double>(2, 0),
          "(x,y) point of polygon");
    }

    validPL->set<std::string>(
        "Level Set Field Name",
        "<field name>",
        "Scalar Field to use for level set region");
    validPL->set<double>(
        "Level Set Field Minimum",
        0.0,
        "Minimum value of field to include in region");
    validPL->set<double>(
        "Level Set Field Maximum",
        0.0,
        "Maximum value of field to include in region");
