...
LinComprNSResid<EvalT, Traits>::LinComprNSResid(const Teuchos::ParameterList& p)
          ...
          p.get<Teuchos::RCP<PHX::DataLayout>>("Node Vector Data Layout"))
{
  ...
  if (IBP_convect_terms == true)
    std::cout << "Integrating convective terms by parts in weak form."
              << std::endl;
...
template <typename EvalT, typename Traits>
void
LinComprNSResid<EvalT, Traits>::postRegistrationSetup(
    typename Traits::SetupData d,
    PHX::FieldManager<Traits>& fm)
{
  this->utils.setFieldData(qFluct, fm);
  this->utils.setFieldData(qFluctGrad, fm);
  if (enableTransient) this->utils.setFieldData(qFluctDot, fm);
...
template <typename EvalT, typename Traits>
void
LinComprNSResid<EvalT, Traits>::evaluateFields(
    typename Traits::EvalData workset)
{
  if (eqn_type == EULER) {  // Euler equations
    if (numDims == 1) {     // 1D case
      ...
      if (IBP_convect_terms ==
          false) {  // variational formulation in which the convective terms are
          ...
          for (std::size_t node = 0; node < numNodes; ++node) {
            ...
            for (std::size_t qp = 0; qp < numQPs; ++qp) {
              Residual(cell, node, 0) +=
                  ubar * qFluctGrad(cell, qp, 0, 0) * wBF(cell, node, qp) +
                  zetabar * qFluctGrad(cell, qp, 1, 0) * wBF(cell, node, qp) +
                  force(cell, qp, 0) *
                      wBF(cell, node, qp);  // ubar*du'/dx + zetabar*dp'/dx + f0
              Residual(cell, node, 1) +=
                  gamma_gas * pbar * qFluctGrad(cell, qp, 0, 0) *
                      wBF(cell, node, qp) +
                  ubar * qFluctGrad(cell, qp, 1, 0) * wBF(cell, node, qp) +
                  force(cell, qp, 1) *
                      wBF(cell,
                          node,
                          qp);  // gamma*pbar*du'/dx + ubar*dp'/dx + f2
            }
          }
        }
      } else {  // variational formulation in which the convective terms are
                // integrated by parts
        for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
          for (std::size_t node = 0; node < numNodes; ++node) {
            for (std::size_t i = 0; i < vecDim; i++)
            ...
            for (std::size_t qp = 0; qp < numQPs; ++qp) {
              Residual(cell, node, 0) +=
                  -1.0 * ubar * qFluct(cell, qp, 0) *
                      wGradBF(cell, node, qp, 0) -
                  zetabar * qFluct(cell, qp, 1) * wGradBF(cell, node, qp, 0) +
                  force(cell, qp, 0) *
                      wBF(cell, node, qp);  // ubar*du'/dx + zetabar*dp'/dx + f0
              Residual(cell, node, 1) +=
                  -1.0 * gamma_gas * pbar * qFluct(cell, qp, 0) *
                      wGradBF(cell, node, qp, 0) -
          ...
          for (std::size_t node = 0; node < numNodes; ++node) {
            ...
            for (std::size_t qp = 0; qp < numQPs; ++qp) {
              Residual(cell, node, 0) +=
                  ...
                  force(cell, qp, 0) *
                      wBF(cell, node, qp);  // ubar*du'/dx + vbar*du'/dy +
                                            // zetabar*dp'/dx + f0
              Residual(cell, node, 1) +=
                  ubar * qFluctGrad(cell, qp, 1, 0) * wBF(cell, node, qp) +
                  ...
                  force(cell, qp, 1) *
                      wBF(cell, node, qp);  // ubar*dv'/dx + vbar*dv'/dy +
                                            // zetabar*dp'/dy + f1
              Residual(cell, node, 2) +=
                  gamma_gas * pbar *
                      (qFluctGrad(cell, qp, 0, 0) +
                       qFluctGrad(cell, qp, 1, 1)) *
                      wBF(cell, node, qp) +
                  ubar * qFluctGrad(cell, qp, 2, 0) * wBF(cell, node, qp) +
                  vbar * qFluctGrad(cell, qp, 2, 1) * wBF(cell, node, qp) +
                  force(cell, qp, 2) *
                      wBF(cell, node, qp);  // gamma*pbar*div(u') + ubar*dp'/dx
                                            // + vbar*dp'/dy + f2
            }
          }
        }
        ...
        for (std::size_t node = 0; node < numNodes; ++node) {
          ...
          if (enableTransient)
            for (std::size_t qp = 0; qp < numQPs; ++qp) {
              for (std::size_t i = 0; i < vecDim; i++) {
                Residual(cell, node, i) =
                    rhobar * qFluctDot(cell, qp, i) * wBF(cell, node, qp);
                if (i == vecDim - 1)
                  Residual(cell, node, i) =
                      qFluctDot(cell, qp, i) * wBF(cell, node, qp);
              }
          ...
          for (std::size_t qp = 0; qp < numQPs; ++qp) {
            Residual(cell, node, 0) +=
                rhobar * ubar * qFluctGrad(cell, qp, 0, 0) *
                    wBF(cell, node, qp) +
                rhobar * vbar * qFluctGrad(cell, qp, 0, 1) *
                    wBF(cell, node, qp) +
                rhobar * Rgas * qFluctGrad(cell, qp, 2, 0) *
                    wBF(cell, node, qp) +
                Rgas * Tbar * qFluctGrad(cell, qp, 3, 0) * wBF(cell, node, qp) +
                1.0 / Re *
                    ((2.0 * mu + lambda) * qFluctGrad(cell, qp, 0, 0) *
                         wGradBF(cell, node, qp, 0) +
                     lambda * qFluctGrad(cell, qp, 1, 1) *
                         wGradBF(cell, node, qp, 0) +
                     mu * qFluctGrad(cell, qp, 1, 0) *
                         wGradBF(cell, node, qp, 1) +
                     mu * qFluctGrad(cell, qp, 0, 1) *
                         wGradBF(cell, node, qp, 1)) +
                force(cell, qp, 0) *
                    wBF(cell,
                        node,
            ...
            Residual(cell, node, 1) +=
                rhobar * ubar * qFluctGrad(cell, qp, 1, 0) *
                    wBF(cell, node, qp) +
                rhobar * vbar * qFluctGrad(cell, qp, 1, 1) *
                    wBF(cell, node, qp) +
                rhobar * Rgas * qFluctGrad(cell, qp, 2, 1) *
                    wBF(cell, node, qp) +
                ...
                1.0 / Re *
                    (mu * qFluctGrad(cell, qp, 1, 0) *
                     ...
                     mu * qFluctGrad(cell, qp, 0, 1) *
                         wGradBF(cell, node, qp, 0) +
                     lambda * qFluctGrad(cell, qp, 0, 0) *
                         wGradBF(cell, node, qp, 1) +
                     (2.0 * mu + lambda) * qFluctGrad(cell, qp, 1, 1) *
                         wGradBF(cell, node, qp, 1)) +
                force(cell, qp, 1) *
                    wBF(cell,
                        node,
                        qp);  // rhobar*ubar*dv'/dx + rhobar*vbar*dv'/dy +
                              // rhobar*R*dT'/dy + R*Tbar*drho'/dy +
            ...
            Residual(cell, node, 2) +=
                ...
                rhobar * ubar * qFluctGrad(cell, qp, 2, 0) *
                    wBF(cell, node, qp) +
                rhobar * vbar * qFluctGrad(cell, qp, 2, 1) *
                    wBF(cell, node, qp) +
                1.0 / Re *
                    (gamma_gas * kappa / Pr *
            ...
            Residual(cell, node, 3) +=
                ...
                vbar * qFluctGrad(cell, qp, 3, 1) * wBF(cell, node, qp) +
                force(cell, qp, 3) *
                    wBF(cell, node, qp);  // rhobar*div(u') + ubar*drho'/dx +
                                          // vbar*drho'/dy + f3
          }
        }
      }
        ...
        for (std::size_t node = 0; node < numNodes; ++node) {
          ...
          for (std::size_t qp = 0; qp < numQPs; ++qp) {
            Residual(cell, node, 0) +=
                ...
                rhobar * vbar * qFluctGrad(cell, qp, 0, 1) *
                    wBF(cell, node, qp) +
                rhobar * wbar * qFluctGrad(cell, qp, 0, 2) *
                    wBF(cell, node, qp) +
                rhobar * Rgas * qFluctGrad(cell, qp, 3, 0) *
                    wBF(cell, node, qp) +
                ...
                1.0 / Re *
                    ((2.0 * mu + lambda) * qFluctGrad(cell, qp, 0, 0) *
                     ...
                     lambda * qFluctGrad(cell, qp, 1, 1) *
                         wGradBF(cell, node, qp, 0) +
                     lambda * qFluctGrad(cell, qp, 2, 2) *
                         wGradBF(cell, node, qp, 0) +
                     mu * qFluctGrad(cell, qp, 1, 0) *
                         wGradBF(cell, node, qp, 1) +
                     mu * qFluctGrad(cell, qp, 0, 1) *
                         wGradBF(cell, node, qp, 1) +
                     mu * qFluctGrad(cell, qp, 2, 0) *
                         wGradBF(cell, node, qp, 2) +
                     mu * qFluctGrad(cell, qp, 0, 2) *
                         wGradBF(cell, node, qp, 2)) +
                force(cell, qp, 0) *
                    wBF(cell,
                        node,
            ...
            Residual(cell, node, 1) +=
                ...
                rhobar * vbar * qFluctGrad(cell, qp, 1, 1) *
                    wBF(cell, node, qp) +
                rhobar * wbar * qFluctGrad(cell, qp, 1, 2) *
                    wBF(cell, node, qp) +
                rhobar * Rgas * qFluctGrad(cell, qp, 3, 1) *
                    wBF(cell, node, qp) +
                ...
                1.0 / Re *
                    (mu * qFluctGrad(cell, qp, 1, 0) *
                     ...
                     lambda * qFluctGrad(cell, qp, 2, 2) *
                         wGradBF(cell, node, qp, 1) +
                     mu * qFluctGrad(cell, qp, 2, 1) *
                         wGradBF(cell, node, qp, 2) +
                     mu * qFluctGrad(cell, qp, 1, 2) *
                         wGradBF(cell, node, qp, 2)) +
                force(cell, qp, 1) *
                    wBF(cell,
                        node,
            ...
            Residual(cell, node, 2) +=
                ...
                rhobar * vbar * qFluctGrad(cell, qp, 2, 1) *
                    wBF(cell, node, qp) +
                rhobar * wbar * qFluctGrad(cell, qp, 2, 2) *
                    wBF(cell, node, qp) +
                rhobar * Rgas * qFluctGrad(cell, qp, 3, 2) *
                    wBF(cell, node, qp) +
                ...
                1.0 / Re *
                    (mu * qFluctGrad(cell, qp, 2, 0) *
                     ...
                     lambda * qFluctGrad(cell, qp, 1, 1) *
                         wGradBF(cell, node, qp, 2) +
                     (2.0 * mu + lambda) * qFluctGrad(cell, qp, 2, 2) *
                         wGradBF(cell, node, qp, 2)) +
                force(cell, qp, 2) *
                    wBF(cell,
                        node,
                        qp);  // rhobar*ubar*dw'/dx + rhobar*vbar*dw'/dy +
                              // rhobar*wbar*dw'/dz + rhobar*R*dT'/dz +
                              // R*Tbar*drho'/dz + 1/Re*(d/dx(mu*dw'/dx +
            ...
            Residual(cell, node, 3) +=
                rhobar * Tbar * (gamma_gas - 1.0) *
                    (qFluctGrad(cell, qp, 0, 0) + qFluctGrad(cell, qp, 1, 1) +
                     qFluctGrad(cell, qp, 2, 2)) *
                    wBF(cell, node, qp) +
                rhobar * ubar * qFluctGrad(cell, qp, 3, 0) *
                    wBF(cell, node, qp) +
                rhobar * vbar * qFluctGrad(cell, qp, 3, 1) *
                    wBF(cell, node, qp) +
                1.0 / Re *
                    (gamma_gas * kappa / Pr *
                     (qFluctGrad(cell, qp, 3, 0) * wGradBF(cell, node, qp, 0) +
                      qFluctGrad(cell, qp, 3, 1) * wGradBF(cell, node, qp, 1) +
                      qFluctGrad(cell, qp, 3, 2) *
                          wGradBF(cell, node, qp, 2))) +
                force(cell, qp, 3) *
                    wBF(cell,
                        node,
