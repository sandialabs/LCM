...
have model meaning: tensile failure as the stress state reaching the
tensile apex of the surface ($F_f - N \to 0$), replacing the
maximum-principal-stress check; shear exhaustion as back-stress
saturation ($\sqrt{J_2^\alpha} \to N$, $G^\alpha = 0$: the yield
surface has translated to the failure envelope), replacing the
criterion that yield onset itself constitutes failure --- the cap
model can distinguish yielded-but-hardening material from exhausted
material, which $J_2$ cannot; compaction exhaustion as
$|\varepsilon_v^p| \to W$ (full pore collapse, the cap-lock regime);
and a plastic distortion limit on
$\varepsilon_{\mathrm{eqps}}$ replacing the total-distortion limit.
The two structural criteria --- tilt angle and displacement norm,
which detect toppled or departed blocks --- are not constitutive and
remain unchanged at the erosion layer, as does the element-death
bookkeeping (per-point failure-mode bitmask, per-cell death
threshold), which is model independent.
...
implementation at machine precision. Failure indicators and
element-death plumbing (Phase 2) and ACE workflow integration
(Phase 3) remain.
