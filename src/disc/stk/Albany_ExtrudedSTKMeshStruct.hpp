...
class ExtrudedSTKMeshStruct : public GenericSTKMeshStruct
{
 public:
  ...
  setFieldAndBulkData(
      ...
      const std::map<std::string, Teuchos::RCP<Albany::StateInfoStruct>>&
          side_set_sis = {},  // empty map as default
      const std::
 ...
 private:
  ...
  computeMap();
  inline int
  prismType(GO const* prismVertexMpasIds, int& minIndex);
  inline void
  tetrasFromPrismStructured(
      GO const* prismVertexMpasIds,
      GO const* prismVertexGIds,
      GO        tetrasIdsOnPrism[][4]);
...
int
ExtrudedSTKMeshStruct::prismType(GO const* prismVertexMpasIds, int& minIndex)
{
  int PrismVerticesMap[6][6] = {{0, 1, 2, 3, 4, 5},
                                {1, 2, 0, 4, 5, 3},
                                {2, 0, 1, 5, 3, 4},
                                {3, 5, 4, 0, 2, 1},
                                {4, 3, 5, 1, 0, 2},
                                {5, 4, 3, 2, 1, 0}};
  minIndex = std::min_element(prismVertexMpasIds, prismVertexMpasIds + 3) -
             prismVertexMpasIds;
...
void
ExtrudedSTKMeshStruct::tetrasFromPrismStructured(
    GO const* prismVertexMpasIds,
    GO const* prismVertexGIds,
    GO        tetrasIdsOnPrism[][4])
{
  ...
  int tetraOfPrism[2][3][4] = {{{0, 1, 2, 5}, {0, 1, 5, 4}, {0, 4, 5, 3}},
                               {{0, 1, 2, 4}, {0, 4, 2, 5}, {0, 4, 5, 3}}};

  int tetraAdjacentToPrismLateralFace[2][3][2] = {{{1, 2}, {0, 1}, {0, 2}},
                                                  {{0, 2}, {0, 1}, {1, 2}}};
  ...
  for (int iTetra = 0; iTetra < 3; iTetra++)
    for (int iVertex = 0; iVertex < 4; iVertex++) {
      tetrasIdsOnPrism[iTetra][iVertex] =
          reorderedPrismLIds[tetraOfPrism[prismType][iTetra][iVertex]];
    }
}

void
ExtrudedSTKMeshStruct::computeMap()
{
  int PrismVerticesMap[6][6] = {{0, 1, 2, 3, 4, 5},
                                {1, 2, 0, 4, 5, 3},
                                {2, 0, 1, 5, 3, 4},
        ...
        for (int tFace = 0; tFace < 4; ++tFace) {
          for (int iTetra = 0; iTetra < 3; ++iTetra) {
            ...
            for (int in = 0; in < 3; ++in) {
              int node = PrismVerticesMap[minIndex]
                                         [tetraOfPrism[pType][iTetra]
                                                      [TetraFaces[tFace][in]]];
              for (int i = 0; i < 4; ++i)
