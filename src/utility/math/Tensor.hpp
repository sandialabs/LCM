...
class BasicTensor
{
 public:
  ...
  static KOKKOS_INLINE_FUNCTION constexpr index_t
  getOrder();
  KOKKOS_INLINE_FUNCTION
  BasicTensor();
  explicit KOKKOS_INLINE_FUNCTION
  BasicTensor(index_t dimension, value_type initialValue = value_type(0));

  KOKKOS_INLINE_FUNCTION
  BasicTensor(const BasicTensor<T, Order>& other);
  ...
  KOKKOS_INLINE_FUNCTION BasicTensor<T, Order>&
                         operator=(const BasicTensor<T, Order>& other);
  // KOKKOS_INLINE_FUNCTION BasicTensor<T, Order>& operator=(BasicTensor<T,
  // Order>&& other);

  KOKKOS_INLINE_FUNCTION iterator
  begin();
  KOKKOS_INLINE_FUNCTION iterator
  end();

  KOKKOS_INLINE_FUNCTION const_iterator
  begin() const;
  KOKKOS_INLINE_FUNCTION const_iterator
  end() const;
  ...
  template <typename... Indices>
  KOKKOS_INLINE_FUNCTION reference
  operator()(Indices... indices);

  template <typename Array, typename... Indices>
  KOKKOS_INLINE_FUNCTION void
  fill(Array arr, Indices... fixed_indices);

  constexpr KOKKOS_INLINE_FUNCTION index_t
  arraySize() const;
...
template <typename T>
KOKKOS_INLINE_FUNCTION Tensor2<T>
                       identity(index_t dim);

template <typename T>
KOKKOS_INLINE_FUNCTION Tensor4<T>
...
template <typename T>
KOKKOS_INLINE_FUNCTION typename Tensor2<T>::value_type
