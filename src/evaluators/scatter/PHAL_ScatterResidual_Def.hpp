...
template <typename Traits>
ScatterResidual<PHAL::AlbanyTraits::Tangent, Traits>::ScatterResidual(
    const Teuchos::ParameterList&        p,
    const Teuchos::RCP<Albany::Layouts>& dl)
    : ScatterResidualBase<PHAL::AlbanyTraits::Tangent, Traits>(p, dl),
      numFields(ScatterResidualBase<PHAL::AlbanyTraits::Tangent, Traits>::
                    numFieldsBase)
{
}
...
template <typename Traits>
void
ScatterResidual<PHAL::AlbanyTraits::Tangent, Traits>::evaluateFields(
    typename Traits::EvalData workset)
{
  ...
  for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
    for (std::size_t node = 0; node < this->numNodes; ++node) {
      for (std::size_t eq = 0; eq < numFields; eq++) {
        ...
        if (Teuchos::nonnull(f)) { f_nonconstView[row] += valref.val(); }
        ...
        if (Teuchos::nonnull(fp)) {
          for (int col = 0; col < workset.num_cols_p; col++) {
            fp_nonconst2dView[col][row] +=
                valref.dx(col + workset.param_offset);
          }
        }
      }
    }
...
template <typename Traits>
ScatterResidual<PHAL::AlbanyTraits::DistParamDeriv, Traits>::ScatterResidual(
    const Teuchos::ParameterList&        p,
    const Teuchos::RCP<Albany::Layouts>& dl)
    : ScatterResidualBase<PHAL::AlbanyTraits::DistParamDeriv, Traits>(p, dl),
      numFields(
          ScatterResidualBase<PHAL::AlbanyTraits::DistParamDeriv, Traits>::
              numFieldsBase)
{
}
...
template <typename Traits>
void
ScatterResidual<PHAL::AlbanyTraits::DistParamDeriv, Traits>::evaluateFields(
    typename Traits::EvalData workset)
{
  auto                                     nodeID = workset.wsElNodeEqID;
  ...
  if (workset.local_Vp[0].size() == 0) {
    return;
  }  // In case the parameter has not been gathered, e.g. parameter is used only
     // in Dirichlet conditions.
            ...
            for (std::size_t eq = 0; eq < numFields; eq++) {
              typename PHAL::Ref<ScalarT const>::type valref =
                  (this->tensorRank == 0 ?
                       ...
                       this->valTensor(cell, node, eq / numDims, eq % numDims));
              val += valref.dx(i) *
                     local_Vp[node * neq + eq + this->offset]
                             [col];  // numField can be less then neq
            }
          }
          const LO row = wsElDofs((int)cell, i, 0);
          if (row >= 0) { fpV_nonconst2dView[col][row] += val; }
        }
      }
    }
  } else {
...
template <typename Traits>
void
ScatterResidualWithExtrudedParams<PHAL::AlbanyTraits::DistParamDeriv, Traits>::
    evaluateFields(typename Traits::EvalData workset)
{
  ...
  if (level_it ==
      extruded_params_levels
          ->end())  // if parameter is not extruded use usual scatter.
    return ScatterResidual<PHAL::AlbanyTraits::DistParamDeriv, Traits>::
        evaluateFields(workset);
  ...
  if (trans) {
    ...
    auto overlapVS = workset.distParamLib->get(workset.dist_param_deriv_name)
                         ->overlap_vector_space();
