...
PUMINodeMetaData::PUMINodeMetaData(
    ...
    : name(name), output(output), dims(dims)
{
  ...
  for (std::size_t i = 1; i < this->dims.size(); i++) nfield_dofs *= dims[i];
}

int
PUMINodeMetaData::ndims() const
{
  if (dims.size() == 3)
    return 2;
  else if (dims.size() == 2)
...
PUMINodeData<DataType, ArrayDim, traits>::PUMINodeData(
    const std::string&                             name_,
    const std::vector<PHX::DataLayout::size_type>& dims_,
    const bool                                     output_)
    : PUMINodeDataBase<DataType>(name_, output_, dims_), beginning_index(0)
{
  // Nothing to do here
}
...
template <typename DataType, unsigned ArrayDim, class traits>
void
PUMINodeData<DataType, ArrayDim, traits>::resize(
    const Teuchos::RCP<const Thyra_VectorSpace> local_node_vs)
{
...
template <typename DataType, unsigned ArrayDim, class traits>
MDArray
PUMINodeData<DataType, ArrayDim, traits>::getMDA(
    const std::vector<apf::Node>& buck)
{
  unsigned numNodes =
      buck.size();  // Total size starts at the number of nodes in the workset

  field_type the_array = traits_type::buildArray(
      &this->buffer[beginning_index], numNodes, this->dims);
...
template <typename DataType, unsigned ArrayDim, class traits>
void
PUMINodeData<DataType, ArrayDim, traits>::saveFieldVector(
    ...
    int                                          offset)
{
  ...
  for (int j = 0; j < this->nfield_dofs; ++j) {
    auto const_overlap_node_view =
        getLocalData(overlap_node_vec->col(offset + j));
    ...
    for (LO i = 0; i < m_local_node_vs->localSubDim(); ++i) {
      ...
      this->buffer[i * this->nfield_dofs + j] =
          const_overlap_node_view[overlap_node_lid];
    }
  }
}
