...
const char* Albany::APFMeshStruct::solution_name[3] = {"Solution",
                                                       "SolutionDot",
                                                       "SolutionDotDot"};
...
static void
readSets(apf::Mesh* m, const char* filename, apf::StkModels& sets)
{
  ...
  while (std::getline(f, sline)) {
    ...
    for (int di = 0; di < 3; ++di)
      if (sline.compare(0, dimNames[di].length(), dimNames[di]) == 0) sdi = di;
    TEUCHOS_TEST_FOR_EXCEPTION(
        sdi == -1,
        std::logic_error,
...
static void
getEBSizes(apf::Mesh* mesh, apf::StkModels& sets, std::vector<int>& el_blocks)
{
  ...
  while ((e = mesh->iterate(mit))) {
    ...
    if (sets.invMaps[d].count(me)) ++(sizeMap[sets.invMaps[d][me]]);
  }
  mesh->end(mit);
...
void
Albany::APFMeshStruct::init(
    const Teuchos::RCP<Teuchos::ParameterList>& params,
    const Teuchos::RCP<const Teuchos_Comm>&     commT)
{
  out = Teuchos::VerboseObjectBase::getDefaultOStream();

  useNullspaceTranslationOnly =
      params->get<bool>("Use Nullspace Translation Only", false);
  ...
  if (params->isParameter("Model Associations File Name")) {
    std::string afile =
        params->get<std::string>("Model Associations File Name");
  ...
  // getEBSizes will seg fault if at least one element block isn't defined in
  // input. Lets exit a little more gracefully.
  TEUCHOS_TEST_FOR_EXCEPTION(
      numEB == 0,
      std::logic_error,
      "PUMI requires at least one element block to be defined in the input "
      "file.");
        ...
        AbstractFieldContainer::FieldContainerRequirements>& /*side_set_req*/)
{
  ...
  // Set the number of equation present per node. Needed by
  // Albany_APFDiscretization.
  neq = neq_;
  ...
  solVectorLayout[0] = params->get<Teuchos::Array<std::string>>(
      "Solution Vector Components", defaultLayout);

  // check that the user entered an even number of strings
  TEUCHOS_TEST_FOR_EXCEPTION(
      (solVectorLayout[0].size() % 2),
      std::logic_error,
      "Error in input file: specification of solution vector layout is "
      "incorrect\n");

  // Set up a default layout for solution dot, based on what was specified in
  // the base layout
  ...
  solVectorLayout[1] = params->get<Teuchos::Array<std::string>>(
      "SolutionDot Vector Components", dotLayout);

  TEUCHOS_TEST_FOR_EXCEPTION(
      (solVectorLayout[1].size() % 2),
      std::logic_error,
      "Error in input file: specification of solution vector dot layout is "
      "incorrect\n");
  ...
  for (int i = 0; i < solVectorLayout[0].size(); i += 2) {
    dotdotLayout[i] =
        solVectorLayout[0][i] + "DotDot";  // concat the term DotDot
    dotdotLayout[i + 1] = solVectorLayout[0][i + 1];
  }

  solVectorLayout[2] = params->get<Teuchos::Array<std::string>>(
      "SolutionDotDot Vector Components", dotdotLayout);

  TEUCHOS_TEST_FOR_EXCEPTION(
      (solVectorLayout[2].size() % 2),
      std::logic_error,
      "Error in input file: specification of solution vector dotdot layout is "
      "incorrect\n");

  solutionInitialized = true;
  residualInitialized = true;

  if (solVectorLayout[0].size() == 0 && (neq == 2 || neq == 4))
    problemDim = 2;
  else
    problemDim = numDim;

  if (solVectorLayout[0].size() == 0) {
    // If the user has not specified a solution vector layout, provide a simple
    // default Note that the logic here requires that the user enter something
    // for the "Solution Vector Components", or they will get the default.
  ...
  for (std::size_t i = 0; i < sis->size(); i++) {
    ...
    if (meshSpecsType() == AbstractMeshStruct::PUMI_MS) {
      if (hasRestartSolution) st.restartDataAvailable = true;
      if ((shouldLoadLandIceData) &&
          (st.entity == StateStruct::NodalDataToElemNode))
        st.restartDataAvailable = true;
    }
#endif

    if (!nameSet.insert(st.name).second) continue;  // ignore duplicates
    ...
    if (st.entity == StateStruct::NodalData) {  // Data at the node points
      ...
      (*nodeContainer)[st.name] =
          Albany::buildPUMINodeField(st.name, dim, st.output);
    } else if (dim.size() == 2) {
      if (st.entity == StateStruct::QuadPoint ||
          st.entity == StateStruct::ElemNode)
        qpscalar_states.push_back(
            Teuchos::rcp(new PUMIQPData<double, 2>(st.name, dim, st.output)));
      else if (st.entity == StateStruct::NodalDataToElemNode)
        elemnodescalar_states.push_back(
            Teuchos::rcp(new PUMIQPData<double, 2>(st.name, dim, st.output)));
    } else if (dim.size() == 3) {
      if (st.entity == StateStruct::QuadPoint ||
          st.entity == StateStruct::ElemNode)
        qpvector_states.push_back(
            Teuchos::rcp(new PUMIQPData<double, 3>(st.name, dim, st.output)));
    } else if (dim.size() == 4) {
      if (st.entity == StateStruct::QuadPoint ||
          st.entity == StateStruct::ElemNode)
        qptensor_states.push_back(
            Teuchos::rcp(new PUMIQPData<double, 4>(st.name, dim, st.output)));
    } else if (
        dim.size() == 1 && st.entity == Albany::StateStruct::WorksetValue)
      scalarValue_states.push_back(
          Teuchos::rcp(new PUMIQPData<double, 1>(st.name, dim, st.output)));
    else
      TEUCHOS_TEST_FOR_EXCEPT_MSG(
          true,
          "dim.size() < 2 || dim.size()>4 || "
...
void
Albany::APFMeshStruct::splitFields(
    Teuchos::Array<Teuchos::Array<std::string>>& fieldLayout)
{  // user is breaking up or renaming solution & residual fields

  for (int fcomp = 0; fcomp <= num_time_deriv; fcomp++) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        (fieldLayout[fcomp].size() % 2),
        std::logic_error,
        "Error in input file: specification of solution vector layout is "
        "incorrect\n");
    ...
    for (std::size_t i = 0; i < fieldLayout[fcomp].size(); i += 2) {
      TEUCHOS_TEST_FOR_EXCEPTION(
          mesh->findField(fieldLayout[fcomp][i].c_str()),
          std::logic_error,
          "Error in input file: specification of solution vector layout is "
          "incorrect\n"
      ...
      // Add the residual field - based on the text entered in the "Solution" PL
      // only
      if (fcomp == 0) {
        std::string res_name = fieldLayout[fcomp][i];
        res_name.append("Res");
        residualInitialized =
            findOrCreateNodalField(res_name.c_str(), valueType);
      }
    }
  }
}
...
int
Albany::APFMeshStruct::computeWorksetSize(
...
Albany::APFMeshStruct::getValidDiscretizationParameters() const
{
  ...
  validPL->set<std::string>(
      "Acis Model Input File Name", "", "File Name For ACIS Model Input");
  validPL->set<std::string>(
      "Parasolid Model Input File Name",
      "",
      "File Name For PARASOLID Model Input");

  Teuchos::TwoDArray<std::string> defaultData;
  validPL->set<Teuchos::TwoDArray<std::string>>(
      "Element Block Associations",
      defaultData,
      "Association between region ID and element block string");
  validPL->set<Teuchos::TwoDArray<std::string>>(
      "Node Set Associations",
      defaultData,
      "Association between geometric face ID and node set string");
  validPL->set<Teuchos::TwoDArray<std::string>>(
      "Edge Node Set Associations",
      defaultData,
      "Association between geometric edge ID and node set string");
  validPL->set<Teuchos::TwoDArray<std::string>>(
      "Vertex Node Set Associations",
      defaultData,
      "Association between geometric edge ID and node set string");
  validPL->set<Teuchos::TwoDArray<std::string>>(
      "Side Set Associations",
      defaultData,
      "Association between face ID and side set string");

  validPL->set<bool>(
      "Use Nullspace Translation Only",
      false,
      "Temporary hack to get MueLu (possibly) working for us");

  validPL->set<std::string>(
      "Model Associations File Name",
      "",
      "File with element block/sideset/nodeset associations");

  validPL->set<bool>(
      "QP Temperature from Nodes",
      false,
