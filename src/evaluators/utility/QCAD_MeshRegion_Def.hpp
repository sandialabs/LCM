...
QCAD::MeshRegion<EvalT, Traits>::MeshRegion(
    std::string                                  coordVecName,
    ...
    const Teuchos::RCP<Albany::Layouts>&         dl_)
{
  ...
  if (ebNameStr.length() == 0)
    ebNameStr = p.get<std::string>("Element Block Names", "");
  if (ebNameStr.length() > 0)
    Albany::splitStringOnDelim(ebNameStr, ',', ebNames);
  ...
  if (p.isSublist("XY Polygon")) {
    ...
    for (int i = 0; i < nPts; i++) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(
          ar.size() != (int)polyPtDim,
          Teuchos::Exceptions::InvalidParameter,
          std::endl
              << "XY-polygon point does not have " << polyPtDim << " elements"
              << std::endl);
      for (std::size_t k = 0; k < polyPtDim; k++) xyPolygon[i][k] = ar[k];
    }
  }
  bRestrictToXYPolygon =
      (nPts >= 3);  // need at least three points to restrict to a polygon
...
template <typename EvalT, typename Traits>
void
QCAD::MeshRegion<EvalT, Traits>::addDependentFields(
    PHX::EvaluatorWithBaseImpl<Traits>* evaluator)
{
...
template <typename EvalT, typename Traits>
void
QCAD::MeshRegion<EvalT, Traits>::postRegistrationSetup(
...
template <typename EvalT, typename Traits>
bool
QCAD::MeshRegion<EvalT, Traits>::elementBlockIsInRegion(
    std::string ebName) const
{
  bool bQuantumEB = false;  // assume eb's aren't "quantum" unless we're told
                            // they are by the material DB

  if (materialDB != Teuchos::null)
    bQuantumEB =
        materialDB->getElementBlockParam<bool>(ebName, "quantum", false);

  if (bQuantumEBsOnly == true && bQuantumEB == false) return false;

  if (ebNames.size() > 0 &&
      std::find(ebNames.begin(), ebNames.end(), ebName) == ebNames.end())
    return false;
...
template <typename EvalT, typename Traits>
bool
QCAD::MeshRegion<EvalT, Traits>::cellIsInRegion(std::size_t cell)
{
  // Check that cell lies *entirely* in box
  for (std::size_t qp = 0; qp < numQPs; ++qp) {
    if ((limitX &&
         (coordVec(cell, qp, 0) < xmin || coordVec(cell, qp, 0) > xmax)) ||
        (limitY &&
         (coordVec(cell, qp, 1) < ymin || coordVec(cell, qp, 1) > ymax)) ||
        (limitZ &&
         (coordVec(cell, qp, 2) < zmin || coordVec(cell, qp, 2) > zmax)))
      return false;

    if (bRestrictToXYPolygon) {
      RealType pt[2];  // MP: when MeshScalarT != double we take ADValue of
  ...
  if (bRestrictToLevelSet) {
    for (std::size_t qp = 0; qp < numQPs; ++qp) {
      // Get the average value for the cell (integral of field over cell divided
      // by cell volume)
