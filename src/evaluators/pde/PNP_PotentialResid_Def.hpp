...
template <typename EvalT, typename Traits>
void
PNP::PotentialResid<EvalT, Traits>::postRegistrationSetup(typename Traits::SetupData d, PHX::FieldManager<Traits>& fm)
{
...
template <typename EvalT, typename Traits>
void
PNP::PotentialResid<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  ...
  FST::integrate(PotentialResidual.get_view(), flux, wGradBF.get_view(),
    ...
    for (std::size_t node = 0; node < numNodes; ++node) {
      for (std::size_t qp = 0; qp < numQPs; ++qp) {
        for (std::size_t j = 0; j < numSpecies; ++j) {
          PotentialResidual(cell, node) -= q[j] * Concentration(cell, qp, j) * wBF(cell, node, qp);
          // cout << "XXX " << cell << " " << node << " " << qp << " " << j << "
          // " << q[j] << "  " << Concentration(cell,qp,j) << endl;
        }
      }
    }
  }
