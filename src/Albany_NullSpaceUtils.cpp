    ...
    typename Traits::array_type&           rbm)
{
  ...
  const int numSpaceDim =
      coordMV->domain()
          ->dim();  // Number of multivectors are the dimension of the problem

  auto      data = getLocalData(coordMV.getConst());
  const int numNodes =
      data[0].size();  // length of each vector in the multivector
    ...
    typename Traits::array_type&           rbm)
{
  ...
  for (int node = 0; node < numNodes; node++) {
    ...
    switch (NSdim - NscalarDof) {
      case 3:
        for (ii = 0; ii < 2;
             ii++) { /* upper right = [ Q ] -- xy rotation only */
          jj = 2 + NscalarDof;
          // std::cout <<"jj " << jj << " " << ii + jj << std::endl;
          if (ii == 0)
            traits_class.ArrObj(dof, ii, jj) = y[node];
...
subtractCentroid(const Teuchos::RCP<Thyra_MultiVector>& coordMV)
{
  ...
  ST   centroid[3];  // enough for up to 3d
  {
    ...
    Teuchos::reduceAll(
        *createTeuchosCommFromThyraComm(spmd_vs->getComm()),
        Teuchos::REDUCE_SUM,
        ndim,
        sum,
        centroid);
    const int numNodes =
        spmd_vs->localSubDim();  // length of each vector in the multivector
...
struct Tpetra_NullSpace_Traits
{
  ...
  double&
  ArrObj(const LO DOF, const int i, const int j)
  {
...
struct Epetra_NullSpace_Traits
{
  ...
  Epetra_NullSpace_Traits(
      const int                   ndof,
      const int                   nscalardof,
      const int                   nsdim,
      const array_type::size_type veclen,
      array_type&                 array)
      : Ndof(ndof),
        NscalarDof(nscalardof),
        NSdim(nsdim),
  ...
  double&
  ArrObj(const array_type::size_type DOF, const int i, const int j)
  {
    ...
    const Teuchos::RCP<Teuchos::ParameterList>& piroParams)
{
  ...
  if (Teuchos::nonnull(stratList) &&
    ...
    if (ptype == "ML") {
      plist = sublist(
          sublist(sublist(stratList, "Preconditioner Types"), ptype),
          "ML Settings");
      mlUsed = true;
    } else if (ptype == "MueLu") {
...
RigidBodyModes::setParameters(
    const int  numPDEs_,
    const int  numElasticityDim_,
    const int  numScalar_,
    ...
    const Teuchos::RCP<const Thyra_VectorSpace>& soln_overlap_vs)
{
  ...
  if (numElasticityDim > 0 || setNonElastRBM == true) {
    if (isMLUsed()) {
      ...
      if (setNonElastRBM == true)
        Coord2RBM_nonElasticity<Epetra_NullSpace_Traits>(
            coordMV, numPDEs, numScalar, nullSpaceDim, err);
      else
        Coord2RBM<Epetra_NullSpace_Traits>(
            coordMV, numPDEs, numScalar, nullSpaceDim, err);
      ...
      trr = Teuchos::rcp(new Tpetra_NullSpace_Traits::base_array_type(
          getTpetraMap(soln_vs), nullSpaceDim + numScalar, false));

      subtractCentroid(coordMV);

      if (setNonElastRBM == true)
        Coord2RBM_nonElasticity<Tpetra_NullSpace_Traits>(
