/*--------------------------------------------------------------------*/
/*    Copyright 2002, 2010, 2011 National Technology & Engineering Solutions of Sandia, LLC (NTESS) */
...
std::vector<mesh::Entity>
GeomDecomp::entity_coordinates(
    ...
    std::vector<std::vector<double>>& coordinates)
{
  ...
  if (enttype == NODE_RANK) {
    throw std::runtime_error(
        "GeomDecomp::entity_coordinates Error: Can not be called for nodal "
        "entities.");
  } else {
...
GeomDecomp::compute_entity_centroid(
    stk::mesh::BulkData& bulk_data,
    const mesh::Entity&  entity,
...
namespace {
void
apply_rotation(std::vector<double>& coor)
{
