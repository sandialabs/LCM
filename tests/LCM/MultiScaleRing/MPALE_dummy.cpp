...
void
print_vec(std::vector<double>& vec)
{
  for (int i = 0; i < 3; i++) {
    std::cout << " [ ";
    for (int j = 0; j < 3; j++) std::cout << vec[i * 3 + j] << " ";
    std::cout << "] " << std::endl;
  }
  std::cout << std::endl;
}
...
main(int argc, char* argv[])
{
  ...
  if (albany == MPI_COMM_NULL) print_error("No parent!");
  ...
  if (size != 1) print_error("Something's wrong with the parent");
