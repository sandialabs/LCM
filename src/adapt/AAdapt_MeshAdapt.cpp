...
void
anlzCoords(const Teuchos::RCP<const Albany::PUMIDiscretization>& pumi_disc);
void
writeMesh(const Teuchos::RCP<Albany::PUMIDiscretization>& pumi_disc);
double
findAlpha(
...
MeshAdapt::MeshAdapt(
    ...
    : AbstractAdapter(params_, paramLib_, StateMgr_, comm_),
      remeshFileIndex(1),
      rc_mgr(refConfigMgr_)
{
  disc = StateMgr_.getDiscretization();

  pumi_discretization =
      Teuchos::rcp_dynamic_cast<Albany::PUMIDiscretization>(disc);
  ...
  if (method == "RPI Constant Size")
    ...
    checkValidStateVariable(
        StateMgr_, params_->get<std::string>("State Variable", ""));
...
namespace {
inline int
getValueType(const PHX::DataLayout& dl)
{
  switch (dl.rank() - 2) {
    ...
    default:
      TEUCHOS_TEST_FOR_EXCEPTION(
          true, std::logic_error, "not a valid rank: " << dl.rank() - 2);
      return -1;
  }
}
...
MeshAdapt::initRcMgr()
{
  ...
  if (rc_mgr->usingProjection()) {
    ...
    rc_mgr->initProjector(
        pumi_discretization->getNodeVectorSpace(),
        pumi_discretization->getOverlapNodeVectorSpace());
  }
}

bool
MeshAdapt::queryAdaptationCriteria(int iteration)
{
  adapt_params_->set<int>("LastIter", iteration);
  std::string strategy =
      adapt_params_->get<std::string>("Remesh Strategy", "Step Number");
  if (strategy == "None") return false;
  if (strategy == "Continuous") return iteration > 1;
  if (strategy == "Step Number") {
    TEUCHOS_TEST_FOR_EXCEPTION(
        !adapt_params_->isParameter("Remesh Step Number"),
        std::logic_error,
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      true, std::logic_error, "Unknown Remesh Strategy " << strategy << '\n');
  return false;
}
...
MeshAdapt::initAdapt()
{
  ...
  if (found != std::string::npos) {
    ...
    *output_stream_ << "Remeshing: renaming exodus output file to - " << str
                    << std::endl;
...
namespace {
void
setUnitEntWeights(ma::Mesh* m, ma::Tag* weights, int dim)
{
  ma::Entity*        e;
  double             one = 1.0;
  apf::MeshIterator* it  = m->begin(dim);
  while ((e = m->iterate(it))) m->setDoubleTag(e, weights, &one);
  m->end(it);
}

void
runParmaVtxElm(ma::Mesh* m, double maxImb)
{
...
void
runZoltanBal(ma::Mesh* m, double maxImb)
{
...
void
postBalance(ma::Mesh* m, std::string const& method, double maxImb)
{
  if (method == "zoltan") {
...
MeshAdapt::afterAdapt()
{
  ...
  if (should_transfer_ip_data) pumi_discretization->detachQPData();
...
MeshAdapt::adaptMesh()
{
  ...
  const double min_part_density =
      adapt_params_->get<double>("Minimum Part Density", 1000);
  ...
  if (rc_mgr.is_null()) {
    ...
    if (!al::correctnessTestSkip()) {
      beforeAdapt();
      adaptShrunken(
          pumi_discretization->getPUMIMeshStruct()->getMesh(),
          min_part_density,
          callback);
      afterAdapt();
    }
    ...
    for (rc::Manager::Field::iterator it  = rc_mgr->fieldsBegin(),
      ...
      for (int i = 0; i < (*it)->num_g_fields; ++i) {
        const Teuchos::RCP<Thyra_MultiVector>& mv =
            rc_mgr->getNodalField(**it, i, overlapped);
        ...
        for (int c = 0; c < ncol; ++c) {
          for (size_t k = 0; k < n; ++k) { data[ncol * k + c] = mv_data[c][k]; }
        }
    ...
    for (rc::Manager::Field::iterator it  = rc_mgr->fieldsBegin(),
      ...
      for (int i = 0; i < (*it)->num_g_fields; ++i) {
        ...
        pumi_discretization->getField(
            (*it)->get_g_name(i).c_str(), &data[0], overlapped, 0, ncol);
        for (int c = 0; c < ncol; ++c) {
          for (size_t k = 0; k < n; ++k) { mv_data[c][k] = data[ncol * k + c]; }
        }
      }
  }
...
// out. That's bad.
bool
MeshAdapt::adaptMeshLoop(
    const double     min_part_density,
    Parma_GroupCode& callback)
{
  const int n_max_outer_iterations = 10, n_max_inner_iterations_if_found = 20,
            // -log2(eps) = 52. Since this iteration uses bisection, we can go
            // for about 50 iterations before failure is definite. The iteration
            // is cheap relative to other parts of adaptation, so there is no
            // reason not to set this parameter to ~50.
  ...
  for (int it = 0; it < n_max_outer_iterations; ++it) {
    const double alpha = al::findAlpha(
        pumi_discretization,
        rc_mgr,
        n_max_inner_iterations_if_found,
    ...
    // Resize x.
    rc_mgr->get_x() =
        Thyra::createMember(pumi_discretization->getVectorSpace());
...
void
MeshAdapt::checkValidStateVariable(
    ...
    const std::string           name)
{
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      Teuchos::Exceptions::InvalidParameter,
      "Error! Invalid State Variable Parameter \""
          << name << "\" looking for \"" << stateName << "\"" << std::endl);
}
...
MeshAdapt::getValidAdapterParameters() const
{
  ...
  validPL->set<bool>(
      "Should Coarsen",
      true,
      "Set to false to disable mesh coarsening operations.");
  validPL->set<int>(
      "Max Number of Mesh Adapt Iterations",
      1,
      "Number of iterations to limit meshadapt to");
  validPL->set<double>(
      "Target Element Size", 0.1, "Value of the constant size field");
  validPL->set<double>(
      "Mesh Size Scaling",
      0.7,
      "Scales the current average size to make the new size field");
  validPL->set<double>(
      "Error Bound", 0.1, "Max relative error for error-based adaptivity");
  validPL->set<long int>(
      "Target Element Count",
      1000,
      "Desired number of elements for error-based adaptivity");
  validPL->set<std::string>(
      "State Variable", "", "SPR operates on this variable");
  validPL->set<Teuchos::Array<std::string>>(
      "Load Balancing", defaultStArgs, "Turn on predictive load balancing");
  validPL->set<double>(
      "Maximum LB Imbalance",
      1.3,
      "Set maximum imbalance tolerance for predictive laod balancing");
  validPL->set<std::string>(
      "Adaptation Displacement Vector", "", "Name of APF displacement field");
  validPL->set<bool>(
      "Transfer IP Data",
      false,
      "Turn on solution transfer of integration point data");
  validPL->set<double>(
      "Minimum Part Density",
      1000,
      "Minimum elements per part: triggers partition shrinking");
  validPL->set<bool>(
      "Write Adapted SMB Files",
      false,
      "Write .smb mesh files after adaptation");
  validPL->set<std::string>(
      "Extruded Size Method", "SPR", "Error estimator for extruded meshes");

  /* Omega_h_Method options */
  validPL->set<double>(
      "Maximum Size",
      1.0,
      "Max element size, prevents infinity when error is zero");
  validPL->set<int>(
      "Metric Smooth Steps", 0, "Metric smoothing, number of iterations");
  validPL->set<double>(
      "Gradation Rate Limit", 1.01, "Max metric gradation rate");
  validPL->set<std::string>(
      "Size Method", "SPR", "Size field for Omega_h adaptation");
  validPL->set<double>(
      "Overshoot Allowance", 3.0, "Max allowed metric edge length");
  validPL->set<double>(
      "Max Edge Angle",
      3.14,
      "Max arc angle for mesh edge: curvature refinement");
...
static double
getAveragePartDensity(apf::Mesh* m)
{
...
static int
getShrinkFactor(apf::Mesh* m, double minPartDensity)
{
...
static void
warnAboutShrinking(int factor)
{
  int nprocs = PCU_Comm_Peers() / factor;
  if (!PCU_Comm_Self())
    fprintf(
        stderr,
        "sensing mesh is spread too thin: adapting with %d procs\n",
        nprocs);
}

void
adaptShrunken(apf::Mesh2* m, double minPartDensity, Parma_GroupCode& callback)
{
...
// rc-todo Can I deal with the "APF warning: 2 empty parts" issue by looping,
// too?
namespace al {

typedef double
ExtremumFn(const double, const double);
inline double
...
void
dispExtremum(
    const Teuchos::ArrayRCP<const double>& x,
    const int                              dim,
    const std::string&                     extremum_str,
    const Teuchos::EReductionType          rt)
{
  double            my_vals[3], global_vals[3];
  const std::size_t nx = x.size() / dim;
  for (int j = 0; j < dim; ++j) { my_vals[j] = x[j]; }
  ...
  if (comm->getRank() == 0) {
    ...
    for (int j = 0; j < dim; ++j) { std::cout << " " << global_vals[j]; }
    std::cout << std::endl;
  }
}
...
anlzCoords(const Teuchos::RCP<const Albany::PUMIDiscretization>& pumi_disc)
{
  ...
  const Teuchos::RCP<const Thyra_Vector> soln =
      pumi_disc->getSolutionField(true);
...
void
writeMesh(const Teuchos::RCP<Albany::PUMIDiscretization>&)
{
}
...
struct CoordState
{
 ...
 public:
  ...
  CoordState(const Teuchos::RCP<Albany::PUMIDiscretization>& pumi_disc)
    ...
    const int n_iterations_to_fail)
{
  ...
  for (int it = 0;;) {
    ...
    ++it;
    const long n_negative_simplices =
        apf::verifyVolumes(pumi_disc->getPUMIMeshStruct()->getMesh(), false);
    // Adjust alpha bounds.
    ...
    if (Teuchos::DefaultComm<int>::getComm()->getRank() == 0) {
      ...
      std::cout << "amb: findAlpha it " << std::setw(2) << it
                << " negative volumes " << std::setw(4) << n_negative_simplices
                << " alpha " << std::setw(w) << alpha << " in [" << std::setw(w)
                << alpha_lo << ", " << std::setw(w) << alpha_hi << "]\n";
    }
