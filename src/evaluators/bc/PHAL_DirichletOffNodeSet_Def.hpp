...
// **********************************************************************
template <typename Traits>
DirichletOffNodeSet<PHAL::AlbanyTraits::Tangent, Traits>::DirichletOffNodeSet(
    Teuchos::ParameterList& p)
...
template <typename Traits>
void
DirichletOffNodeSet<PHAL::AlbanyTraits::Tangent, Traits>::evaluateFields(
    typename Traits::EvalData dirichletWorkset)
{
  // Gather all node IDs from all the stored nodesets
  std::set<int> nodeSetsRows;
  ...
  LO num_local_dofs =
      fp != Teuchos::null ?
          Albany::getSpmdVectorSpace(fp->range())->localSubDim() :
          (JV != Teuchos::null ?
               Albany::getSpmdVectorSpace(JV->range())->localSubDim() :
               (f != Teuchos::null ?
                    Albany::getSpmdVectorSpace(f->space())->localSubDim() :
                    0));
  for (LO row = 0; row < num_local_dofs; ++row) {
    if (nodeSetsRows.find(row) == nodeSetsRows.end()) {
      // It's a row not on the given node sets
      ...
      if (fp != Teuchos::null) {
        for (int i = 0; i < dirichletWorkset.num_cols_p; i++) {
          fp_nonconst2dView[i][row] =
              -this->value.dx(dirichletWorkset.param_offset + i);
        }
      }
    }
  }
...
template <typename Traits>
void
DirichletOffNodeSet<PHAL::AlbanyTraits::DistParamDeriv, Traits>::evaluateFields(
    typename Traits::EvalData dirichletWorkset)
{
  // Gather all node IDs from all the stored nodesets
  std::set<int> nodeSetsRows;
