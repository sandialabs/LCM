...
template <typename Traits>
void
ScatterScalarResponse<PHAL::AlbanyTraits::Tangent, Traits>::postEvaluate(
    typename Traits::PostEvalData workset)
{
  ...
  if (g != Teuchos::null) { g_nonconstView = Albany::getNonconstLocalData(g); }
  ...
  for (PHAL::MDFieldIterator<const ScalarT> gr(this->global_response);
    ...
    if (g != Teuchos::null) { g_nonconstView[res] = val.val(); }
