    ...
    const Teuchos::RCP<Albany::Layouts>& dl_side)
{
  ...
  coords_side_vertices = decltype(coords_side_vertices)(
      p.get<std::string>("Coordinate Vector Vertex Name"),
  ...
  ref_cub_points =
      Kokkos::DynRankView<RealType, PHX::Device>("XXX", numSideQPs, sideDim);
  ref_weights = Kokkos::DynRankView<RealType, PHX::Device>(
      "XXX", numSideQPs);  // Not needed per se, but need to be passed to the
  ...
  Teuchos::RCP<shards::CellTopology> cellType =
      p.get<Teuchos::RCP<shards::CellTopology>>("Cell Type");
  ...
  for (int side = 0; side < numSides; ++side) {
    ...
    auto sideBasis = Albany::getIntrepid2Basis(
        *cellType->getCellTopologyData(sideDim, side));
    numSideVertices[side]   = cellType->getVertexCount(sideDim, side);
    phi_at_cub_points[side] = Kokkos::DynRankView<RealType, PHX::Device>(
        "XXX", numSideVertices[side], numSideQPs);
    sideBasis->getValues(
        phi_at_cub_points[side], ref_cub_points, Intrepid2::OPERATOR_VALUE);
  }
...
template <typename EvalT, typename Traits>
void
MapToPhysicalFrameSide<EvalT, Traits>::evaluateFields(
      ...
      for (int dim = 0; dim < numDim; ++dim) {
        ...
        for (int v = 0; v < numSideVertices[side]; ++v)
          coords_side_qp(cell, side, qp, dim) +=
              coords_side_vertices(cell, side, v, dim) *
              phi_at_cub_points[side](v, qp);
      }
    }
  }
}
