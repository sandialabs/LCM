    ...
    const Teuchos::RCP<Albany::Layouts>& dl)
{
  ...
  // Build output fields
  sideCoordVec = decltype(sideCoordVec)(
      p.get<std::string>("Side Coordinate Vector Name"),
      dl_side->vertices_vector);
  tangents = decltype(tangents)(
      p.get<std::string>("Tangents Name"), dl_side->qp_tensor_cd_sd);
  metric =
      decltype(metric)(p.get<std::string>("Metric Name"), dl_side->qp_tensor);
  w_measure = decltype(w_measure)(
      p.get<std::string>("Weighted Measure Name"), dl_side->qp_scalar);
  inv_metric = decltype(inv_metric)(
      p.get<std::string>("Inverse Metric Name"), dl_side->qp_tensor);
  metric_det = decltype(metric_det)(
      p.get<std::string>("Metric Determinant Name"), dl_side->qp_scalar);
  BF     = decltype(BF)(p.get<std::string>("BF Name"), dl_side->node_qp_scalar);
  GradBF = decltype(GradBF)(
      p.get<std::string>("Gradient BF Name"), dl_side->node_qp_gradient);
  ...
  if (compute_normals) {
    normals = decltype(normals)(
        p.get<std::string>("Side Normal Name"), dl_side->qp_vector_spacedim);
    coordVec = decltype(coordVec)(
        p.get<std::string>("Coordinate Vector Name"), dl->vertices_vector);
  ...
  numCellDims  = dl_side->vertices_vector->extent(
      3);  // Vertices vector always has the ambient space dimension
  numSideDims = numCellDims - 1;

  cubature =
      p.get<Teuchos::RCP<Intrepid2::Cubature<PHX::Device>>>("Cubature Side");
  intrepidBasis =
...
template <typename EvalT, typename Traits>
void
ComputeBasisFunctionsSide<EvalT, Traits>::postRegistrationSetup(
    ...
    PHX::FieldManager<Traits>& fm)
{
  ...
  cub_points = Kokkos::DynRankView<RealType, PHX::Device>(
      "XXX", numSideQPs, numSideDims);
  cub_weights = Kokkos::DynRankView<RealType, PHX::Device>("XXX", numSideQPs);
  val_at_cub_points = Kokkos::DynRankView<RealType, PHX::Device>(
      "XXX", numSideNodes, numSideQPs);
  grad_at_cub_points = Kokkos::DynRankView<RealType, PHX::Device>(
      "XXX", numSideNodes, numSideQPs, numSideDims);
  cub_weights = Kokkos::DynRankView<RealType, PHX::Device>("XXX", numSideQPs);
  ...
  intrepidBasis->getValues(
      val_at_cub_points, cub_points, Intrepid2::OPERATOR_VALUE);
  intrepidBasis->getValues(
      grad_at_cub_points, cub_points, Intrepid2::OPERATOR_GRAD);
    ...
    for (int itan = 0; itan < numSideDims; ++itan) {
      for (int icoor = 0; icoor < numCellDims; ++icoor) {
        for (int qp = 0; qp < numSideQPs; ++qp) {
          ...
          for (int node = 0; node < numSideNodes; ++node) {
            tangents(cell, side, qp, icoor, itan) +=
                sideCoordVec(cell, side, node, icoor) *
                grad_at_cub_points(node, qp, itan);
          }
        }
      }
    }
    ...
    for (int qp = 0; qp < numSideQPs; ++qp) {
      for (int idim = 0; idim < numSideDims; ++idim) {
        ...
        for (int coor = 0; coor < numCellDims; ++coor) {
          metric(cell, side, qp, idim, idim) +=
              tangents(cell, side, qp, coor, idim) *
              tangents(cell, side, qp, coor, idim);  // g = J'*J
        }
        ...
        for (int jdim = idim + 1; jdim < numSideDims; ++jdim) {
          ...
          for (int coor = 0; coor < numCellDims; ++coor) {
            metric(cell, side, qp, idim, jdim) +=
                tangents(cell, side, qp, coor, idim) *
                tangents(cell, side, qp, coor, jdim);  // g = J'*J
          }
          metric(cell, side, qp, jdim, idim) =
              metric(cell, side, qp, idim, jdim);
        }
      }
    }
      ...
      case 1:
        for (int qp = 0; qp < numSideQPs; ++qp) {
          ...
          w_measure(cell, side, qp) =
              cub_weights(qp) * std::sqrt(metric(cell, side, qp, 0, 0));
      ...
      case 2:
        for (int qp = 0; qp < numSideQPs; ++qp) {
          metric_det(cell, side, qp) =
              metric(cell, side, qp, 0, 0) * metric(cell, side, qp, 1, 1) -
              metric(cell, side, qp, 0, 1) * metric(cell, side, qp, 1, 0);
          w_measure(cell, side, qp) =
              cub_weights(qp) * std::sqrt(metric_det(cell, side, qp));
          inv_metric(cell, side, qp, 0, 0) =
              metric(cell, side, qp, 1, 1) / metric_det(cell, side, qp);
          inv_metric(cell, side, qp, 1, 1) =
              metric(cell, side, qp, 0, 0) / metric_det(cell, side, qp);
          inv_metric(cell, side, qp, 0, 1) = inv_metric(cell, side, qp, 1, 0) =
              -metric(cell, side, qp, 0, 1) / metric_det(cell, side, qp);
        }
        break;
      default:
        ALBANY_ABORT("Error! The dimension of the side should be 1 or 2.\n");
    }
      ...
      for (int qp = 0; qp < numSideQPs; ++qp) {
        for (int ider = 0; ider < numSideDims; ++ider) {
          ...
          for (int jder = 0; jder < numSideDims; ++jder)
            GradBF(cell, side, node, qp, ider) +=
                inv_metric(cell, side, qp, ider, jder) *
                grad_at_cub_points(node, qp, jder);
        }
      }
    }
  }
    ...
    for (int side = 0; side < numSides; ++side) {
      ...
      Kokkos::DynRankView<MeshScalarT, PHX::Device> normal_lengths =
          Kokkos::createDynRankView(
              sideCoordVec.get_view(), "normal_lengths", numCells_, numSideQPs);
      Kokkos::DynRankView<MeshScalarT, PHX::Device> normals_view =
          Kokkos::createDynRankView(
              sideCoordVec.get_view(),
              "normals",
              numCells_,
              numSideQPs,
              numCellDims);
      ...
      Kokkos::DynRankView<RealType, PHX::Device> refPointsSide(
          "refPointsSide", numSideQPs, numCellDims);
      Kokkos::DynRankView<MeshScalarT, PHX::Device> physPointsCell =
          Kokkos::createDynRankView(
              coordVec.get_view(), "XXX", numCells_, numNodes, numCellDims);
      ...
      // direction vector
      Intrepid2::CellTools<PHX::Device>::getPhysicalSideNormals(
          normals_view, jacobian_side, side, *cellType);

      // scale normals (unity)
      Intrepid2::RealSpaceTools<PHX::Device>::vectorNorm(
          normal_lengths, normals_view, Intrepid2::NORM_TWO);
      Intrepid2::FunctionSpaceTools<PHX::Device>::scalarMultiplyDataData(
          normals_view, normal_lengths, normals_view, true);

      for (int icoor = 0; icoor < numCellDims; ++icoor)
        for (int qp = 0; qp < numSideQPs; ++qp)
          for (std::size_t iCell = 0; iCell < numCells_; ++iCell)
            normals(cellVec(iCell), side, qp, icoor) =
                normals_view(iCell, qp, icoor);
    }
  }
}
