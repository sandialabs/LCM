...
ExtrudedAdapt::~ExtrudedAdapt() { delete helper; }

void
ExtrudedAdapt::setParams(const Teuchos::RCP<Teuchos::ParameterList>& p)
{
  std::string size_method = p->get<std::string>("Extruded Size Method", "SPR");
  if (size_method == "SPR")
...
void
ExtrudedAdapt::preProcessOriginalMesh()
{
  ...
  if (spr_helper) {
    ...
    std::string flat_name =
        ma::getFlatName(Albany::APFMeshStruct::solution_name[0], nlayers - 1);
...
void
ExtrudedAdapt::preProcessShrunkenMesh()
{
...
void
ExtrudedAdapt::postProcessShrunkenMesh()
{
...
void
ExtrudedAdapt::postProcessFinalMesh()
{
