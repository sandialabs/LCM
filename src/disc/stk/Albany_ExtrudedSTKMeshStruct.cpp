      ...
      periodic(false)
{
  ...
  ssPartVecLateral[ssnLat] =
      &metaData->declare_part(ssnLat, metaData->side_rank());
  ssPartVec[ssnBottom] =
      &metaData->declare_part(ssnBottom, metaData->side_rank());
  ...
  basalMeshStruct = Teuchos::rcp_dynamic_cast<Albany::AbstractSTKMeshStruct>(
      inputBasalMesh, false);
  TEUCHOS_TEST_FOR_EXCEPTION(
      basalMeshStruct == Teuchos::null,
      std::runtime_error,
      "Error! Could not cast basal mesh to AbstractSTKMeshStruct.\n");

  stk::mesh::MetaData& metaData2D =
      *basalMeshStruct->metaData;  // bulkData2D.mesh_meta_data();
  ...
  for (auto part : partvec) {
    if (part->primary_entity_rank() == metaData2D.side_rank()) {
      ...
      ssPartVecLateral[partName] =
          &metaData->declare_part(partName, metaData->side_rank());
#ifdef ALBANY_SEACAS
  ...
  for (auto it : ssPartVecLateral) ssPartVec[it.first] = it.second;
  ...
  if (shape == "Tetrahedron") {
    ...
    basalside_elem_name =
        shards::getCellTopologyData<shards::Triangle<3>>()->name;
  } else if (shape == "Wedge") {
    ElemShape = Wedge;
    basalside_elem_name =
        shards::getCellTopologyData<shards::Triangle<3>>()->name;
    ...
    basalside_elem_name =
        shards::getCellTopologyData<shards::Quadrilateral<4>>()->name;
  } else
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        Teuchos::Exceptions::InvalidParameterValue,
        std::endl
            << "Error in ExtrudedSTKMeshStruct: Element Shape " << shape
            << " not recognized. Possible values: Tetrahedron, Wedge, "
               "Hexahedron");

  std::string elem2d_name(basalMeshStruct->getMeshSpecs()[0]->ctd.base->name);
  TEUCHOS_TEST_FOR_EXCEPTION(
      basalside_elem_name != elem2d_name,
      Teuchos::Exceptions::InvalidParameterValue,
      std::endl
          << "Error in ExtrudedSTKMeshStruct: Expecting topology name of "
             "elements of 2d mesh to be "
          << basalside_elem_name << " but it is " << elem2d_name);
  ...
  Ordering  = params->get("Columnwise Ordering", false) ?
  ...
  int worksetSize      = this->computeWorksetSize(
      worksetSizeMax, basalWorksetSize * numElemsInColumn);
...
void
Albany::ExtrudedSTKMeshStruct::setFieldAndBulkData(
    const Teuchos::RCP<const Teuchos_Comm>&                   comm,
    const Teuchos::RCP<Teuchos::ParameterList>&               params,
        ...
        AbstractFieldContainer::FieldContainerRequirements>& side_set_req)
{
  ...
  if (params->isSublist("Side Set Discretizations")) {
    params2D = Teuchos::rcp(new Teuchos::ParameterList(
        params->sublist("Side Set Discretizations").sublist("basalside")));
  } else {
    // Old style: the 2D parameter are mixed with the 3D
    params2D = Teuchos::rcp(new Teuchos::ParameterList());
    params2D->set("Use Serial Mesh", params->get("Use Serial Mesh", false));
    params2D->set(
        "Exodus Input File Name",
        params->get("Exodus Input File Name", "IceSheet.exo"));
  }
  Teuchos::RCP<Albany::StateInfoStruct> dummy_sis =
  ...
  this->sideSetMeshStructs.at("basalside")
      ->setFieldAndBulkData(
          comm, params2D, neq_, basal_req, basal_sis, worksetSize);
  ...
  if (useGlimmerSpacing)
    for (int i = 0; i < numLayers + 1; i++)
      levelsNormalizedThickness[numLayers - i] =
          1.0 - (1.0 - std::pow(double(i) / numLayers + 1.0, -2)) /
                    (1.0 - std::pow(2.0, -2));
  else  // uniform layers
    for (int i = 0; i < numLayers + 1; i++)
  ...
  stk::mesh::Selector select_owned_in_part =
      stk::mesh::Selector(metaData2D.universal_part()) &
      stk::mesh::Selector(metaData2D.locally_owned_part());

  stk::mesh::Selector select_overlap_in_part =
      stk::mesh::Selector(metaData2D.universal_part()) &
      (stk::mesh::Selector(metaData2D.locally_owned_part()) |
       stk::mesh::Selector(metaData2D.globally_shared_part()));

  std::vector<stk::mesh::Entity> cells2D;
  stk::mesh::get_selected_entities(
      select_overlap_in_part,
      bulkData2D.buckets(stk::topology::ELEMENT_RANK),
      cells2D);

  std::vector<stk::mesh::Entity> nodes2D;
  stk::mesh::get_selected_entities(
      select_overlap_in_part,
      bulkData2D.buckets(stk::topology::NODE_RANK),
      nodes2D);

  // For sides, use the 'select_owned_in_part', since boundary sides should
  // belong to just one rank. This for sure eliminate the risk of finding a side
  // with only one element connected, but that is on an 'internal' sideset,
  // which happens to be at the interface of 2 ranks. Note: I'm not 100% sure
  // those internal interface sides would return 1 as the number of connected
  //       elements (they may return 2, storing the ghost), but just in case.
  std::vector<stk::mesh::Entity> sides2D;
  stk::mesh::get_selected_entities(
      select_owned_in_part,
      bulkData2D.buckets(metaData2D.side_rank()),
      sides2D);

  // std::cout << "Num Global Elements: " << maxGlobalElements2D<< " " <<
  // maxGlobalVertices2dId<< " " << maxGlobalSides2D << std::endl;

  Teuchos::Array<Tpetra_GO> indices(nodes2D.size());
  for (size_t i = 0; i < nodes2D.size(); ++i) {
    indices[i] = bulkData2D.identifier(nodes2D[i]) - 1;
  }
  Teuchos::RCP<const Tpetra_Map> nodes_map =
      Tpetra::createNonContigMapWithNode<LO, Tpetra_GO, KokkosNode>(
          indices(), comm);
  ...
  Teuchos::RCP<const Tpetra_Map> cells_map =
      Tpetra::createNonContigMapWithNode<LO, Tpetra_GO, KokkosNode>(
          indices(), comm);
  ...
  Teuchos::RCP<const Tpetra_Map> sides_map =
      Tpetra::createNonContigMapWithNode<LO, Tpetra_GO, KokkosNode>(
          indices(), comm);
  ...
  this->layered_mesh_numbering =
      (Ordering == LAYER) ?
          Teuchos::rcp(new LayeredMeshNumbering<LO>(
              lVertexColumnShift, Ordering, layerThicknessRatio)) :
          Teuchos::rcp(new LayeredMeshNumbering<LO>(
              vertexLayerShift, Ordering, layerThicknessRatio));

  std::vector<double> ltr(layerThicknessRatio.size());
  for (size_t i = 0; i < ltr.size(); ++i) { ltr[i] = layerThicknessRatio[i]; }
  fieldContainer->getMeshVectorStates()["layer_thickness_ratio"] = ltr;
  fieldContainer->getMeshScalarIntegerStates()["ordering"] =
      static_cast<int>(Ordering);
  fieldContainer->getMeshScalarIntegerStates()["stride"] =
      (Ordering == LAYER) ? lVertexColumnShift : vertexLayerShift;
  ...
  std::string thickness_name =
      params->get<std::string>("Thickness Field Name", "thickness");
  std::string surface_height_name =
      params->get<std::string>("Surface Height Field Name", "surface_height");

  ScalarFieldType* surface_height_field = metaData2D.get_field<ScalarFieldType>(
      stk::topology::NODE_RANK, surface_height_name);
  ScalarFieldType* thickness_field = metaData2D.get_field<ScalarFieldType>(
      stk::topology::NODE_RANK, thickness_name);
  ...
  stk::mesh::FieldBase const* coordinates_field2d =
      metaData2D.coordinate_field();

  std::vector<GO> prismMpasIds(NumBaseElemeNodes),
      prismGlobalIds(2 * NumBaseElemeNodes);
  ...
  *out << "[ExtrudedSTKMesh] Adding nodes... ";
  ...
  for (int i = 0; i < num_nodes; i++) {
    int ib = (Ordering == LAYER) * (i % lVertexColumnShift) +
             (Ordering == COLUMN) * (i / vertexLayerShift);
    int il = (Ordering == LAYER) * (i / lVertexColumnShift) +
             (Ordering == COLUMN) * (i % vertexLayerShift);
    ...
    if (il == 0)
      node = bulkData->declare_entity(
          stk::topology::NODE_RANK, nodeId, singlePartVecBottom);
    else if (il == numLayers)
      node = bulkData->declare_entity(
          stk::topology::NODE_RANK, nodeId, singlePartVecTop);
    else
      node = bulkData->declare_entity(
          stk::topology::NODE_RANK, nodeId, nodePartVec);
    ...
    double const* coord2d =
        (double const*)stk::mesh::field_data(*coordinates_field2d, node2d);
    coord[0] = coord2d[0];
    coord[1] = coord2d[1];
    ...
    coord[2] =
        sHeight_val[0] - thick_val[0] * (1. - levelsNormalizedThickness[il]);
  }
  ...
  *out << "[ExtrudedSTKMesh] Adding elements... ";
  ...
  for (int i = 0; i < num_cells; i++) {
    int ib = (Ordering == LAYER) * (i % lElemColumnShift) +
             (Ordering == COLUMN) * (i / elemLayerShift);
    int il = (Ordering == LAYER) * (i / lElemColumnShift) +
             (Ordering == COLUMN) * (i % elemLayerShift);
    int shift = il * vertexColumnShift;

    // TODO: this could be done only in the first layer and then copied into the
    // other layers
    ...
    switch (ElemShape) {
      case Tetrahedron: {
        tetrasFromPrismStructured(
            &prismMpasIds[0], &prismGlobalIds[0], tetrasLocalIdsOnPrism);

        stk::mesh::EntityId prismId =
            il * elemColumnShift +
            elemLayerShift * (bulkData2D.identifier(cells2D[ib]) - 1);
        for (int iTetra = 0; iTetra < 3; iTetra++) {
          ...
          for (int j = 0; j < 4; j++) {
            stk::mesh::Entity node = bulkData->get_entity(
                stk::topology::NODE_RANK, tetrasLocalIdsOnPrism[iTetra][j] + 1);
      ...
      case Hexahedron: {
        stk::mesh::EntityId prismId =
            ...
            elemLayerShift * (bulkData2D.identifier(cells2D[ib]) - 1);
        stk::mesh::Entity elem = bulkData->declare_entity(
            stk::topology::ELEMENT_RANK, prismId + 1, singlePartVec);
        for (int j = 0; j < 2 * NumBaseElemeNodes; j++) {
          stk::mesh::Entity node = bulkData->get_entity(
              stk::topology::NODE_RANK, prismGlobalIds[j] + 1);
          bulkData->declare_relation(elem, node, j);
        }
        int* p_rank = (int*)stk::mesh::field_data(*proc_rank_field, elem);
        p_rank[0]   = comm->getRank();
  ...
  *out << "done!\n";
  ...
  switch (ElemShape) {
    case Tetrahedron:
      ...
      basalSideLID = 3;  // depends on how the tetrahedron is located in the
                         // Prism, see tetraFaceIdOnPrismFaceId below.
      upperSideLID = 1;
      break;
    case Wedge:
      numSubelemOnPrism  = 1;
      numBasalSidePoints = 3;
      basalSideLID =
          3;  // depends on how the tetrahedron is located in the Prism.
      upperSideLID = 4;
      break;
    case Hexahedron:
      numSubelemOnPrism  = 1;
      ...
      basalSideLID =
          4;  // depends on how the tetrahedron is located in the Prism.
      upperSideLID = 5;
      break;
  }

  // First, we store the lower and upper faces of prisms, which corresponds to
  // triangles of the basal mesh Note: this ensures the side_id on basal sides
  // equals the corresponding cell_id on the basal mesh
  //       which is useful in side_discretization handling.
  // WARNING: these sides do NOT follow column-wise ordering (even if
  // requested).
  ...
  *out << "[ExtrudedSTKMesh] Adding basalside sides... ";
  ...
  for (const auto& elem2d : cells2D) {
    ...
    stk::mesh::Entity   side      = bulkData->declare_entity(
        metaData->side_rank(), elem2d_id + 1, singlePartVec);
    stk::mesh::Entity elem = bulkData->get_entity(
        stk::topology::ELEMENT_RANK,
        elem2d_id * numSubelemOnPrism * elemLayerShift + 1);
    ...
    for (int j = 0; j < numBasalSidePoints; j++) {
      stk::mesh::Entity node =
          rel_elemNodes[this->meshSpecs[0]->ctd.side[basalSideLID].node[j]];
  ...
  *out << "[ExtrudedSTKMesh] Adding upperside sides... ";
  ...
  for (const auto& elem2d : cells2D) {
    stk::mesh::EntityId elem2d_id = bulkData2D.identifier(elem2d) - 1;
    stk::mesh::Entity   side      = bulkData->declare_entity(
        metaData->side_rank(), elem2d_id + upperBasalOffset + 1, singlePartVec);
    stk::mesh::Entity elem = bulkData->get_entity(
        stk::topology::ELEMENT_RANK,
        elem2d_id * numSubelemOnPrism * elemLayerShift +
            (numLayers - 1) * numSubelemOnPrism * elemColumnShift + 1 +
            (numSubelemOnPrism - 1));
    ...
    for (int j = 0; j < numBasalSidePoints; j++) {
      stk::mesh::Entity node =
          rel_elemNodes[this->meshSpecs[0]->ctd.side[upperSideLID].node[j]];
      bulkData->declare_relation(side, node, j);
    }
  }
  ...
  *out << "done!\n";
  ...
  // The following two arrays have being computed offline using the computeMap
  // function in .hpp file.

  // tetraFaceIdOnPrismFaceId[ minIndex ][ PrismFaceID ]
  int tetraFaceIdOnPrismFaceId[6][5] = {{0, 1, 2, 3, 1},
                                        {2, 0, 1, 3, 1},
                                        {1, 2, 0, 3, 1},
                                        {2, 1, 0, 1, 3},
                                        {0, 2, 1, 1, 3},
  ...
  // tetraAdjacentToPrismLateralFace[ minIndex ][ prismType ][ PrismFaceID ][
  // iTetra ]. There are to Terahedra adjacent to a Prism face. iTetra in {0,1}
  int tetraAdjacentToPrismLateralFace[6][2][3][2] = {
      {{{1, 2}, {0, 1}, {0, 2}}, {{0, 2}, {0, 1}, {1, 2}}},
  ...
  *out << "[ExtrudedSTKMesh] Adding lateral sides... ";
  ...
  for (int i = 0; i < num_sides; i++) {
    int ib = (Ordering == LAYER) * (i % lsideColumnShift) +
             (Ordering == COLUMN) * (i / sideLayerShift);
    // if(!isBoundaryside[ib]) continue; //WARNING: assuming that all the edges
    // stored are boundary edges!!

    stk::mesh::Entity side2d = sides2D[ib];

    // We are extracting *all* sides in the basal mesh that are owned by this
    // rank. It could happen that the basal mesh also stores some 'internal'
    // sides. We need to skip those, so check how many elements is this side
    // connected to.
    if (bulkData2D.num_elements(side2d) > 1) { continue; }

    stk::mesh::Entity const* rel = bulkData2D.begin_elements(side2d);
    stk::mesh::ConnectivityOrdinal const* ordinals =
        bulkData2D.begin_element_ordinals(side2d);

    int il = (Ordering == LAYER) * (i / lsideColumnShift) +
             (Ordering == COLUMN) * (i % sideLayerShift);
    stk::mesh::Entity   elem2d = rel[0];
    stk::mesh::EntityId sideLID =
        ordinals[0];  // bulkData2D.identifier(rel[0]);
    ...
    switch (ElemShape) {
      case Tetrahedron: {
        ...
        stk::mesh::EntityId sideId = 2 * sideColumnShift * il +
                                     2 * side2dId * sideLayerShift +
                                     upperBasalOffset + 1;
        stk::mesh::Entity side0 = bulkData->declare_entity(
            metaData->side_rank(), sideId, singlePartVec);
        stk::mesh::Entity side1 = bulkData->declare_entity(
            metaData->side_rank(), sideId + 1, singlePartVec);

        int                 minIndex;
        int                 pType = prismType(&prismMpasIds[0], minIndex);
        stk::mesh::EntityId tetraId =
            3 * il * elemColumnShift + 3 * elemLayerShift * basalElemId;

        stk::mesh::Entity elem0 = bulkData->get_entity(
            stk::topology::ELEMENT_RANK,
            tetraId +
                tetraAdjacentToPrismLateralFace[minIndex][pType][sideLID][0] +
                1);
        stk::mesh::Entity elem1 = bulkData->get_entity(
            stk::topology::ELEMENT_RANK,
            tetraId +
                tetraAdjacentToPrismLateralFace[minIndex][pType][sideLID][1] +
                1);

        bulkData->declare_relation(
            elem0, side0, tetraFaceIdOnPrismFaceId[minIndex][sideLID]);
        bulkData->declare_relation(
            elem1, side1, tetraFaceIdOnPrismFaceId[minIndex][sideLID]);
        ...
        for (int j = 0; j < 3; j++) {
          stk::mesh::Entity node0 = rel_elemNodes0
              [this->meshSpecs[0]
                   ->ctd.side[tetraFaceIdOnPrismFaceId[minIndex][sideLID]]
                   .node[j]];
          bulkData->declare_relation(side0, node0, j);
          bulkData->change_entity_parts(node0, singlePartVecLateral);
          stk::mesh::Entity node1 = rel_elemNodes1
              [this->meshSpecs[0]
                   ->ctd.side[tetraFaceIdOnPrismFaceId[minIndex][sideLID]]
                   .node[j]];
      ...
      case Wedge:
      case Hexahedron: {
        stk::mesh::EntityId sideId = sideColumnShift * il +
                                     side2dId * sideLayerShift +
                                     upperBasalOffset + 1;
        stk::mesh::Entity side = bulkData->declare_entity(
            metaData->side_rank(), sideId, singlePartVec);

        stk::mesh::EntityId prismId =
            il * elemColumnShift + elemLayerShift * basalElemId;
        stk::mesh::Entity elem =
            bulkData->get_entity(stk::topology::ELEMENT_RANK, prismId + 1);
        bulkData->declare_relation(elem, side, sideLID);

        stk::mesh::Entity const* rel_elemNodes = bulkData->begin_nodes(elem);
        for (int j = 0; j < 4; j++) {
          stk::mesh::Entity node =
              rel_elemNodes[this->meshSpecs[0]->ctd.side[sideLID].node[j]];
          bulkData->declare_relation(side, node, j);
  ...
  auto partvec = metaData2D.get_mesh_parts();
  for (auto part : partvec) {
    if (part->primary_entity_rank() != metaData2D.side_rank()) continue;
    stk::mesh::get_selected_entities(
        ...
        switch (ElemShape) {
          case Tetrahedron: {
            stk::mesh::EntityId sideId = 2 * sideColumnShift * il +
                                         2 * side2dId * sideLayerShift +
                                         upperBasalOffset + 1;
            stk::mesh::Entity side0 =
                bulkData->get_entity(metaData->side_rank(), sideId);
            stk::mesh::Entity side1 =
                bulkData->get_entity(metaData->side_rank(), sideId + 1);
  ...
  for (auto part : nodepartvec) {
    if (part->primary_entity_rank() != stk::topology::NODE_RANK) { continue; }
    stk::mesh::get_selected_entities(
        stk::mesh::Selector(*part),
        bulkData2D.buckets(stk::topology::NODE_RANK),
        boundaryNodes2D);
    ...
    for (const auto& node2D : boundaryNodes2D) {
      ...
      for (int il = 0; il < (numLayers + 1); ++il) {
        ...
        stk::mesh::Entity node =
            bulkData->get_entity(stk::topology::NODE_RANK, nodeId);
        bulkData->change_entity_parts(node, singlePartVecLateral);
      }
    }
  }
  ...
  *out << "done!\n";
  ...
  // Extrude fields
  extrudeBasalFields(
      nodes2D, cells2D, maxGlobalElements2dId, maxGlobalVertices2dId);
  interpolateBasalLayeredFields(
      nodes2D,
      cells2D,
      levelsNormalizedThickness,
      maxGlobalElements2dId,
      maxGlobalVertices2dId);
  ...
  // Check that the nodeset created from sidesets contain the right number of
  // nodes
  this->checkNodeSetsFromSideSetsIntegrity();

  // We can finally extract the side set meshes and set the fields and bulk data
  // in all of them
  ...
  if (params->get("Export 2D Data", false)) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        !ofile.is_open(),
        std::logic_error,
        "Error! Cannot open file 'basal_mesh.msh'.\n");
...
void
Albany::ExtrudedSTKMeshStruct::buildCellSideNodeNumerationMap(
    const std::string&              sideSetName,
    std::map<GO, GO>&               sideMap,
    std::map<GO, std::vector<int>>& sideNodeMap)
{
  if (sideSetName != "basalside" || Ordering == LayeredMeshOrdering::LAYER ||
      basalMeshStruct->side_maps_present) {
    GenericSTKMeshStruct::buildCellSideNodeNumerationMap(
        sideSetName, sideMap, sideNodeMap);
    return;
  }

  // Extract 2D cells
  stk::mesh::Selector selector =
      stk::mesh::Selector(basalMeshStruct->metaData->locally_owned_part());
  std::vector<stk::mesh::Entity> cells2D;
  stk::mesh::get_selected_entities(
      selector,
      basalMeshStruct->bulkData->buckets(stk::topology::ELEM_RANK),
      cells2D);

  // If the mesh is already partitioned, this can happen...
  if (cells2D.size() == 0) return;
  ...
  ISFT* side_to_cell_map = basalMeshStruct->metaData->get_field<ISFT>(
      stk::topology::ELEM_RANK, "side_to_cell_map");
  IVFT* side_nodes_ids_map = basalMeshStruct->metaData->get_field<IVFT>(
  ...
  for (const auto& cell2D : cells2D) {
    ...
    *cell3D_id = bulkData->identifier(cell3D);
    ...
    // Now we determine the lid of the side within the element and also the node
    // ordering
    cell2D_nodes = basalMeshStruct->bulkData->begin_nodes(cell2D);
    side3D_nodes = bulkData->begin_nodes(side3D);
    for (int i(0); i < num_nodes; ++i) {
      cell2D_nodes_ids[i] =
          basalMeshStruct->bulkData->identifier(cell2D_nodes[i]);
      side3D_nodes_ids[i] = bulkData->identifier(side3D_nodes[i]);
      // Need to recover the "original" (basal) node id that generated the
      // extrude mesh node Id; it's the id the node would have if the mesh had
      // been built with LAYER ordering..
      original_nodes_ids[i] =
          ((bulkData->identifier(side3D_nodes[i]) - 1) / (numLayers + 1)) + 1;
    }
...
void
Albany::ExtrudedSTKMeshStruct::interpolateBasalLayeredFields(
    const std::vector<stk::mesh::Entity>& nodes2d,
    const std::vector<stk::mesh::Entity>& cells2d,
    const std::vector<double>&            levelsNormalizedThickness,
    GO                                    maxGlobalCells2dId,
    GO                                    maxGlobalNodes2dId)
{
  ...
  if (params->isParameter("Interpolate Basal Node Layered Fields")) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        !params->isParameter("Basal Node Layered Fields Ranks"),
        std::logic_error,
        "Error! To interpolate basal node layered fields, you also need the "
        "'Basal Node Layered Fields Ranks' parameter.\n");
    node_fields_names = params->get<Teuchos::Array<std::string>>(
        "Interpolate Basal Node Layered Fields");
    node_fields_ranks =
        params->get<Teuchos::Array<int>>("Basal Node Layered Fields Ranks");
  }

  if (params->isParameter("Interpolate Basal Elem Layered Fields")) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        !params->isParameter("Basal Elem Layered Fields Ranks"),
        std::logic_error,
        "Error! To interpolate basal elem layered fields, you also need the "
        "'Basal Elem Layered Fields Ranks' parameter.\n");
    cell_fields_names = params->get<Teuchos::Array<std::string>>(
        "Interpolate Basal Elem Layered Fields");
    cell_fields_ranks =
        params->get<Teuchos::Array<int>>("Basal Elem Layered Fields Ranks");
  }

  if (node_fields_names.size() == 0 && cell_fields_names.size() == 0) return;
  ...
  *out << "[ExtrudedSTKMesh] Interpolating layered basal fields...\n";
  ...
  for (int ifield = 0; ifield < numNodeFields; ++ifield) {
    *out << "  - Interpolating " << ranks[node_fields_ranks[ifield]]
         << " field " << node_fields_names[ifield] << "...";
    out->getOStream()->flush();

    // We also need to load the normalized layers coordinates
    std::string tmp_str = node_fields_names[ifield] + "_NLC";
    auto it = basalMeshStruct->getFieldContainer()->getMeshVectorStates().find(
        tmp_str);
    TEUCHOS_TEST_FOR_EXCEPTION(
        it == basalMeshStruct->getFieldContainer()->getMeshVectorStates().end(),
        std::logic_error,
        "Error in ExtrudedSTKMeshStruct: normalized layers coords for layered "
        "field '"
    ...
    for (int inode = 0; inode < numNodes2d; ++inode) {
      ...
      switch (node_fields_ranks[ifield]) {
        case 1: {
          VFT* field2d = metaData2d.get_field<VFT>(
              stk::topology::NODE_RANK, node_fields_names[ifield]);
          TEUCHOS_TEST_FOR_EXCEPTION(
              field2d == 0,
              std::runtime_error,
              "Error! Cannot interpolate layered field '"
                  << node_fields_names[ifield]
                  << "' since it is not present in the 2d mesh.\n");
          values2d = stk::mesh::field_data(*field2d, node2d);
          break;
        }
        case 2: {
          TFT* field2d = metaData2d.get_field<TFT>(
              stk::topology::NODE_RANK, node_fields_names[ifield]);
          TEUCHOS_TEST_FOR_EXCEPTION(
              field2d == 0,
              std::runtime_error,
        ...
        default:
          TEUCHOS_TEST_FOR_EXCEPTION(
              true,
              std::logic_error,
              "Error! Invalid/unsupported field rank.\n");
      }

      // Loop on the layers
      for (int il = 0; il <= numLayers; ++il) {
        // Retrieve 3D node
        int node3dId = Ordering == COLUMN ?
                           node2dId * (numLayers + 1) + il + 1 :
                           il * maxGlobalNodes2dId + node2dId + 1;
        stk::mesh::Entity node3d =
            bulkData->get_entity(stk::topology::NODE_RANK, node3dId);
        ...
        auto where = std::upper_bound(
            fieldLayersCoords.begin(), fieldLayersCoords.end(), meshLayerCoord);
        il1 = std::distance(fieldLayersCoords.begin(), where);
        if (il1 == 0)  // mesh layer is below the first field layer
        {
          il0 = 0;
          h0  = 0.;  // Useless, (the 2 values in the convex combination will be
                     // the same) but for clarity we fix it to 0
        } else if (il1 == numFieldLayers)  // mesh layer is above the last field
                                           // layer
        {
          ...
          il0 = il1 - 1;
          h0  = (fieldLayersCoords[il1] - meshLayerCoord) /
               (fieldLayersCoords[il1] - fieldLayersCoords[il0]);
        }

        // Extracting 3d pointer and stuffing the right data in it
        // TODO: find a way for ExtrudedSTKMeshStruct to automatically add the
        // fields to be interpolated, so the user does not have to
        //       specify them twice (in the 2d mesh and in the 3d mesh) in the
        //       input file. Note: this must be done before you call the
        //       SetupFieldData method, which adds all the fields to the stk
        //       mesh.
        switch (node_fields_ranks[ifield]) {
          case 1: {
            SFT* field3d = metaData->get_field<SFT>(
                stk::topology::NODE_RANK, node_fields_names[ifield]);
            TEUCHOS_TEST_FOR_EXCEPTION(
                field3d == 0,
                std::runtime_error,
                "Error! Cannot interpolate layered field '"
                    << node_fields_names[ifield]
                    << "' since it is not present in the 3d mesh. Perhaps you "
                       "forgot to specify it in the section 'Required Fields "
                       "Info' of the extruded mesh?\n");
            values3d    = stk::mesh::field_data(*field3d, node3d);
            values3d[0] = h0 * values2d[il0] + (1 - h0) * values2d[il1];
            break;
          }
          case 2: {
            VFT* field3d = metaData->get_field<VFT>(
                stk::topology::NODE_RANK, node_fields_names[ifield]);
            TEUCHOS_TEST_FOR_EXCEPTION(
                field3d == 0,
                std::runtime_error,
            ...
            for (int j = 0; j < numScalars / numFieldLayers; ++j)
              values3d[j] = h0 * values2d[j * numFieldLayers + il0] +
                            (1 - h0) * values2d[j * numFieldLayers + il1];
            break;
          }
          default:
            TEUCHOS_TEST_FOR_EXCEPTION(
  ...
  for (int ifield = 0; ifield < numCellFields; ++ifield) {
    *out << "  - Interpolating " << ranks[cell_fields_ranks[ifield]]
         << " field " << cell_fields_names[ifield] << "...";
    // We also need to load the normalized layers coordinates
    ...
    for (int icell = 0; icell < numCells2d; ++icell) {
      ...
      switch (cell_fields_ranks[ifield]) {
        case 1: {
          VFT* field2d = metaData2d.get_field<VFT>(
              stk::topology::ELEMENT_RANK, cell_fields_names[ifield]);
          TEUCHOS_TEST_FOR_EXCEPTION(
              field2d == 0,
              std::runtime_error,
              "Error! Cannot interpolate layered field '"
                  << cell_fields_names[ifield]
                  << "' since it is not present in the 2d mesh.\n");
      ...
      for (int il = 0; il < numLayers; ++il) {
        ...
        stk::mesh::EntityId prismId = Ordering == COLUMN ?
                                          numLayers * cell2dId + il :
                                          maxGlobalCells2dId * il + cell2dId;
        std::vector<stk::mesh::Entity> cells3d;
        switch (ElemShape) {
          case Tetrahedron:
            cells3d.push_back(bulkData->get_entity(
                stk::topology::ELEMENT_RANK, 3 * prismId + 1));
            cells3d.push_back(bulkData->get_entity(
                stk::topology::ELEMENT_RANK, 3 * prismId + 2));
            cells3d.push_back(bulkData->get_entity(
                stk::topology::ELEMENT_RANK, 3 * prismId + 3));
            break;
          case Wedge:
          case Hexahedron:
            cells3d.push_back(
        ...
        // Since the
        double meshLayerCoord = 0.5 * (levelsNormalizedThickness[il] +
                                       levelsNormalizedThickness[il + 1]);
...
void
Albany::ExtrudedSTKMeshStruct::extrudeBasalFields(
    const std::vector<stk::mesh::Entity>& nodes2d,
    const std::vector<stk::mesh::Entity>& cells2d,
    GO                                    maxGlobalCells2dId,
    GO                                    maxGlobalNodes2dId)
{
  ...
  if (params->isParameter("Extrude Basal Node Fields")) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        !params->isParameter("Basal Node Fields Ranks"),
        std::logic_error,
        "Error! To extrude basal node fields, you also need the 'Basal Node "
        "Fields Ranks' parameter.\n");
    node_fields_names =
        params->get<Teuchos::Array<std::string>>("Extrude Basal Node Fields");
    node_fields_ranks =
        params->get<Teuchos::Array<int>>("Basal Node Fields Ranks");
  }

  if (params->isParameter("Extrude Basal Elem Fields")) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        !params->isParameter("Basal Elem Fields Ranks"),
        std::logic_error,
        "Error! To extrude basal elem fields, you also need the 'Basal Elem "
        "Fields Ranks' parameter.\n");
    cell_fields_names =
        params->get<Teuchos::Array<std::string>>("Extrude Basal Elem Fields");
    cell_fields_ranks =
        params->get<Teuchos::Array<int>>("Basal Elem Fields Ranks");
  }
    ...
    switch (node_fields_ranks[ifield]) {
      case 1: {
        *out << "  - Extruding Scalar Node field " << node_fields_names[ifield]
        ...
        SFT* field2d = metaData2d.get_field<SFT>(
            stk::topology::NODE_RANK, node_fields_names[ifield]);
        SFT* field3d = metaData->get_field<SFT>(
            stk::topology::NODE_RANK, node_fields_names[ifield]);

        // TODO: find a way for ExtrudedSTKMeshStruct to automatically add the
        // fields to be extruded, so the user does not have to
        //       specify them twice (in the 2d mesh and in the 3d mesh) in the
        //       input file. Note: this must be done before you call the
        //       SetupFieldData method, which adds all the fields to the stk
        //       mesh.
        ...
        TEUCHOS_TEST_FOR_EXCEPTION(
            ...
            "Error! Cannot extrude field '"
                << node_fields_names[ifield]
                << "' since it is not present in the 3d mesh. Perhaps you "
                   "forgot to specify it in the section 'Required Fields Info' "
        ...
        *out << "  - Extruding Vector Node field " << node_fields_names[ifield]
        ...
        for (int inode = 0; inode < numNodes2d; ++inode) {
          ...
          for (int il = 0; il <= numLayers; ++il) {
            ...
            values3d = stk::mesh::field_data(*field3d, node3d);
            for (int j = 0; j < numScalars; ++j) values3d[j] = values2d[j];
          }
        }
        break;
      }
      case 3: {
        *out << "  - Extruding Tensor Node field " << node_fields_names[ifield]
        ...
        TFT* field2d = metaData2d.get_field<TFT>(
            stk::topology::NODE_RANK, node_fields_names[ifield]);
        TFT* field3d = metaData->get_field<TFT>(
            stk::topology::NODE_RANK, node_fields_names[ifield]);
      ...
      default:
        TEUCHOS_TEST_FOR_EXCEPTION(
            true, std::logic_error, "Error! Invalid/unsupported field rank.\n");
    }
    *out << "done!\n";
  }
        ...
        *out << "  - Extruding Scalar Elem field " << cell_fields_names[ifield]
        ...
        for (int icell = 0; icell < numCells2d; ++icell) {
          ...
          for (int il = 0; il < numLayers; ++il) {
            // Retrieving the id of the 3d cells
            stk::mesh::EntityId prismId =
                Ordering == COLUMN ? numLayers * cell2dId + il :
                                     maxGlobalCells2dId * il + cell2dId;
            std::vector<stk::mesh::Entity> cells3d;
            switch (ElemShape) {
              case Tetrahedron:
              ...
              case Hexahedron:
                cells3d.push_back(bulkData->get_entity(
                    stk::topology::ELEMENT_RANK, prismId + 1));
            }
        ...
        *out << "  - Extruding Vector Elem field " << cell_fields_names[ifield]
        ...
        for (int icell = 0; icell < numCells2d; ++icell) {
          ...
          for (int il = 0; il < numLayers; ++il) {
            // Retrieving the id of the 3d cells
            stk::mesh::EntityId prismId =
                Ordering == COLUMN ? numLayers * cell2dId + il + 1 :
                                     maxGlobalCells2dId * il + cell2dId + 1;
            std::vector<stk::mesh::Entity> cells3d;
            switch (ElemShape) {
              case Tetrahedron:
                cells3d.push_back(bulkData->get_entity(
                    stk::topology::ELEMENT_RANK, prismId + 2));
                cells3d.push_back(bulkData->get_entity(
                    stk::topology::ELEMENT_RANK, prismId + 1));
              case Wedge:
              case Hexahedron:
                cells3d.push_back(
                    bulkData->get_entity(stk::topology::ELEMENT_RANK, prismId));
            }
...
Albany::ExtrudedSTKMeshStruct::getValidDiscretizationParameters() const
{
  Teuchos::RCP<Teuchos::ParameterList> validPL =
      this->getValidGenericSTKParameters("Valid Extruded_DiscParams");
  validPL->set<bool>(
      "Export 2D Data", "", "If true, exports the 2D mesh in GMSH format");
  validPL->set<Teuchos::Array<std::string>>(
      "Extrude Basal Node Fields",
      Teuchos::Array<std::string>(),
      "List of basal node fields to be extruded");
  validPL->set<Teuchos::Array<std::string>>(
      "Extrude Basal Elem Fields",
      Teuchos::Array<std::string>(),
      "List of basal elem fields to be extruded");
  validPL->set<Teuchos::Array<int>>(
      "Basal Node Fields Ranks",
      Teuchos::Array<int>(),
      "Ranks of basal node fields to be extruded");
  validPL->set<Teuchos::Array<int>>(
      "Basal Elem Fields Ranks",
      Teuchos::Array<int>(),
      "Ranks of basal elem fields to be extruded");
  validPL->set<Teuchos::Array<std::string>>(
      "Interpolate Basal Node Layered Fields",
      Teuchos::Array<std::string>(),
      "List of basal node layered fields to be interpolated");
  validPL->set<Teuchos::Array<std::string>>(
      "Interpolate Basal Elem Layered Fields",
      Teuchos::Array<std::string>(),
      "List of basal node layered fields to be interpolated");
  validPL->set<Teuchos::Array<int>>(
      "Basal Node Layered Fields Ranks",
      Teuchos::Array<int>(),
      "List of basal node layered fields to be interpolated");
  validPL->set<Teuchos::Array<int>>(
      "Basal Elem Layered Fields Ranks",
      Teuchos::Array<int>(),
      "List of basal node layered fields to be interpolated");
  validPL->set<std::string>(
      "GMSH 2D Output File Name",
      "",
      "File Name for GMSH 2D Basal Mesh Export");
  validPL->set<std::string>(
      "Exodus Input File Name", "", "File Name For Exodus Mesh Input");
  validPL->set<std::string>(
      "Element Shape",
      "Hexahedron",
      "Shape of the Element: Tetrahedron, Wedge, Hexahedron");
  validPL->set<int>(
      "NumLayers",
      10,
      "Number of vertical Layers of the extruded mesh. In a vertical column, "
      "the mesh will have numLayers+1 nodes");
  validPL->set<bool>(
      "Use Glimmer Spacing",
      false,
      "When true, the layer spacing is computed according to Glimmer formula "
      "(layers are denser close to the bedrock)");
  validPL->set<bool>(
      "Columnwise Ordering",
      false,
  ...
  validPL->set<std::string>(
      "Thickness Field Name",
      "thickness",
      "Name of the 'thickness' field to use for extrusion");
  validPL->set<std::string>(
      "Surface Height Field Name",
      "surface_height",
      "Name of the 'surface_height' field to use for extrusion");
