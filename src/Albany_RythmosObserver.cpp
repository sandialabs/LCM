...
RythmosObserver::RythmosObserver(const Teuchos::RCP<Application>& app_)
    : impl(new ObserverImpl(app_)), initial_step(true)
{
  // Nothing to do
}

void
RythmosObserver::observeStartTimeStep(
    ...
    const int /* timeStepIter */)
{
  ...
  // IK, 7/18/14: commented out the next line.  It was causing the 1st solution
  // (initial condition) to not be written to the Exodus file.  The hack was in
  // place b/c of the return in the end of the line.
  // std::cout << "AGS:: Hack do not observe first step, time = "  << time <<
  // std::endl; return;
  ...
  const ScalarType time =
      impl->getTimeParamValueOrDefault(stepper.getStepStatus().time);
...
void
RythmosObserver::observeCompletedTimeStep(
    const Rythmos::StepperBase<ScalarType>& stepper,
    const Rythmos::StepControlInfo<ScalarType>& /* stepCtrlInfo */,
    const int /* timeStepIter */)
{
  ...
  if (solnandsens != Teuchos::null) {
    ...
    for (int i=1; i < solnandsens->productSpace()->numBlocks(); i++) {
      ...
      const Epetra_Vector sens=
  *(Thyra::get_Epetra_Vector(impl.getNonOverlappedMap(), sensvec));
