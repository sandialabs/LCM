...
\begin{figure}[tb]
  ...
  (dashed). \textbf{(a)} The frozen surface ($f = 1$) in
...
coefficients. Let $f \in [0,1]$ be the ice saturation, with $f = 1$
fully frozen and $f = 0$ fully thawed. At each integration point the
kernel evaluates every cap parameter as a function of $f$ and hands the
...
\end{itemize}
Writing $p_{\mathrm f}$ and $p_{\mathrm t}$ for the frozen and thawed
values of a parameter, these rules are
\begin{equation*}
  p(f) = (1-f)\,p_{\mathrm t} + f\,p_{\mathrm f}
  \quad\text{for } p \in \{A,\,C,\,N,\,\kappa_0,\,W,\,D_1,\,c^\alpha,\,K\},
  \qquad
  G(f) = G_{\mathrm t}^{\,1-f}\,G_{\mathrm f}^{\,f},
\end{equation*}
...
frozen ($f = 1$) or thawed ($f = 0$) sublist exactly, so either limit
