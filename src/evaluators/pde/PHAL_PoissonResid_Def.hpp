...
PHAL::PoissonResid<EvalT, Traits>::PoissonResid(Teuchos::ParameterList const& p, const Teuchos::RCP<Albany::Layouts>& dl)
...
template <typename EvalT, typename Traits>
void
PHAL::PoissonResid<EvalT, Traits>::postRegistrationSetup(typename Traits::SetupData d, PHX::FieldManager<Traits>& fm)
{
...
template <typename EvalT, typename Traits>
void
PHAL::PoissonResid<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  ...
  auto neg_source = Sacado::createDynRankView(Source.get_view(), "neg_Source", Source.extent(0), Source.extent(1));
  if (haveSource) {
    for (int i = 0; i < Source.extent(0); i++)
      for (int j = 0; j < Source.extent(1); j++) neg_source(i, j) = -1.0 * Source(i, j);
    FST::integrate(PhiResidual.get_view(), neg_source, wBF.get_view(),
                   true);  // "true" sums into
  }
}
//*****
