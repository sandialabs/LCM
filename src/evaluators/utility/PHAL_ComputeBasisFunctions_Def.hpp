...
template <typename EvalT, typename Traits>
void
ComputeBasisFunctions<EvalT, Traits>::postRegistrationSetup(typename Traits::SetupData d, PHX::FieldManager<Traits>& fm)
{
  ...
  jacobian     = Sacado::createDynRankView(jacobian_det.get_view(), "XXX", numCells, numQPs, numDims, numDims);
  jacobian_inv = Sacado::createDynRankView(jacobian_det.get_view(), "XXX", numCells, numQPs, numDims, numDims);
...
template <typename EvalT, typename Traits>
void
ComputeBasisFunctions<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
