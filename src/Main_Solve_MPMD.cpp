...
/******************************************************************************/
{
 public:
  ...
  void
  importData(const std::string& name, const Plato::SharedData& sf);
  void
  exportData(const std::string& name, Plato::SharedData& sf);
  void
  exportDataMap(
 ...
 private:
  ...
  void
  copyFieldIntoState(const std::string& name, const Plato::SharedData& sf);
  void
  copyFieldFromState(const std::string& name, Plato::SharedData& sf);
  void
  copyFieldIntoDistParam(const std::string& name, const Plato::SharedData& sf);
  void
  copyFieldFromDistParam(const std::string& name, Plato::SharedData& sf);

  void
  copyValueFromState(const std::string& name, Plato::SharedData& sf);

  bool
  isElemNodeState(std::string localFieldName);
  bool
  isDistParam(std::string localFieldName);

  void
  addField(pugi::xml_node&);
  void
  addValue(pugi::xml_node&);
...
/******************************************************************************/
{
  ...
  try {
    ...
    Teuchos::RCP<Teuchos::ParameterList> appParams =
        Teuchos::createParameterList("Albany Parameters");
    Teuchos::updateParametersFromXmlFileAndBroadcast(
        cmd.yaml_filename, appParams.ptr(), *m_comm);
    ...
    Teuchos::ParameterList& topoParams =
        probParams.get<Teuchos::ParameterList>("Topologies");
    int ntopos = topoParams.get<int>("Number of Topologies");

    Teuchos::RCP<ATO::TopologyArray> topologyArray =
        Teuchos::rcp(new Teuchos::Array<Teuchos::RCP<ATO::Topology>>(ntopos));
    for (int itopo = 0; itopo < ntopos; itopo++) {
      Teuchos::ParameterList& tParams =
          topoParams.sublist(Albany::strint("Topology", itopo));
      (*topologyArray)[itopo] = Teuchos::rcp(new ATO::Topology(tParams, itopo));
    }

    // add topology objects
    probParams.set<Teuchos::RCP<ATO::TopologyArray>>(
        "Topologies", topologyArray);
...
/******************************************************************************/
{
  ...
  if (m_inputTree.next_sibling("Operation")) {
    ...
    message << "Only one 'Operation' definition allowed per Albany performer.  "
               "Multiple provided."
            << std::endl;
...
/******************************************************************************/
void
MPMD_App::addValue(pugi::xml_node& inputNode)
/******************************************************************************/
{
  ...
  if (isElemNodeState(localName)) {
    ...
    std::stringstream message;
    message << "Cannot find specified LocalName: '" << localName << "'"
            << std::endl;
    Plato::ParsingException pe(message.str());
    throw pe;
  }
...
/******************************************************************************/
void
MPMD_App::addField(pugi::xml_node& inputNode)
/******************************************************************************/
{
  string argumentName = Plato::Parse::getString(inputNode, "ArgumentName");
...
/******************************************************************************/
bool
MPMD_App::isElemNodeState(std::string localFieldName)
/******************************************************************************/
{
  Albany::StateManager&  stateMgr     = m_app->getStateMgr();
  ...
  if (numWorksets) {
    ...
    if (it != elemNodeFlds[0].end()) { return true; }
  }
  return false;
}
/******************************************************************************/
...
/******************************************************************************/
{
  Teuchos::ParameterList& solveParams =
      m_solverFactory->getAnalysisParameters().sublist("Solve", false);

  Teuchos::Array<Teuchos::RCP<const Thyra::VectorBase<ST>>> thyraResponses;
  Teuchos::Array<Teuchos::Array<Teuchos::RCP<const Thyra::MultiVectorBase<ST>>>>
      thyraSensitivities;

  Piro::PerformSolve(
      *m_solver, solveParams, thyraResponses, thyraSensitivities);
}
...
/******************************************************************************/
void
MPMD_App::importData(const std::string& name, const Plato::SharedData& sf)
/******************************************************************************/
{
  ...
  } else if (sf.myLayout() == Plato::data::layout_t::SCALAR) {
  }
}

/******************************************************************************/
void
MPMD_App::copyFieldIntoState(
...
/******************************************************************************/
{
  ...
  m_cas_manager->scatter(
      m_localVector, m_overlapVector, Albany::CombineMode::INSERT);
  Teuchos::RCP<Albany::NodeFieldContainer> nodeContainer =
      stateMgr.getNodalDataBase()->getNodeContainer();
  auto it = nodeContainer->find(name + "_node");
  if (it != nodeContainer->end())
    (*nodeContainer)[name + "_node"]->saveFieldVector(
        m_overlapVector, /*offset=*/0);

  // copy the field into the state manager
  Teuchos::RCP<const Thyra_VectorSpace> overlapNodeVS =
      m_overlapVector->space();
  Teuchos::ArrayRCP<double> otopo =
      Albany::getNonconstLocalData(m_overlapVector);
...
/******************************************************************************/
void
MPMD_App::copyValueFromState(const std::string& name, Plato::SharedData& sf)
/******************************************************************************/
{
  Albany::StateManager& stateMgr = m_app->getStateMgr();
...
/******************************************************************************/
void
MPMD_App::copyFieldFromState(const std::string& name, Plato::SharedData& sf)
/******************************************************************************/
{
  Albany::StateManager& stateMgr = m_app->getStateMgr();
  ...
  auto ov_vs_indexer =
      Albany::createGlobalLocalIndexer(m_overlapVector->space());
  for (int ws = 0; ws < numWorksets; ws++) {
  ...
  m_localVector->assign(0.0);
  m_cas_manager->combine(
      m_overlapVector, m_localVector, Albany::CombineMode::ADD);
  Teuchos::ArrayRCP<double> ltopo = Albany::getNonconstLocalData(m_localVector);
...
/******************************************************************************/
void
MPMD_App::copyFieldIntoDistParam(
    const std::string& /* name */,
    const Plato::SharedData& /* sf */)
/******************************************************************************/
{
...
/******************************************************************************/
void
MPMD_App::copyFieldFromDistParam(
    const std::string& /* name */,
    Plato::SharedData& /* sf */)
/******************************************************************************/
{
    ...
    std::vector<int>&            aMyOwnedGlobalIDs)
{
  ...
  if (aDataLayout == Plato::data::layout_t::SCALAR_FIELD) {
    ...
    for (int lid = 0; lid < numLocalVals; lid++) {
      aMyOwnedGlobalIDs[lid] =
          vs_indexer->getGlobalElement(lid) + 1;  // Albany's gids start from 0
    }
  }
}
/******************************************************************************/
...
/******************************************************************************/
void
MPMD_App::exportData(const std::string& name, Plato::SharedData& sf)
/******************************************************************************/
{
  if (sf.myLayout() == Plato::data::layout_t::SCALAR_FIELD) {
    {
...
/******************************************************************************/
{
  ...
  for (auto f : valueMap) { message << f.first << std::endl; }
  Plato::ParsingException pe(message.str());
  throw pe;
}
...
/******************************************************************************/
{
  ...
  for (auto f : fieldMap) { message << f.first << std::endl; }
  Plato::ParsingException pe(message.str());
  throw pe;
}
