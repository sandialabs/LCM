    ...
    postEvaluate(typename Traits::PostEvalData workset)
{
  ...
  if (g != Teuchos::null) {
    ...
    for (int res = 0; res < this->field_components.size(); res++) {
      g_nonconstView[res] =
          this->global_response(this->field_components[res]).val();
    }
  }
  ...
  Teuchos::ArrayRCP<Teuchos::ArrayRCP<ST>> overlapped_dg_nonconst2dView =
      Albany::getNonconstLocalData(overlapped_dg);
    ...
    for (int res = 0; res < this->field_components.size(); res++) {
      ...
      for (int node_dof = 0; node_dof < numNodes; node_dof++) {
        ...
        for (int eq_dof = 0; eq_dof < neq; eq_dof++) {
          ...
          overlapped_dg_nonconst2dView[res][dof] =
              this->global_response(this->field_components[res]).dx(deriv);
...
template <typename Traits>
void
QCAD::FieldValueScatterScalarResponse<
    PHAL::AlbanyTraits::DistParamDeriv,
    Traits>::postEvaluate(typename Traits::PostEvalData workset)
{
  /*
    // Here we scatter the *global* response
...
QCAD::ResponseFieldValue<EvalT, Traits>::ResponseFieldValue(
    ...
    : coordVec("Coord Vec", dl->qp_vector), weights("Weights", dl->qp_scalar)
{
  ...
  if (plist->isParameter("Operation Vector Field Name")) {
    ...
    opFieldName = plist->get<std::string>("Operation Field Name");
  ...
  if (plist->isParameter("Return Vector Field Name")) {
    ...
    retFieldName = plist->get<std::string>("Return Field Name", opFieldName);
  ...
  if (!bReturnOpField)
    this->addDependentField(
        retField.fieldTag());  // when return field is *different* from op field
...
template <typename EvalT, typename Traits>
void
QCAD::ResponseFieldValue<EvalT, Traits>::postRegistrationSetup(
    ...
    PHX::FieldManager<Traits>& fm)
{
  ...
  QCAD::FieldValueScatterScalarResponse<EvalT, Traits>::postRegistrationSetup(
      d, fm);
}
...
template <typename EvalT, typename Traits>
void
QCAD::ResponseFieldValue<EvalT, Traits>::evaluateFields(
    typename Traits::EvalData workset)
{
  ScalarT opVal, qpVal, cellVol;

  if (!opRegion->elementBlockIsInRegion(workset.EBName)) return;
  ...
  for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
    ...
    cellVol = 0.0;
    for (std::size_t qp = 0; qp < numQPs; ++qp) cellVol += weights(cell, qp);
...
template <typename EvalT, typename Traits>
void
QCAD::ResponseFieldValue<EvalT, Traits>::postEvaluate(
    typename Traits::PostEvalData workset)
{
  ...
  Teuchos::reduceAll<int, ScalarT>(*workset.comm, reductType, 1, &send, &max);
...
QCAD::ResponseFieldValue<EvalT, Traits>::getValidResponseParameters() const
{
  ...
  validPL->set<std::string>(
      "Operation Field Name", "", "Scalar field to perform operation on");
  validPL->set<std::string>(
      "Operation Vector Field Name",
      "",
      "Vector field to perform operation on");
  validPL->set<std::string>(
      "Return Field Name",
