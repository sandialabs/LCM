...
QuadPointsToCellInterpolationBase<EvalT, Traits, ScalarT>::QuadPointsToCellInterpolationBase(
...
template <typename EvalT, typename Traits, typename ScalarT>
void
QuadPointsToCellInterpolationBase<EvalT, Traits, ScalarT>::evaluateFields(typename Traits::EvalData workset)
{
  MeshScalarT meas;
  ...
  for (int cell = 0; cell < workset.numCells; ++cell) {
    ...
    {
      field_cell(cell) = 0;
      for (int qp(0); qp < numQPs; ++qp) field_cell(cell) += field_qp(cell, qp) * w_measure(cell, qp);
      field_cell(cell) /= meas;
    } else if (qp_dims.size() == 3)  // vector
    {
      for (int dim(0); dim < qp_dims[2]; ++dim) {
        field_cell(cell, dim) = 0;
        for (int qp(0); qp < numQPs; ++qp) field_cell(cell, dim) += field_qp(cell, qp, dim) * w_measure(cell, qp);
