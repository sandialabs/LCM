...
#define message(verbosity, lvl, e)                         \
  do {                                                     \
    if (verbosity >= lvl) {                                \
      std::stringstream ss;                                \
      ss << e;                                             \
...
Environment::Environment() { comm_ = Tpetra::getDefaultComm(); }

bool
parse_cmd_line(int argc, char** argv, Input& in)
{
  in.verbosity = 1;
  bopt::options_description od("options");
  od.add_options()("help,h", "Help message.")(
      "verbosity,v",
      bopt::value<int>(&in.verbosity),
      "Verbosity level: 0 (none) to 10 (high).")(
      "file,f", bopt::value<std::string>(&in.filename), "Input mesh.")(
      "modfile,m",
      bopt::value<std::string>(&in.model_filename),
...
std::string
get_file_extension(const std::string& file)
{
...
Direction::Enum
get_direction(const std::string& file)
{
...
read_exodus(const Input& in, const Environment& env)
{
  ...
  return Teuchos::rcp(new Albany::IossSTKMeshStruct(
      Teuchos::rcp(&p, false), Teuchos::null, env.comm()));
}

void
exo2pumi(const Input& in)
{
  Environment                             env;
...
Teuchos::RCP<apf::Mesh2>
read_pumi(const Input& in, const Environment& env)
{
  ...
  gmi_register_mesh();
  if (get_file_extension(model_filename) == "null") gmi_register_null();
