...
template <typename EvalT, typename Traits>
void
CahnHillRhoResid<EvalT, Traits>::postRegistrationSetup(typename Traits::SetupData d, PHX::FieldManager<Traits>& fm)
{
  this->utils.setFieldData(wBF, fm);
  this->utils.setFieldData(rhoGrad, fm);
  this->utils.setFieldData(wGradBF, fm);
  this->utils.setFieldData(chemTerm, fm);
  if (haveNoise) this->utils.setFieldData(noiseTerm, fm);
...
template <typename EvalT, typename Traits>
void
CahnHillRhoResid<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  // Form Equation 2.2
  ...
  for (std::size_t cell = 0; cell < workset.numCells; ++cell)
    for (std::size_t qp = 0; qp < numQPs; ++qp)
      for (std::size_t i = 0; i < numDims; ++i) gamma_term(cell, qp, i) = rhoGrad(cell, qp, i) * gamma;

  FST::integrate(rhoResidual.get_view(), gamma_term, wGradBF.get_view(),
