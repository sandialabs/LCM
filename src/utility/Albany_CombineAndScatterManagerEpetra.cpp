...
namespace {
Epetra_CombineMode
combineModeE(const Albany::CombineMode modeA)
{
  Epetra_CombineMode modeE;
  switch (modeA) {
    case Albany::CombineMode::ADD: modeE = Epetra_CombineMode::Add; break;
    case Albany::CombineMode::INSERT: modeE = Epetra_CombineMode::Insert; break;
    case Albany::CombineMode::ZERO: modeE = Epetra_CombineMode::Zero; break;
    case Albany::CombineMode::ABSMAX: modeE = Epetra_CombineMode::AbsMax; break;
    default:
...
CombineAndScatterManagerEpetra::CombineAndScatterManagerEpetra(
...
void
CombineAndScatterManagerEpetra::combine(
    const Thyra_Vector& src,
    Thyra_Vector&       dst,
...
#ifdef ALBANY_DEBUG
  TEUCHOS_TEST_FOR_EXCEPTION(
      !srcE->getMap()->isSameAs(*importer->getTargetMap()),
      std::runtime_error,
      "Error! The map of the input src vector does not match the importer's "
      "target map.\n");
  TEUCHOS_TEST_FOR_EXCEPTION(
      !dstE->getMap()->isSameAs(*importer->getSourceMap()),
      std::runtime_error,
    ...
    const CombineMode        CM) const
{
  ...
  // Legend: V = Vector, MV = MultiVector, TV = Epetra_Vector, TMV =
  // Epetra_MultiVector, T_xyz = Thyra_xyz One can create a T_TV, then pass it
  ...
  // duplication), since T_T(M)V store ConstNonConstObj containers to the Epetra
  ...
  Teuchos::RCP<const Epetra_MultiVector> srcE =
      getConstEpetraMultiVector(src, false);
  ...
  if (srcE.is_null()) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        srcV == nullptr,
        std::runtime_error,
        "Error! Input src does not seem to be a Epetra_MultiVector or a "
        "Epetra_Vector.\n");
  ...
  if (dstE.is_null()) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        dstV == nullptr,
        std::runtime_error,
        "Error! Input dst does not seem to be a Epetra_MultiVector or a "
        "Epetra_Vector.\n");
...
#ifdef ALBANY_DEBUG
  TEUCHOS_TEST_FOR_EXCEPTION(
      !srcE->getMap()->isSameAs(*importer->getSourceMap()),
      std::runtime_error,
      "Error! The row map of the input src matrix does not match the "
      "importer's source map.\n");
  TEUCHOS_TEST_FOR_EXCEPTION(
      !dstE->getMap()->isSameAs(*importer->getTargetMap()),
      std::runtime_error,
...
// Scatter methods
void
CombineAndScatterManagerEpetra::scatter(
    const Thyra_Vector& src,
    Thyra_Vector&       dst,
    const CombineMode   CM) const
{
...
void
CombineAndScatterManagerEpetra::create_ghosted_aura_owners() const
{
  ...
  if (sizeof(GO) == sizeof(Epetra_GO)) {
    ...
    egids = Teuchos::arrayView(
        reinterpret_cast<Epetra_GO*>(gids.getRawPtr()), gids.size());
  } else {
    ...
    for (int i = 0; i < num_gids; ++i) {
      ALBANY_EXPECT(
          gids[i] <= max_safe_gid,
          "Error in create_ghosted_aura_owners! Input gids exceed Epetra_GO "
          "ranges.\n");
  ...
  auto err = map->RemoteIDList(
      num_gids, egids.getRawPtr(), ghosted_aura_owners.getRawPtr(), nullptr);

  TEUCHOS_TEST_FOR_EXCEPTION(
      err != 0,
      std::runtime_error,
      "Error! Something went wrong while computing remote lids.\n");
}

void
CombineAndScatterManagerEpetra::create_owned_aura_users() const
{
