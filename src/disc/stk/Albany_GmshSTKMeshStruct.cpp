...
void
Albany::GmshSTKMeshStruct::init_counters_to_zero()
{
  NumSides = 0;
  NumNodes = 0;
  NumElems = 0;
...
void
Albany::GmshSTKMeshStruct::init_pointers_to_null()
{
  pts   = nullptr;
  tetra = nullptr;
  tet10 = nullptr;
        ...
        AbstractFieldContainer::FieldContainerRequirements> const& side_set_req)
{
  ...
  if (commT->getRank() == 0) {
    ...
    for (int i = 0; i < NumSides; i++) {
      ...
      // node connectivity In particular, the element that is connected to all
      // NumSideNodes nodes is the one.
      bool found = false;
...
Albany::GmshSTKMeshStruct::getValidDiscretizationParameters() const
{
  ...
  validPL->set<std::string>(
      ...
      "mesh.msh",
      "Name of the file containing the 2D mesh, with list of coordinates, "
      "elements' connectivity and boundary edges' connectivity");
...
void
Albany::GmshSTKMeshStruct::loadLegacyMesh()
{
  std::ifstream ifile;
  open_fname(ifile);
  ...
  // scan the entity list once to establish what kind of elements we have. We
  // support linear Tetrahedra/Hexahedra in 3D and linear Triangle/Quads in 2D
...
void
Albany::GmshSTKMeshStruct::set_NumNodes(std::ifstream& ifile)
{
  std::string line;
...
void
Albany::GmshSTKMeshStruct::load_node_data(std::ifstream& ifile)
{
  if (version == GmshVersion::V2_2) {
    int id = 0;
    for (int i = 0; i < NumNodes; ++i) {
...
void
Albany::GmshSTKMeshStruct::set_num_entities(std::ifstream& ifile)
{
  std::string line;
  ifile.seekg(0, std::ios::beg);
...
void
Albany::GmshSTKMeshStruct::increment_element_type(int e_type)
{
  switch (e_type) {
...
void
Albany::GmshSTKMeshStruct::size_all_element_pointers()
{
...
void
Albany::GmshSTKMeshStruct::set_generic_mesh_info()
{
  if (nb_tetra > 0) {
    this->numDim = 3;
...
void
Albany::GmshSTKMeshStruct::store_element_info(
    int                e_type,
    int&               iline,
    int&               iline3,
    int&               itria,
...
void
Albany::GmshSTKMeshStruct::load_element_data(std::ifstream& ifile)
{
  // Reset the stream to the beginning of the element section
  std::string line;
  ifile.seekg(0, std::ios::beg);
...
void
Albany::GmshSTKMeshStruct::loadAsciiMesh()
{
  std::ifstream ifile;
  open_fname(ifile);
...
void
Albany::GmshSTKMeshStruct::loadBinaryMesh()
{
  std::ifstream ifile;
  open_fname(ifile);
...
void
Albany::GmshSTKMeshStruct::set_allowable_gmsh_versions()
{
...
void
Albany::GmshSTKMeshStruct::broadcast_name_tag_pair(
    std::vector<std::string>                names,
    int*                                    tags_array,
    int                                     pair_number,
    const Teuchos::RCP<Teuchos_Comm const>& commT,
