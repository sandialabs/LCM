  ...
  if (type == "Constant") {
    if (rank == 2) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(
          ...
          "Tensor constant value for material property "
              ...
              << " but expected dimensions " << numRows << "x" << numCols);
    ...
    if (type == "Truncated KL Expansion")
      matPropType = KL_RAND_FIELD;
        ...
        type == "Log Normal RF" || type == "Exponential Truncated KL Expansion")
    ...
    coordVec = decltype(coordVec)(
        p.get<std::string>("Coordinate Vector Name"), coord_dl);
    ...
    exp_rf_kl = Teuchos::rcp(
        new Stokhos::KL::ExponentialRandomField<RealType>(*mp_list));
    ...
    sigma_a     = decltype(sigma_a)(
        p.get<std::string>("Absorption Cross Section Name"), layout);
    sigma_s = decltype(sigma_s)(
        p.get<std::string>("Scattering Cross Section Name"), layout);
    mu = decltype(mu)(
        p.get<std::string>("Average Scattering Angle Name"), layout);
    ...
    timeValues =
        mp_list->get<Teuchos::Array<RealType>>("Time Values").toVector();
    depValues =
...
template <typename EvalT, typename Traits>
void
NSMaterialProperty<EvalT, Traits>::postRegistrationSetup(
...
template <typename EvalT, typename Traits>
void
NSMaterialProperty<EvalT, Traits>::evaluateFields(
    typename Traits::EvalData workset)
{
  if (matPropType == SCALAR_CONSTANT) {
    ...
    for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
      for (std::size_t qp = 0; qp < dims[1]; ++qp) {
        matprop(cell, qp) =
            scalar_constant_value / sqrt(ref_temp) * sqrt(T(cell, qp));
      }
    }
    ...
    for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
      for (std::size_t qp = 0; qp < dims[1]; ++qp) {
        matprop(cell, qp) = 1.0 / (3.0 * (1.0 - mu(cell, qp)) *
                                   (sigma_a(cell, qp) + sigma_s(cell, qp)));
      }
    }
    ...
    if (index == 0)
      ...
      slope =
          ((depValues[index] - depValues[index - 1]) /
           (timeValues[index] - timeValues[index - 1]));
      scalar_constant_value =
          depValues[index - 1] + slope * (time - timeValues[index - 1]);
    }
...
NSMaterialProperty<EvalT, Traits>::getValue(const std::string& n)
{
  if (matPropType == SCALAR_CONSTANT || matPropType == SQRT_TEMP ||
    ...
    for (std::size_t dim = 0; dim < vector_constant_value.size(); ++dim)
      if (n == Albany::strint(name_mp, dim)) return vector_constant_value[dim];
  } else if (matPropType == TENSOR_CONSTANT) {
    for (std::size_t dim1 = 0; dim1 < tensor_constant_value.getNumRows();
         ++dim1)
      for (std::size_t dim2 = 0; dim2 < tensor_constant_value.getNumCols();
           ++dim2)
    ...
    for (int i = 0; i < rv.size(); i++)
      if (n == Albany::strint(name_mp + " KL Random Variable", i)) return rv[i];
  }
#endif
  TEUCHOS_TEST_FOR_EXCEPTION(
      true,
