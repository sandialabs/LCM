    ...
    const Teuchos::RCP<Teuchos::ParameterList>& responseList)
{
  ...
  *out << "Field Dimensions: Workset=" << worksetSize
  ...
  if (number_of_time_deriv > 0) dof_names_dot[0] = "Temperature_dot";
  ...
  if (!conductivityIsDistParam) {  // Thermal conductivity
    ...
    // p->set<RCP<DistributedParameterLibrary> >("Distributed Parameter
    // Library", distParamLib);
    Teuchos::ParameterList& paramList = params->sublist("ThermalConductivity");
    p->set<Teuchos::ParameterList*>("Parameter List", &paramList);

    // Here we assume that the instance of this problem applies on a single
    // element block
  ...
  } else if (materialDB != Teuchos::null) {  // Sources can be specified in
                                             // terms of materials or element
                                             // blocks
