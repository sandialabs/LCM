      ...
      out(Teuchos::VerboseObjectBase::getDefaultOStream())
{
  ...
  if (problemParams->isSublist("Adaptation")) {  // If the user has specified adaptation on input,
                                                 // grab the sublist
  ...
  if (paramLib_->isParameter("Time")) {
    ...
    if (appParams->sublist("Problem").get<std::string>("Solution Method", "Steady") == "Continuation") {
      initialValue = appParams->sublist("Piro").sublist("LOCA").sublist("Stepper").get<double>("Initial Value", 0.0);
    } else if (appParams->sublist("Problem").get<std::string>("Solution Method", "Steady") == "Transient") {
      initialValue = appParams->sublist("Piro").sublist("Trapezoid Rule").get<double>("Initial Time", 0.0);
    }
  ...
  Teuchos::RCP<Teuchos::ParameterList> pbParams = Teuchos::sublist(appParams_, "Problem", true);
...
#if defined(ALBANY_STK_PERCEPT)
      if (method == "Unif Size") {
    adapter_ = Teuchos::rcp(new STKAdaptT<STKUnifRefineField>(adaptParams_, paramLib_, stateMgr_, comm_));
  } else
#endif
  {
    ALBANY_ABORT(
        std::endl
        << "Error! Unknown adaptivity method requested:" << method << " !" << std::endl
...
AdaptiveSolutionManager::adaptProblem()
{
  ...
  if (adapter_->adaptMesh()) {
    ...
    // If dynamic cast fails
    ALBANY_PANIC(me == Teuchos::null, std::endl << "Error! : Cast to Albany::ModelEvaluator failed!" << std::endl);
...
void
AdaptiveSolutionManager::resizeMeshDataArrays(const Teuchos::RCP<const Albany::AbstractDiscretization>& disc)
{
...
AdaptiveSolutionManager::updateAndReturnOverlapSolution(Thyra_Vector const& solution /* not overlapped */)
{
...
Teuchos::RCP<Thyra_Vector const>
AdaptiveSolutionManager::updateAndReturnOverlapSolutionDot(Thyra_Vector const& solution_dot /* not overlapped */)
{
...
Teuchos::RCP<Thyra_Vector const>
AdaptiveSolutionManager::updateAndReturnOverlapSolutionDotDot(Thyra_Vector const& solution_dotdot /* not overlapped */)
{
...
AdaptiveSolutionManager::updateAndReturnOverlapSolutionMV(const Thyra_MultiVector& solution /* not overlapped */)
{
...
void
AdaptiveSolutionManager::scatterX(const Thyra_MultiVector& solution) /* not overlapped */
{
  cas_manager->scatter(solution, *overlapped_soln, Albany::CombineMode::INSERT);
}
