...
// TpetraMap_To_EpetraMap: takes in Tpetra::Map object, converts it to its
// equivalent Epetra_Map object, and returns an RCP pointer to this Epetra_Map
Teuchos::RCP<const Epetra_Map>
Petra::TpetraMap_To_EpetraMap(
    const Teuchos::RCP<const Tpetra_Map>&  tpetraMap_,
    const Teuchos::RCP<const Epetra_Comm>& comm_)
{
  ...
  if (tpetraMap_->isDistributed() ||
      tpetraMap_->getComm()->getSize() == Teuchos::OrdinalTraits<int>::one()) {
...
// equivalent Tpetra::Map object, and returns an RCP pointer to this Tpetra::Map
Teuchos::RCP<const Tpetra_Map>
Petra::EpetraMap_To_TpetraMap(
    const Teuchos::RCP<const Epetra_Map>&         epetraMap_,
    const Teuchos::RCP<const Teuchos::Comm<int>>& commT_)
{
  const std::size_t numElements =
      Teuchos::as<std::size_t>(epetraMap_->NumMyElements());
  const auto indexBase = Teuchos::as<GO>(epetraMap_->IndexBase());
  if (epetraMap_->DistributedGlobal() ||
      epetraMap_->Comm().NumProc() == Teuchos::OrdinalTraits<int>::one()) {
    Teuchos::Array<Tpetra_GO> indices(numElements);
    int*                      epetra_indices = epetraMap_->MyGlobalElements();
    for (LO i = 0; i < numElements; i++) indices[i] = epetra_indices[i];
    const Tpetra::global_size_t computeGlobalElements =
        Teuchos::OrdinalTraits<Tpetra::global_size_t>::invalid();
    return Teuchos::rcp(
        new Tpetra_Map(computeGlobalElements, indices, indexBase, commT_));
  } else {
    return Teuchos::rcp(new Tpetra_Map(
        numElements, indexBase, commT_, Tpetra::LocallyReplicated));
  }
}
...
Teuchos::RCP<const Tpetra_Map>
Petra::EpetraMap_To_TpetraMap(
    const Epetra_BlockMap&                        epetraMap_,
    const Teuchos::RCP<const Teuchos::Comm<int>>& commT_)
{
  const std::size_t numElements =
      Teuchos::as<std::size_t>(epetraMap_.NumMyElements());
...
// TpetraCrsGraph_To_EpetraCrsGraph: takes in Tpetra::CrsGraph object, converts
// it to its equivalent Epetra_CrsGraph object, and returns an RCP pointer to
// this Epetra_CrsGraph
Teuchos::RCP<Epetra_CrsGraph>
Petra::TpetraCrsGraph_To_EpetraCrsGraph(
    const Teuchos::RCP<const Tpetra_CrsGraph>& tpetraCrsGraph_,
    const Teuchos::RCP<const Epetra_Comm>&     comm_)
{
  // get row map of Tpetra::CrsGraph & convert to Epetra::Map
  Teuchos::RCP<const Tpetra_Map> tpetraRowMap_ = tpetraCrsGraph_->getRowMap();
  Teuchos::RCP<const Epetra_Map> epetraRowMap_ =
      TpetraMap_To_EpetraMap(tpetraRowMap_, comm_);

  // get col map of Tpetra::CrsGraph & convert to Epetra::Map
  Teuchos::RCP<const Tpetra_Map> tpetraColMap_ = tpetraCrsGraph_->getColMap();
  Teuchos::RCP<const Epetra_Map> epetraColMap_ =
      TpetraMap_To_EpetraMap(tpetraColMap_, comm_);

  int maxEntries =
      Teuchos::as<int>(tpetraCrsGraph_->getGlobalMaxNumRowEntries());
  Teuchos::RCP<Epetra_CrsGraph> epetraCrsGraph_ = Teuchos::rcp(
      new Epetra_CrsGraph(Copy, *epetraRowMap_, *epetraColMap_, maxEntries));
...
// TpetraCrsMatrix_To_EpetraCrsMatrix: copies Tpetra::CrsMatrix object into its
// analogous Epetra_CrsMatrix object
Teuchos::RCP<Epetra_CrsMatrix>
Petra::TpetraCrsMatrix_To_EpetraCrsMatrix(
    const Teuchos::RCP<const Tpetra_CrsMatrix>& tpetraCrsMatrix,
    const Teuchos::RCP<const Epetra_Comm>&      comm)
{
  Teuchos::RCP<Epetra_CrsGraph> epetraCrsGraph =
      Petra::TpetraCrsGraph_To_EpetraCrsGraph(
          tpetraCrsMatrix->getCrsGraph(), comm);
  Teuchos::RCP<Epetra_CrsMatrix> epetraCrsMatrix =
      Teuchos::rcp(new Epetra_CrsMatrix(Copy, *epetraCrsGraph));
...
void
Petra::TpetraCrsMatrix_To_EpetraCrsMatrix(
    const Teuchos::RCP<const Tpetra_CrsMatrix>& tpetraCrsMatrix_,
    ...
    const Teuchos::RCP<const Epetra_Comm>&      comm_)
{
  ...
  Teuchos::RCP<const Epetra_Map> tpetraRowMapE_ =
      TpetraMap_To_EpetraMap(tpetraRowMap_, comm_);
  bool isRowSame = tpetraRowMapE_->SameAs(epetraRowMap_);
  // if epetraCrsMatrix_ and tpetraCrsMatrix_ do not have the same row map,
  // throw an exception
  if (isRowSame != true) {
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      (isRowSame != true),
      std::logic_error,
      "Error in Petra::TpetraCrsMatrix_To_EpetraCrsMatrix! Arguments "
      "Epetra_CrsMatrix and Tpetra::CrsMatrix do not have same row map."
          << std::endl);
  ...
  Teuchos::RCP<const Epetra_Map> tpetraColMapE_ =
      TpetraMap_To_EpetraMap(tpetraColMap_, comm_);
  bool isColSame = tpetraColMapE_->SameAs(epetraColMap_);
  // if epetraCrsMatrix_ and tpetraCrsMatrix_ do not have the same column map,
  // throw an exception
  TEUCHOS_TEST_FOR_EXCEPTION(
      (isColSame != true),
      std::logic_error,
      "Error in Petra::TpetraCrsMatrix_To_EpetraCrsMatrix! Arguments "
      "Epetra_CrsMatrix and Tpetra::CrsMatrix do not have same column map."
          << std::endl);
...
Petra::TpetraVector_To_EpetraVector(
    const Teuchos::RCP<const Tpetra_Vector>& tpetraVector_,
    ...
    const Teuchos::RCP<const Epetra_Comm>&   comm_)
{
  ...
  // Teuchos::RCP<Epetra_Map> tpetraMapE_ = TpetraMap_To_EpetraMap(tpetraMap_,
  // comm_); bool isSame = tpetraMapE_->SameAs(epetraMap_); if epetraVector_ and
  // tpetraVector_ do not have the same map, throw an exception
  // TEUCHOS_TEST_FOR_EXCEPTION((isSame != true),
  //                          std::logic_error,
  //                          "Error in Petra::TpetraVector_To_EpetraVector!
  //                          Arguments Epetra_Vector and Tpetra::Vector do not
  //                          have same map." <<  std::endl) ;
  ...
  for (std::size_t i = 0; i < tpetraVector_->getMap()->getNodeNumElements();
       ++i)
    ...
    const Teuchos::RCP<const Epetra_Comm>&   comm_)
{
  ...
  if (Teuchos::is_null(epetraVector_)) {
    ...
    Teuchos::RCP<const Epetra_Map> emap =
        TpetraMap_To_EpetraMap(tpetraMap, comm_);
...
// TpetraMultiVector_To_EpetraMultiVector: copies Tpetra::MultiVector object
// into its analogous Epetra_MultiVector object
void
Petra::TpetraMultiVector_To_EpetraMultiVector(
    const Teuchos::RCP<const Tpetra_MultiVector>& tpetraMV_,
    Epetra_MultiVector&                           epetraMV_,
    const Teuchos::RCP<const Epetra_Comm>&        comm_)
{
...
// EpetraVector_To_TpetraVectorConst: copies Epetra_Vector to const
// Tpetra_Vector
Teuchos::RCP<const Tpetra_Vector>
Petra::EpetraVector_To_TpetraVectorConst(
    const Epetra_Vector&                          epetraVector_,
    const Teuchos::RCP<const Teuchos::Comm<int>>& commT_)
{
  ...
  Teuchos::ArrayView<ST> valuesAV =
      Teuchos::arrayView(values, mapT->getNodeNumElements());
  Teuchos::RCP<const Tpetra_Vector> tpetraVector_ =
      Teuchos::rcp(new Tpetra_Vector(mapT, valuesAV));
  return tpetraVector_;
}

// EpetraMultiVector_To_TpetraMultiVector: copies Epetra_MultiVector to
// non-const Tpetra_MultiVector
Teuchos::RCP<Tpetra_MultiVector>
Petra::EpetraMultiVector_To_TpetraMultiVector(
    const Epetra_MultiVector&                     epetraMV_,
    const Teuchos::RCP<const Teuchos::Comm<int>>& commT_)
{
  ...
  Teuchos::ArrayView<ST> valuesAV =
      Teuchos::arrayView(values, Length * numVectors);
  // create Tpetra_MultiVector copy of epetraMV_
  Teuchos::RCP<Tpetra_MultiVector> tpetraMV_ =
      Teuchos::rcp(new Tpetra_MultiVector(mapT, valuesAV, Length, numVectors));
  return tpetraMV_;
}

// EpetraVector_To_TpetraVectorNonConst: copies Epetra_Vector to non-const
// Tpetra_Vector
Teuchos::RCP<Tpetra_Vector>
Petra::EpetraVector_To_TpetraVectorNonConst(
    const Epetra_Vector&                          epetraVector_,
    const Teuchos::RCP<const Teuchos::Comm<int>>& commT_)
{
  // get map from epetraVector_ and convert to Tpetra::Map
  ...
  Teuchos::ArrayView<ST> valuesAV =
      Teuchos::arrayView(values, mapT->getNodeNumElements());
  Teuchos::RCP<Tpetra_Vector> tpetraVector_ =
      Teuchos::rcp(new Tpetra_Vector(mapT, valuesAV));
  return tpetraVector_;
}

// EpetraCrsMatrix_To_TpetraCrsMatrix: copies Epetra_CrsMatrix to its analogous
// Tpetra_CrsMatrix
Teuchos::RCP<Tpetra_CrsMatrix>
Petra::EpetraCrsMatrix_To_TpetraCrsMatrix(
    const Epetra_CrsMatrix&                       epetraCrsMatrix_,
    const Teuchos::RCP<const Teuchos::Comm<int>>& commT_)
{
  // get row map of Epetra::CrsMatrix & convert to Tpetra::Map
  auto tpetraRowMap_ =
      EpetraMap_To_TpetraMap(epetraCrsMatrix_.RowMap(), commT_);

  // get col map of Epetra::CrsMatrix & convert to Tpetra::Map
  auto tpetraColMap_ =
      EpetraMap_To_TpetraMap(epetraCrsMatrix_.ColMap(), commT_);
  ...
  Teuchos::RCP<Tpetra_CrsGraph> tpetraCrsGraph_ = Teuchos::rcp(
      new Tpetra_CrsGraph(tpetraRowMap_, tpetraColMap_, maxEntries));
  ...
  // convert Epetra::CrsMatrix to Tpetra::CrsMatrix, after creating
  // Tpetra::CrsMatrix based on above Tpetra::CrsGraph
  Teuchos::RCP<Tpetra_CrsMatrix> tpetraCrsMatrix_ =
      Teuchos::rcp(new Tpetra_CrsMatrix(tpetraCrsGraph_));
...
Petra::Converter::Converter(const Teuchos::RCP<const Teuchos_Comm>& commT)
    : commT_(commT), commE_(Albany::createEpetraCommFromTeuchosComm(commT))
{
}
