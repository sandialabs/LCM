...
QCAD::ResponseFieldIntegral<EvalT, Traits>::ResponseFieldIntegral(
    Teuchos::ParameterList&              p,
    const Teuchos::RCP<Albany::Layouts>& dl)
    : coordVec("Coord Vec", dl->qp_vector), weights("Weights", dl->qp_scalar)
{
  ...
  if (paramsFromProblem != Teuchos::null) {
    // Material database
    materialDB = paramsFromProblem->get<Teuchos::RCP<Albany::MaterialDatabase>>(
        "MaterialDB");
    ...
    length_unit_in_m =
        1.0e-6;  // default length unit = microns (backward compat)
  }
  ...
  //! Initialize Region
  opRegion = Teuchos::rcp(new QCAD::MeshRegion<EvalT, Traits>(
      "Coord Vec", "Weights", *plist, materialDB, dl));
  ...
  if (fieldName.length() > 0) {
    ...
    if (fieldName.length() > 0)
      fieldIsComplex.push_back(true);
    else
      fieldIsComplex.push_back(false);
  }
  ...
  for (int i = 1; i < QCAD::MAX_FIELDNAMES_IN_INTEGRAL; i++) {
    ...
    if (fieldName.length() > 0) {
      fieldNames.push_back(fieldName);
      conjugateFieldFlag.push_back(
          plist->get<bool>(Albany::strint("Conjugate Field", i), false));

      fieldName =
          plist->get<std::string>(Albany::strint("Field Name Im", i), "");
      fieldNames_Imag.push_back(fieldName);
      if (fieldName.length() > 0)
        fieldIsComplex.push_back(true);
      else
  ...
  std::string integrandLinLengthUnit;  // linear length unit of integrand (e.g.
                                       // "cm" for integrand in cm^-3)
  integrandLinLengthUnit =
      plist->get<std::string>("Integrand Length Unit", "cm");
  ...
  if (integrandLinLengthUnit == "m")
    integrand_length_unit_in_m = 1.0;
  else if (integrandLinLengthUnit == "cm")
    integrand_length_unit_in_m = 1e-2;
  else if (integrandLinLengthUnit == "um")
    integrand_length_unit_in_m = 1e-6;
  else if (integrandLinLengthUnit == "nm")
    integrand_length_unit_in_m = 1e-9;
  else if (integrandLinLengthUnit == "mesh")
    integrand_length_unit_in_m = length_unit_in_m;
  else
    integrand_length_unit_in_m =
        length_unit_in_m;  // assume same unit as mesh (e.g. if unit string is
                           // blank)

  double X0 = length_unit_in_m /
              integrand_length_unit_in_m;  // length scaling to get to
                                           // integrand's lenght unit
  ...
  for (std::size_t i = 0; i < fieldNames.size(); i++) {
    ...
    PHX::MDField<const ScalarT, Cell, QuadPoint> fi(
        fieldNames_Imag[i], scalar_dl);
...
template <typename EvalT, typename Traits>
void
QCAD::ResponseFieldIntegral<EvalT, Traits>::postRegistrationSetup(
...
template <typename EvalT, typename Traits>
void
QCAD::ResponseFieldIntegral<EvalT, Traits>::preEvaluate(
...
template <typename EvalT, typename Traits>
void
QCAD::ResponseFieldIntegral<EvalT, Traits>::evaluateFields(
  ...
  if (opRegion->elementBlockIsInRegion(workset.EBName)) {
    ...
    for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
      ...
      for (std::size_t qp = 0; qp < numQPs; ++qp) {
        ...
        // Loop over all possible combinations of Re/Im parts which form product
        // terms and
        // add the relevant ones (depending on whether we're returning the
        // overall real or imaginary part of the integral) to get the integrand
        // value for this (cell,qp). We do this by mapping the Re/Im choice onto
        // a string of N bits, where N is the number of fields being multiplied
        // together. (0 = RePart, 1 = ImPart)
        ...
        for (std::size_t i = 0; i < max; i++) {
          ...
          for (n = 0; n < nBits; n++) {
            if ((0x1 << n) &
                i) {  // if n-th bit of i is set (use Im part of n-th field)
          ...
          if (n < nBits)
            continue;  // we exited early, signaling this product can't
                       // contribute

          // check if this combination of Re/Im parts contributes to the overall
          // Re or Im part we return
          if ((bReturnImagPart && nOneBits % 2) ||
              (!bReturnImagPart && nOneBits % 2 == 0)) {
            term = (nOneBits % 4 >= 2) ? -1.0 :
                                         1.0;  // apply minus sign if nOneBits %
                                               // 4 == 2 (-1) or == 3 (-i)
            if (nExtraMinuses % 2)
              term *= -1.0;  // apply minus sign due to conjugations
            // nContrib2++;
            ...
            for (std::size_t m = 0; m < nBits; m++) {
              if ((0x1 << m) & i) {
                ...
                // if( abs(fields_Imag[m](cell,qp)) > dbMaxIm[m]) dbMaxIm[m] =
                // abs(fields_Imag[m](cell,qp));
              } else {
                term *= fields[m](cell, qp);
                // if( abs(fields[m](cell,qp)) > dbMaxRe[m]) dbMaxRe[m] =
                // abs(fields[m](cell,qp));
              }
            }
        ...
        // dbI += val; //DEBUG
        // std::cout << "local response size = " << this->local_response.size()
        // << std::endl; std::cout << "cell = " << cell << std::endl; std::cout
        // << "workset size = " << workset.numCells << std::endl;
    ...
    /*if(fieldNames.size() > 1) {
      std::cout << "DB: " << (bReturnImagPart == true ? "Im" : "Re") << " Field
      Integral - int("; for(std::size_t i=0; i<fieldNames.size(); i++) std::cout
      << fieldNames[i] << "," << (conjugateFieldFlag[i] ? "-" : "") <<
      (fieldIsComplex[i] ? fieldNames_Imag[i] : "X") << " * "; std::cout << "
      dV) -- I += " << dbI << "  (ebName = " << workset.EBName << " contrib1="
      << nContrib1 << " contrib2=" << nContrib2 << ")" << std::endl; std::cout
      << "DB MAX of Fields Re: " << dbMaxRe[0] << "," << dbMaxRe[1] << "," <<
      dbMaxRe[2] << "," << dbMaxRe[3] << std::endl; std::cout << "DB MAX of
      Fields Im: " << dbMaxIm[0] << "," << dbMaxIm[1] << "," << dbMaxIm[2] <<
      "," << dbMaxIm[3] << std::endl;
      }*/
  }
...
template <typename EvalT, typename Traits>
void
QCAD::ResponseFieldIntegral<EvalT, Traits>::postEvaluate(
...
QCAD::ResponseFieldIntegral<EvalT, Traits>::getValidResponseParameters() const
{
  ...
  validPL->set<bool>(
      "Conjugate Field",
      false,
      "Whether a (complex-valued) field should be conjugated in product of "
      "fields");
  for (int i = 1; i < QCAD::MAX_FIELDNAMES_IN_INTEGRAL; i++)
    validPL->set<std::string>(
        Albany::strint("Field Name", i),
        "",
        "Name of Field to integrate (multiplied into integrand)");
  for (int i = 1; i < QCAD::MAX_FIELDNAMES_IN_INTEGRAL; i++)
    validPL->set<std::string>(
        Albany::strint("Field Name Im", i),
        "",
        "Name of Imaginar part of Field to integrate (multiplied into "
        "integrand)");
  for (int i = 1; i < QCAD::MAX_FIELDNAMES_IN_INTEGRAL; i++)
    validPL->set<bool>(
        Albany::strint("Conjugate Field", i),
        false,
        "Whether field should be conjugated in product of fields");

  validPL->set<std::string>(
      "Integrand Length Unit",
      "cm",
      "Linear length unit of integrand, e.g. cm for integrand in cm^-3.  Can "
      "be m, cm, um, nm, or mesh.");
  validPL->set<bool>("Positive Return Only", false);
  validPL->set<bool>(
      "Return Imaginary Part",
      false,
      "True return imaginary part of integral, False returns real part");

  validPL->set<std::string>(
      "Description",
      "",
      "Description of this response used by post processors");
