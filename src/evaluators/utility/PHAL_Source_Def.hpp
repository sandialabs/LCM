...
template <typename EvalT, typename Traits>
void
Constant<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
...
template <typename EvalT, typename Traits>
void
Trigonometric<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
  ...
  if (m_num_dim == 2) {
    ...
    std::cout << "Trigonometric source implemented only for 2D; setting f = 1 "
                 "constant source."
              << std::endl;
    for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
      for (std::size_t iqp = 0; iqp < m_num_qp; iqp++) {
...
template <typename EvalT, typename Traits>
void
Quadratic<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
...
#ifdef ALBANY_STOKHOS
template <typename EvalT, typename Traits>
class TruncatedKL : public Source_Base<EvalT, Traits>,
                    public Sacado::ParameterAccessor<EvalT, SPL_Traits>
{
 public:
  typedef typename EvalT::ScalarT     ScalarT;
...
template <typename EvalT, typename Traits>
bool
TruncatedKL<EvalT, Traits>::check_for_existance(
    Teuchos::ParameterList* source_list)
{
...
template <typename EvalT, typename Traits>
TruncatedKL<EvalT, Traits>::TruncatedKL(Teuchos::ParameterList& p)
{
  m_source_list = p.get<Teuchos::ParameterList*>("Parameter List", NULL);
  Teuchos::ParameterList& paramList =
      m_source_list->sublist("Truncated KL Expansion");

  m_exp_rf_kl = Teuchos::rcp(
      new Stokhos::KL::ExponentialRandomField<RealType>(paramList));
...
template <typename EvalT, typename Traits>
void
TruncatedKL<EvalT, Traits>::EvaluatedFields(
    Source<EvalT, Traits>&  source,
    Teuchos::ParameterList& p)
{
  Teuchos::RCP<PHX::DataLayout> dl =
...
template <typename EvalT, typename Traits>
void
TruncatedKL<EvalT, Traits>::DependentFields(
    Source<EvalT, Traits>&  source,
    Teuchos::ParameterList& p)
{
...
template <typename EvalT, typename Traits>
void
TruncatedKL<EvalT, Traits>::FieldData(
    PHX::EvaluatorUtilities<EvalT, Traits>& utils,
    PHX::FieldManager<Traits>&              fm)
{
  utils.setFieldData(m_source, fm);
...
template <typename EvalT, typename Traits>
void
TruncatedKL<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
...
TruncatedKL<EvalT, Traits>::getValue(const std::string& n)
{
  for (int i = 0; i < m_rv.size(); i++) {
    if (n == Albany::strint(param_name_base, i)) return m_rv[i];
  }
  TEUCHOS_TEST_FOR_EXCEPTION(
      true,
      Teuchos::Exceptions::InvalidParameter,
...
template <typename EvalT, typename Traits>
class NeutronFission : public Source_Base<EvalT, Traits>
{
 public:
  typedef typename EvalT::ScalarT     ScalarT;
  typedef typename EvalT::MeshScalarT MeshScalarT;
...
template <typename EvalT>
inline bool
Gaussian<EvalT>::check_for_existance(Teuchos::ParameterList& source_list)
{
...
class Wavelet_Base
{
 public:
  ...
  virtual RealType
  evaluateFields(const RealType time) = 0;
};
...
class Monotone : public Wavelet_Base
{
 public:
  ...
  virtual ~Monotone() {}
  virtual RealType
  evaluateFields(const RealType time);

 private:
};

inline bool
Monotone::check_for_existance(Teuchos::ParameterList& source_list)
{
...
inline RealType
Monotone::evaluateFields(const RealType time)
{
  return 1.0;
}
...
template <typename EvalT, typename Traits>
void
PointSource<EvalT, Traits>::evaluateFields(typename Traits::EvalData workset)
{
