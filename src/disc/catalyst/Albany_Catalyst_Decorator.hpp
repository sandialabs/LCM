...
class Decorator : public AbstractDiscretization
{
 public:
  ...
  getMapT() const override;
  //! Get Tpetra DOF map
  Teuchos::RCP<const Tpetra_Map>
  getMapT(const std::string& field_name) const override;

  //! Get overlapped DOF map
  Teuchos::RCP<const Epetra_Map>
  getOverlapMap() const override;
  //! Get Tpetra overlapped DOF map
  Teuchos::RCP<const Tpetra_Map>
  getOverlapMapT() const override;
  //! Get field overlapped DOF map
  Teuchos::RCP<const Tpetra_Map>
  getOverlapMapT(const std::string& field_name) const override;

  //! Get Jacobian graph
  Teuchos::RCP<const Epetra_CrsGraph>
  getJacobianGraph() const override;
  //! Get Tpetra Jacobian graph
  Teuchos::RCP<const Tpetra_CrsGraph>
  getJacobianGraphT() const override;

#ifdef ALBANY_AERAS
  //! Get implicit Tpetra Jacobian graph (for Aeras hyperviscosity)
  Teuchos::RCP<const Tpetra_CrsGraph>
  getImplicitJacobianGraphT() const override;
#endif

  //! Get overlap Jacobian graph
  Teuchos::RCP<const Epetra_CrsGraph>
  getOverlapJacobianGraph() const override;
  //! Get Tpetra overlap Jacobian graph
  Teuchos::RCP<const Tpetra_CrsGraph>
  getOverlapJacobianGraphT() const override;
#ifdef ALBANY_AERAS
  //! Get implicit Tpetra Jacobian graph (for Aeras hyperviscosity)
  Teuchos::RCP<const Tpetra_CrsGraph>
  getImplicitOverlapJacobianGraphT() const override;
#endif

  //! Get Node map
  Teuchos::RCP<const Epetra_Map>
  getNodeMap() const override;

  //! Get Tpetra Node map
  Teuchos::RCP<const Tpetra_Map>
  getNodeMapT() const override;

  //! Get Field Node map
  Teuchos::RCP<const Tpetra_Map>
  getNodeMapT(const std::string& field_name) const override;

#if defined(ALBANY_EPETRA)
  //! Get overlapped Node map
  Teuchos::RCP<const Epetra_Map>
  getOverlapNodeMap() const override;
#endif

  //! Get overlapped Node map
  Teuchos::RCP<const Tpetra_Map>
  getOverlapNodeMapT() const override;
  ...
  //! Get Field Node map
  Teuchos::RCP<const Tpetra_Map>
  getOverlapNodeMapT(const std::string& field_name) const override;
  ...
  newVtkUnstructuredGrid();

  Teuchos::RCP<Epetra_Vector>
  getSolutionField(bool overlapped = false) const override;

  Teuchos::RCP<Tpetra_Vector>
  getSolutionFieldT(bool overlapped = false) const override;

  Teuchos::RCP<Tpetra_MultiVector>
  getSolutionMV(bool overlapped = false) const override;

  void
  getFieldT(Tpetra_Vector& field_vector, const std::string& field_name)
      const override;
  ...
  //! Write the solution to the output file - Tpetra version. Calls next two
  //! together.
  void
  writeSolutionT(
  ...
  getLayeredMeshNumbering() override;
